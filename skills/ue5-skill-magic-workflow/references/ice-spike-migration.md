...
冰锥保留 floor(30+5L+魔攻×(1.2+0.25L)+智力×(1.2+0.25L))，1–20 级，数量 2+floor((L−1)/5)。1/6/11/16 级分别 2/3/4/5 枚；基础蓝耗 30、冷却 10 秒、悬浮 30 秒，射程 12 米、速度 24 米／秒。火球的纯魔攻、蓝耗成长和 12→8 秒是独立用户调优，不能套用给冰锥。
...
用户后续指定头顶悬浮横排：取消绕身旋转和自转，阵列随视角整体移动；根据上视锥放置，让后半冰体留在屏幕上方、尖锐前半段露出。横排槽位保持居中对称，按最新要求，每枚在槽位内用独立连续噪声做小幅上下／左右随机漂移，随机起点和速度只在凝聚时初始化，不逐帧跳位置或整排同步摆动。从各自实际位置发射，不恢复旧的环身轨道。具体位置参数和未测试边界见同一 V2 记录。
