...
状态（buff/debuff）域本身的新增或迁移（不是单技能挂状态），先读 [状态系统迁移](references/status-effects-buff-migration.md)：源侧五卷穷举→覆盖矩阵的验收方法、`UStatusEffectsComponent`/`UCombatStatusFormula` 架构合同、双侧承伤链逐段取整顺序、免疫闸门/控制阻断/净化白名单清单、事件式卡片发布与 static 缓存纪律、emoji 占位图标策略与并行工作树的 hunk 级发布坑。`status_effects.json` 是状态显示唯一目录源（改后需重启编辑器）。

释放失败、优先权打断和死亡取消要退还未释放法术的蓝并回滚冷却，先读 [施法打断退蓝](references/cast-interrupt-refund.md)。火球死亡只退本组件尚未提交的手势付款。
