...
1. **顶点密度先行**：`append_revolve_polygon` 的"填充盘"如果剖面只有一圈径向点，顶面就是一个三角形扇
   （r=R→0 一圈），**WPO 无从下手**。把剖面写成 **N 圈等半径点**（本工程用 14 圈）→ 顶面变成密集网格，
   WPO 才有顶点可动（3 个盘 ≈ 16k tri，可接受）。
2. **高度场一处定义、两处用**：同一段 HLSL（多层正弦 + 噪声 + 落水涟漪）
   → ① 乘 `WaveHeight` 接 `MP_WORLD_POSITION_OFFSET`；② 取**解析梯度**接 `MP_NORMAL`
   （`tangent_space_normal=False`，世界空间）。这样"轮廓在动"与"亮面在动"一致。
3. **要"随机"就加噪声层**：两张同噪声贴图（滚动 UV）做方向导数即可得到不规则梯度，
   比纯正弦少很多机械感。
