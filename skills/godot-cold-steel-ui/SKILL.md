...
- 属性行、状态条和装备卡等可悬停浮窗入口必须同时支持键盘/手柄焦点。使用`FOCUS_ALL`，`focus_entered`与`mouse_entered`展示同一浮窗；鼠标离开时若仍有焦点不得关闭，焦点离开时若鼠标仍在卡片内也不得关闭。面板隐藏时统一收起浮窗。
...
属性栏回归使用`tests/test_status_page.gd`，从当前`HUD` autoload取得`backpack_hud`，不要依赖已淘汰的主场景`StatusBar/BackpackHud`路径；同时验证鼠标悬停和键盘焦点均可打开属性浮窗。攻击等派生值与当前`PlayerStatus`结果比较，不写死未装备时的历史常量。
