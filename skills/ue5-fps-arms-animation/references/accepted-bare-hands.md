...
7. 弓／弩这类不用 AnimBP 的相机空间武器同样以裸手为基准：它需要**自己的原生骨架**（`SK_Axe_BareArmsV7` 用斧骨架、
   `SK_M4_BareArmsV7` 用 M4 骨架，各自独立，弓不能借用其中任何一个的绑定矩阵）。在该 profile 做出来之前，
   弓的数据键 `bow_viewmodel` / `bow_animation_prefix` 保持留空，画面是「悬空弓 + 程序化弦」——
   **刻意不用假手凑数**。结构与数据键合同见 [第一人称弓与部件表](../../ue5-weapon-workflow/references/first-person-bow-parts.md)。
...
因此参考片段（如 Paragon Sparrow 的 `idle` / `RMB_Drawback`）只需要**实测秒数**当节奏上界，不必把片段或骨架引进工程；
拉弓段用拉距比例采样同一个时钟，这就是「参考某段动作」的正确接法。完整口径见上面链接的弓部件表参考。
