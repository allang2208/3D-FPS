...
- 用户要求"保留已优化的手、接回原动画的手臂"，或嫌直臂/折臂都不对：[伸直与接回原手臂的收敛路径](references/reload-handoff.md#拉栓段返工的收敛路径伸直--接回原手臂ash-12-2026-09-25)。**手一旦被扶正，原肘位就不能照搬**（原肘圆会扫过镜头，实测眼到网格 1.2 cm）；可迁移的是原臂根、弯曲曲线与骨骼滚转，肘的方位按帧取"两骨圆上离眼最远"的近邻解，判据用眼到网格距离而不是画面内前向深度。
- 近战/换弹修完仍报"扭曲、闪动、形变"，或准备挪臂根、挪整枪：[近战返工：只转弯面](references/grip-arm-refinement.md#肘极搜索与整枪前移先判断最近的是手还是上臂ash-12-2026-09-25)。先量**每帧位移**（一帧 >5 cm 就是闪动）与**可达比**（>0.96 会退化成长直臂）；能靠"弯面绕臂轴最小旋转"解决的，不要动臂根或整枪。

- 同一把近战武器更换长柄、双手握距与配重锤联动：[改造握把的手部适配](references/modular-melee-grips.md)。保留原动作家族、接触状态和骨段长度，按实际握柄阶段调整左臂。

- 双手剑、双手近战挥砍/重击/突刺和格挡，或手腕扭转、左右臂交叉：[双手近战动作](references/two-handed-melee.md)。固定双手握点、分离剑身展示角与攻击刃向；符文剑 V21 格挡与 V22 蓄力左臂已获用户接受。2026-09-25 的 V53 剑面检视已被否定并退回；同日按 BV1zz4y117f6 正向转刀做到 V79（预备右移 + 起手下甩 + 转剑窗 0.40–0.928 s + 走路自动播），用户暂停于「基本先这样」。细节见 [双手近战动作](references/two-handed-melee.md)。
