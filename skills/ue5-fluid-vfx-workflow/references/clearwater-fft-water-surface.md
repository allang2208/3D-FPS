...
- **把中间量接 Emissive 做临时材质变体**，比反复改数值重跑 60 s 截图快得多。
  方法学沉淀在 [ue5-debug-validation 的材质探针参考](../../ue5-debug-validation/references/material-emissive-probes.md)。

### 2026-09-26 下午探针判决（21 个探针，结论以实测为准）

| 量 | 状态 | 说明 |
| --- | --- | --- |
| 世界坐标 P / Time / 标量参数 | ✅ 活 | PINS / SCLR 探针 |
| 焦散标量（材质图采样 → 节点引脚） | ✅ 活 | 网状图案完整 |
| **全部 VectorParameter（Wave×48/SunColor/SunDir/Sigma…）** | ❌ **运行时精确读零** | H/PINS/SUNV/OPTP |
| **SceneColor（半透明 Unlit 下）** | ❌ 读黑 | 海床透不进水面 |

水面 = `F·Refl(天空渐变)` 独活 → 正是平滑竖直渐变。资产侧一切正确（默认值/覆写/连线/编译/编辑器
解析全回读核验）；参数数量(到 59)/include/Time/SceneColor 引脚/unroll 循环/双节点共享/WPO 双频/
nanite 标志**逐项单独复刻全部健康**。病灶跟着**已保存的 master 资产**走（duplicate 继承），从零建
的全活。**悬置实验**：编辑器关闭后重跑 author（幂等重建）——出波=资产级陈旧状态，重建即修；仍死=
从零可复现病例，离线二分到翻译器。

### 向量修好后仍会歪的六个移植缺陷（正本在 Docs/Fluids/clearwater-water-migration-20260926.md §7.5；相机俯仰符号一条属关卡细节未收入）

1. **CW_SKY 用 `.y` 当 Up**——WebGL Y-up 直译进 UE Z-up：反射天空渐变轴错、太阳瓣错轴（应 `.z`/`SunDir.z`）。
2. **高光数学上点不亮**——GlintWidening=0.0016 → Beckmann 瓣 ~2.4°，而半向量偏离 Up 30–45°；
   需按 LEAN 用实际斜率方差（目标 RMS²≈0.006+）加宽。
3. **烘焙谱偏平**——RMS 斜率 0.016 vs 参考 0.078（差 5 倍），先归一到目标斜率再出锅。
4. **折射无扭曲**——Floor 直接用未偏移 SceneColor，没有折射 UV 偏移。
5. **衰减深度是常数**——见 §10 末尾。
6. 天空/水面色还是"天空即水色"——掠射角 Fresnel→1，先把 CW_SKY 修对再调颜色。

## 10. 测试盆地的深度分区（2026-09-26 重做）

水面测试关卡的地形要**主动设计**，不是"一个碗"：

| 区 | 半径 | 床面 | 存在理由 |
| --- | --- | --- | --- |
| 深水盆 | ≤30 m | 约 −300 cm | 眼睛在脚上 ~170 cm——**深度必须 >2.2 m 站底眼睛才没入**，否则水下后处理根本测不到（旧 1.6 m 深的盆站在盆底眼睛还在水面上） |
| 缓坡 | 30–60 m | −300→−50 | 走进走出深水的过渡，坡度 ≤10% 可行走 |
| 浅水滩 | 60–80 m | 约 −50 cm±起伏 | 涉水测水线/近岸焦散/岸边交互 |
| 岸线 | 80–100 m | →+60 | 水线半径 ~8950 cm；**落点床面 +49、干舷 6.5 m** |

**传送落点必须干燥**是硬需求：把"落点半径的床面高度 + 到水线的干舷"做进烘焙脚本的自检输出
（`clearwater_meshes.py` 的 `spawn_bed_z_cm` / `spawn_dry_margin_cm`），以后改剖面不会无意识淹掉出生点。
起伏噪声在接近水线前完全衰减（水线是干净曲线，落点高度确定）。
材质的衰减深度（WaterDepthCm）仍是**均匀常数**——分区地形下它在深浅两区都不准，逐像素深度
依赖 SceneDepth（当前读黑），随向量参数修复一并处理。
