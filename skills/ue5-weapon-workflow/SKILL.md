...
- 在原厂件基础上加长/改造的配件（扩容/加长弹匣、延长枪管、导气管等）：[加长件改造规则](references/extmag-lengthening.md)——整段复制优先、不拉伸带体，带高取整数肋距、带位避开换弹抓握区，材质按"UV 岛是否改变"分两条路（原厂件原位延长＝绑该枪同类槽材质）。
...
8. 数值与说明分开维护：卡片描述只写基本描述，增减百分比由目录倍率推导后显示在详情行旁，`effects` 与 `stats` 必须同义同数；改基础数值（射程、射击间隔等）时同步枪匠总览、详情行与物品提示三处显示。
