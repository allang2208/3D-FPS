...
先读实际仓库 WORKFLOW 第 8 节及宿主 AGENTS。2026-09-10 起远端 main 是根目录 UE5 工程；旧 Godot 位于 `archive/godot-before-ue5-20260910` 标签。当前宿主 `D:/FPS3D/FPSGAME` 没有独立 Git；`E:/3d/3-dfps` 的 origin 是 `https://github.com/allang2208/3D-FPS.git`。本机工作分支可能是 master，远端默认是 main：先 fetch、读取远端 HEAD 和待推提交，不按本地分支名称猜目标。
