...
- 被拒绝的左手施法请求由快捷槽自己播提示：绑定技能报告 `IsHandOccupiedNotice()` 时，状态文字按 `FFPSLeftHandNotice` 做闪烁＋上跳（3 秒窗口），期间数字冷却读秒让位；该提示只在玩家按键被拒绝后出现，不改变技能冷却与资源状态。记录见 `Docs/Skills/left-hand-occupied-notice-20260916.md`。
- 投射物法术统一「点按发射 / 长按预览、松手发射」：悬浮（凝聚完成）时按住绑定槽，由 `FPSMagicPreview` 画红色轨迹线段（火球从悬浮球体、冰锥从各枚悬浮位置到预测撞击点，预测复用飞行扫掠与穿尸体规则），松手才真正发射；凝聚期间按下仍是原排队行为，世界交互键 E 的优先级不变。记录见 `Docs/Skills/magic-aim-preview-20260917.md`。
