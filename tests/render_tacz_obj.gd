...
func _process(_d) -> bool:
	...
	if _f == 1:
		...
		var mesh: Mesh = load("res://assets/models/tacz_ak47/ak47_test.obj")
		var mi := MeshInstance3D.new(); mi.mesh = mesh; root.add_child(mi)
		cam.position = Vector3(1.15, 0.06, 0.0)
		cam.look_at(Vector3(0.0, 0.0, 0.0), Vector3.UP)
	if _f == 8:
		var img := root.get_viewport().get_texture().get_image()
		img.save_png("user://tacz_obj_side.png")
		print("SAVED ", ProjectSettings.globalize_path("user://tacz_obj_side.png"))
		quit(0)
		return false
	return false
