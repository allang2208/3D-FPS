...
func _run() -> void:
	...
	_advance(zombie.attack_active_end + 0.05)
	...
	zombie.position = Vector3.ZERO
	zombie._start_attack(Vector3(0, 0, 1))
	zombie._tick_attack(zombie.attack_active_end + .1)
	...
	_advance(zombie.death_duration + .1)
	...
	_advance(1)
	check(zombie.is_queued_for_deletion(), "corpse_removed_after_animation_and_hold")
	world.queue_free()
	await process_frame
