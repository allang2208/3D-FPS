...
helpers(O/'author_arm.py',['repair'])
...
for family in ['base','vertical','canted','prism','angled']:
 ...
 for clip in ['reload','reload_empty']:
  ...
  report[key]={**info,'source':str(D/(name+'.fbx')),'blend':str(O/f'SVD_{family}_Editable.blend'),'previous_blend':str(source),
   'changed':'Rebuild grouped finger hinges and the accepted front-finger/rear-thumb semantic opposition, fit the whole palm, preserve joint translations, scale, mechanical and right-hand tracks',
