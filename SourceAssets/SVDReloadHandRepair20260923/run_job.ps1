param([string]$Script='import_animations.py')
$ErrorActionPreference='Stop'
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/SVDReloadHandRepair20260923'
...
try {
 ...
 $editors=@($running | Where-Object {$_.Name -eq 'UnrealEditor.exe' -and $_.CommandLine -notmatch '(?i)(^|\s)-game(\s|$)'})
 if($editors.Count){$bridge=$true}
 else {
  if($Script -eq 'end_play.py'){Write-Output 'SVD_HAND_NO_EDITOR_RUNNING';return}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(SVD_HAND_|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
