...
bpy.ops.wm.open_mainfile(filepath=str(O/'SVD_base_Editable.blend'))
...
(O/'source_after.json').write_text(json.dumps(out))
...
(O/'arm_surface_after.json').write_text(json.dumps({'parts':parts,'joints':{n:list(inv@r.matrix_world@m.translation) for n,m in p.items() if n.endswith('_l')}}))
...
for view,offset in [('outside',(.4,-.5,.08)),('inside',(.4,.5,.08))]:
 ...
 scene.render.filepath=str(O/('after_'+view+'.jpg'));bpy.ops.render.render(write_still=True)
