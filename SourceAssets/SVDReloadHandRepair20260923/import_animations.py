...
O=Path('D:/FPS3D/FPSGAME/SourceAssets/SVDReloadHandRepair20260923');PROJECT=O.parents[1]
...
if editor and editor.get_game_world():raise RuntimeError('Active play session: preserve state; no animation import')
...
try:
 for key,info in jobs:
  ...
  (O/'import_receipt.json').write_text(json.dumps(receipt,indent=2));print('SVD_HAND_ANIMATION_SAVED',key,flush=True)
finally:u.SystemLibrary.execute_console_command(None,flag+' '+str(prior))
print('SVD_HAND_ANIMATION_IMPORT_COMPLETE',len(receipt),flush=True)
