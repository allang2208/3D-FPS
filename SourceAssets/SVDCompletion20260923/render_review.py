...
for name,pos,target in [('side',(-1.,.08,.18),(0,-.20,.015)),('quarter',(-.6,.8,.42),(0,-.22,.02))]:
 cam.location=rt@Vector(pos);aim=rt@Vector(target);cam.rotation_euler=(aim-cam.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(O/(file.stem+('_'+args[1]+'_'+args[2] if len(args)>2 else '')+'_'+name+'.png'));bpy.ops.render.render(write_still=True)
