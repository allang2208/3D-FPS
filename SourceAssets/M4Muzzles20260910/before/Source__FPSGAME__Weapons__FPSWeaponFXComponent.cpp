...
FVector UFPSWeaponFXComponent::MuzzleLocation() const { return WeaponMesh->GetSocketLocation(MuzzleSocket); }
...
void UFPSWeaponFXComponent::OnShot(bool bADS)
{
    ...
    FlashLight->SetIntensity(950.0f * Scale);
...
void UFPSWeaponFXComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    {
        FlashLight->SetWorldLocation(MuzzleLocation() + MuzzleForward() * 2.0f);
        FlashLight->SetIntensity(950.0f * FMath::Clamp(FlashScale, 0.0f, 2.0f) * LastADSMultiplier * FMath::Square(FlashTime / 0.045f));
    }
