...
receipt=dict(stage='placing',map=TARGET,actors=[],hidden_actors=[],tests_run=False,screenshots_taken=False,
    lighting_settings_changed=False,runtime_generator_changed=False,reference='Four user detail screenshots')
def write():(ROOT/'Receipts/scene-install.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
write()
for e in manifest['objects']:
    a=actors.get(e['actor'])
    if not a:
        a=AA.spawn_actor_from_class(u.StaticMeshActor,u.Vector());a.set_actor_label(e['actor']);a.set_editor_property('tags',[u.Name('DungeonWorkshopSurface20260921')])
    ...
    a.set_folder_path('DungeonAtmosphereV2/RoomInteriors/Workshop/SurfaceDetails')
...
write();print('WORKSHOP_SURFACE_SCENE_SAVED '+json.dumps(dict(actors=len(receipt['actors']),hidden=receipt['hidden_actors'],saved_at=receipt['saved_at'],tests_run=False)))
