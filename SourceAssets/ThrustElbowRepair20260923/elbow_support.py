...
# The original thrust winds both elbows by almost half a revolution. Idle V4
# limits (45/60 degrees) left a visible neck in the fully extended arms.
UPPER_CAP=math.radians(70.)
ELBOW_CAP=math.radians(100.)
...
def support_elbow(source,rest,stations,side,state,weight):
 ...
 rotate_only(U,upper_turn)
 # This mesh's open shoulder ring is 70.8% upperarm_twist_01 and 27.1%
 # clavicle. Lock that helper, then ramp the roll through the distal helper.
 # Turning all upper-arm helpers together re-exposes the open sleeve at a
 # wide first-person FOV even though every joint origin is unchanged.
 upper_rest=rest[F].translation-rest[U].translation
 lock='upperarm_twist_01_'+side;distal='upperarm_twist_02_'+side
 lock_station=(rest[lock].translation-rest[U].translation).dot(upper_rest)/upper_rest.length_squared
 distal_station=(rest[distal].translation-rest[U].translation).dot(upper_rest)/upper_rest.length_squared
 share=max(0.,min(1.,(distal_station-lock_station)/(1.-lock_station)))
 rotate_only(distal,Quaternion(upper_axis,upper_roll*share))
