...
DEST='/Game/Dungeons/AtmosphereV2/RoomInteriors/WorkshopTools'
...
for key,recipe in recipes.items():
    name='M_WSTools_'+key;path=DEST+'/Materials/'+name;m=u.load_asset(path)
    if not m:
        ...
        for ch,filename in recipe.get('maps',{}).items():
            t=texture(filename,'T_WSTools_'+key+'_'+ch,ch)
...
for entry in manifest['objects']:
    ...
    for i,slot in enumerate(mesh.get_editor_property('static_materials')):
        ...
        path=manifest['material_aliases'].get(slot_name) or receipt['materials'].get(slot_name.removeprefix('WSTools_'))
...
print('WORKSHOP_TOOLS_ASSETS_SAVED',len(receipt['meshes']),len(receipt['materials']))
