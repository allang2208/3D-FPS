...
fit['basis']=json.loads((O.parent/'common_hand_pose.json').read_text())
fit['basis'].update(json.loads((O/'contact_overrides.json').read_text()))
specs=[
...
for clip,folder,action,end,hz in specs:
 ...
 for k,old in enumerate(samples):
  ...
  if w>0:
   ...
   support_offset=Vector(profile['shoulder_offset'])*w
   ...
   support_direction=(old['WPN_root']@Matrix(fit['grip_in_root'])).to_3x3()@Vector(profile['forearm_direction']);desired=desired.lerp(support_direction,w).normalized()
