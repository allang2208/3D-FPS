...
O=Path('D:/FPS3D/FPSGAME/SourceAssets/M4ReloadPolish20260910');DEST='/Game/Weapons/M4ReloadPolish'
mesh=unreal.load_asset('/Game/Weapons/M4HK416Replica/SK_M4_FoldingSights_HK416');rig=unreal.load_asset(DEST+'/CR_M4_Polish') or unreal.AssetToolsHelpers.get_asset_tools().duplicate_asset('CR_M4_Polish',DEST,unreal.load_asset('/Game/Weapons/M4HandMATRepair/CR_M4_Hand_MAT'));assert rig;rig.set_preview_mesh(mesh);unreal.EditorAssetLibrary.save_loaded_asset(rig,False)
...
for clip,end in [('reload',504),('reload_empty',648),('equip_charge',152)]:
 seq=unreal.AssetToolsHelpers.get_asset_tools().create_asset('LS_M4_Polish_'+clip,DEST,unreal.LevelSequence,unreal.LevelSequenceFactoryNew());assert seq
 seq.set_display_rate(unreal.FrameRate(240,1));seq.set_playback_start(0);seq.set_playback_end(end+1)
 ...
 anim=unreal.load_asset('/Game/Weapons/M4ReloadPolish/A_M4_HK416_'+clip);assert anim.get_editor_property("number_of_sampled_keys")==end+1
