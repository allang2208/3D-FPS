...
void AFPSGAMECharacter::InitializeWeaponVisuals()
{
    ...
    DrumReloadAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4HK416Replica/A_M4_HK416_drum_reload.A_M4_HK416_drum_reload"));
    DrumReloadEmptyAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4HK416Replica/A_M4_HK416_drum_reload_empty.A_M4_HK416_drum_reload_empty"));
    if (FParse::Param(FCommandLine::Get(), TEXT("DrumGripCandidate")))
    {
        DrumReloadAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumGripCandidate/A_M4_DrumGrip_reload.A_M4_DrumGrip_reload"));
        DrumReloadEmptyAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumGripCandidate/A_M4_DrumGrip_reload_empty.A_M4_DrumGrip_reload_empty"));
    }
    ...
    BoltReleaseSound = bUsingM4Infima ? LoadObject<USoundBase>(nullptr, TEXT("/Game/Weapons/M4HK416Audio/S_HK416_BoltRelease.S_HK416_BoltRelease")) : nullptr;
...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    M4ActionFramingAlpha = FMath::Lerp(M4ActionFramingAlpha, bUseActionFraming ? 1.0f : 0.0f,
    ...
    const FVector HipFraming = bUsingM4Infima
        ? FMath::Lerp(HipViewmodelLocation, M4ActionViewmodelLocation + ((bDrumInstalled && FParse::Param(FCommandLine::Get(), TEXT("DrumGripCandidate"))) ? FVector(12.0f, 0.0f, 12.0f) : FVector::ZeroVector), M4ActionFramingAlpha)
        : HipViewmodelLocation;
...
void AFPSGAMECharacter::FireShot()
{
    ...
    const FVector Muzzle = (AKMViewmodel->DoesSocketExist(TEXT("WPN_SOCKET_Muzzle")) ? AKMViewmodel->GetSocketLocation(TEXT("WPN_SOCKET_Muzzle")) : TraceStart);
    ...
    if (!bHit) bHit = GetWorld()->LineTraceSingleByChannel(Hit, Muzzle, AimTarget + TraceDirection * 2.0f, ECC_Visibility, Params);
...
void AFPSGAMECharacter::ApplyShotFeedback()
{
    ...
    GunFlipVelocity += 1.5f * RecoilLoad;
    ...
    const float ImpulseScale = FMath::Lerp(9.0f, 13.0f, WeaponADSFactor) * VisualRecoilScale;
...
void AFPSGAMECharacter::StartEquipCharge()
{
    ...
    if (EquipAnimation)
    {
        WeaponStateDuration = bUsingM4Infima ? 0.62f : EquipAnimation->GetPlayLength();
...
UAnimSequence* AFPSGAMECharacter::LoadAKMAnimation(const TCHAR* AssetName)
{
    if (bUsingM4Infima)
    {
        ...
        if (Clip) return LoadObject<UAnimSequence>(nullptr, *FString::Printf(TEXT("/Game/Weapons/M4HK416Replica/A_M4_HK416_%s.A_M4_HK416_%s"), Clip, Clip));
    }
    FString Folder = bUsingM4Infima ? TEXT("M4InfimaRigV4") : (bUsingReplacement ? TEXT("AKMReplacement") : TEXT("AKM"));
...
USoundBase* AFPSGAMECharacter::LoadAKMSound(const TCHAR* AssetName)
{
    if (bUsingM4Infima)
    {
        ...
        else if (FCString::Strcmp(AssetName, TEXT("S_AKM_Equip")) == 0) HKName = TEXT("S_HK416_Equip");
...
void AFPSGAMECharacter::EmitMechanicalCue(int32 CueIndex)
{
    ...
    if (Sound && Lateness < Sound->GetDuration()) PlayMechanicalSound(Sound, AKMSource::ActionVolume);
    if (bRunGunplayAcceptance && bUsingM4Infima && !FParse::Param(FCommandLine::Get(), TEXT("EquipFramingAudit")))
    {
        ...
        UE_LOG(LogTemp, Display, TEXT("M4_AUDIO_CUE empty=%d index=%d source=%.6f target=%.6f runtime=%.6f late=%.6f sound=%s"),
            bPendingEmptyReload, CueIndex, ReloadSourceTime(WeaponStateElapsed), MechanicalCueTimes[CueIndex], WeaponStateElapsed, Lateness, *GetNameSafe(Sound));
    }
...
void AFPSGAMECharacter::UpdateActionPose(float DeltaSeconds)
{
    ...
    if (ActiveActionAnimation)
    {
        ...
        GunplayAnimation->ActionClip = ActiveActionAnimation;
