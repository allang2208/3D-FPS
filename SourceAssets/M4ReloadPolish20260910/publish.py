...
O=Path(__file__).resolve().parent;project=O.parents[1];destination='/Game/Weapons/M4ReloadPolish';report={}
...
for clip in ['reload','reload_empty','equip_charge']:
 name='A_M4_HK416_'+clip;file=project/'Content/Weapons/M4ReloadPolish'/(name+'.uasset');saved=backup/file.name
 if not saved.exists():shutil.copy2(project/'Content/Weapons/M4HK416Replica'/file.name,saved)
...
sound_file=project/'Content/Weapons/M4HK416Audio/S_HK416_Equip.uasset'
if not(backup/sound_file.name).exists():shutil.copy2(sound_file,backup/sound_file.name)
t=unreal.AssetImportTask();t.filename=str(O/'Audio/equip.wav');t.destination_path='/Game/Weapons/M4ReloadPolish';t.destination_name='S_HK416_Equip';t.automated=True;t.replace_existing=True;t.save=True;unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([t])
sound=unreal.load_asset('/Game/Weapons/M4ReloadPolish/S_HK416_Equip');assert sound;sound.set_editor_property('loading_behavior',unreal.SoundWaveLoadingBehavior.FORCE_INLINE);assert unreal.EditorAssetLibrary.save_loaded_asset(sound,False);report['equip_audio']={'duration':sound.duration,'time_stretch_factor':.72/.62,'pitch_preserved':True}
