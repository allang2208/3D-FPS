...
$caseDir='D:/FPS3D/FPSGAME/SourceAssets/SVDAttachments20260923'
...
try {
    ...
    $result=$LASTEXITCODE
    Select-String -LiteralPath "$caseDir/$job.log" -Pattern 'LogPython:.*(SVD_ATTACH|Error|Traceback)' | Select-Object -Last 12 | ForEach-Object {$_.Line}
    exit $result
