...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    if (bDrumInstalled && IsReloading())
    {
        ...
        // Keep the current hip framing through release. Make room only when
        // the support hand leaves the fore-end to fetch the new drum.
        ActionFramingTarget = FMath::SmoothStep(bPendingEmptyReload ? 18.0f : 22.0f,
            bPendingEmptyReload ? 35.0f : 50.0f, Frame);
    }
