"""Import/save the outgoing-belt mesh in a Python commandlet or existing editor."""
...
# Commandlets have no LevelEditorSubsystem world. The interactive bridge caller
# is responsible for stopping PIE before invoking this same asset import.
...
if not mesh or not t.imported_object_paths:raise RuntimeError('Outgoing belt mesh import failed')
mapping=bind_materials(mesh,{},bindings)
E.set_metadata_tag(mesh,'PKMOutgoingBelt','OutgoingBelt19; source part 113 follows ammunition; 25 empty leaf links')
...
(O/'imported.json').write_text(json.dumps({'mesh':mesh.get_path_name(),'skeleton':mesh.skeleton.get_path_name(),
 'materials':mapping,'saved':True,'animations_reimported':False,'pipeline':'Python asset import'},indent=2))
print('PKM19_OUTGOING_BELT_MESH_AND_SKELETON_SAVED')
