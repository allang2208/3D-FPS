...
donor_dir=ROOT.parent/'MannyGraspDonor20260912'
grasp=json.loads((donor_dir/'Opening/0.8/aligned_fit.json').read_text())
grasp_basis=json.loads((donor_dir/'donor_fit.json').read_text())['basis']
grasp_hand=Matrix(grasp['grip_in_root']).inverted()@Matrix(grasp['hand_in_root'])
...
def frame_pose(t,kind):
 ...
 W=levelW@trx((0,.135,-.02))@trx((0,.015*tilt,.012*tilt))@rot('Y',-13*tilt)@rot('X',-9*tilt)
 ...
 # The accepted whole-hand grasp follows the narrow front support's axis.
 # Rotate the grasp center as a unit; never bend fingertips across the receiver.
 LH=W@fit@trx((0,-.067,.008))@rot('X',90)@rot('Z',180)@trx((0,0,.056))@grasp_hand
 ...
 solve_arm('l',left,lopen,desired,bodyMove+Vector((0,.065,0)));solve_arm('r',right,ropen,desired,bodyMove)
 support=1 if idle else seq(t,[(0,1),(.35,1),(.65,0),(2.12,0),(2.3,1),(length,1)])
 if support>0:
  finger_locals={n:desired[parents[n]].inverted()@desired[n] for n in base if n in grasp_basis and n.endswith('_l')}
  for name in base:
   if name not in grasp_basis or not name.endswith('_l'):continue
   parent=parents[name];local_rest=rest[parent].inverted()@rest[name]
   old_local=finger_locals[name]
   location,old_q,scale=(local_rest.inverted()@old_local).decompose()
   q=Quaternion().slerp(Matrix(grasp_basis[name]).to_quaternion(),.8)
   desired[name]=desired[parent]@local_rest@Matrix.LocRotScale(location,old_q.slerp(q,support),scale)
