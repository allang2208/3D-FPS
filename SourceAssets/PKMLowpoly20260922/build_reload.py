...
ROOT=pathlib.Path(__file__).parent;OUT=ROOT/'Animation03';OUT.mkdir(exist_ok=True)
...
def solve_arm(side,target,open_amount,desired,bodyMove):
 ...
 shift=Vector()
 if distance>(l1+l2)*.975:
  shift=delta.normalized()*(distance-(l1+l2)*.975);shoulder+=shift;elbow+=shift;wrist+=shift
 ...
 pole=(base[names[1]].translation-base[names[0]].translation)
...
def frame_pose(t,kind):
 ...
 W=baseW@trx((0,.06,-.02))@trx((0,.015*tilt,.012*tilt))@rot('Y',-13*tilt)@rot('X',-9*tilt)
 ...
 # The support palm is deliberately placed ahead of the box, with shoulder reach adjustment.
 LH=contact('l',(.024,-.047,.006),(-1,0,-.05),(0,0,1),W)
 RH=W@baseW.inverted()@base['hand_r']
 if idle:
  left,right=LH,RH;lopen=.12;ropen=0
 else:
  LC=contact('l',(0,.285,.099),(0,-1,0),(0,0,-1),W)
  LCmove=W@fit@coverM@fit.inverted()@W.inverted()@LC
  Lclear=contact('l',Vector((0,.038,.077)) if empty else oldPts[0]+Vector((0,0,.009)),(-1,0,0),(0,0,-1),W)
  ...
  lopen=seq(t,[(0,.12),(.5,.8),(.65,.5),(1.12,.5),(1.32,.7),(1.55,.25),(1.9,.25),(2.1,.85),(2.3,.12),(length,.12)])
  Rbox=contact('r',oldBox@Vector((-.112,.04,-.072)),(0,-.3,-1),(1,0,0),W)
  RnewBox=contact('r',newBox@Vector((-.112,.04,-.072)),(0,-.3,-1),(1,0,0),W)
  Rout=contact('r',(-.20,.16,-.07),(0,-.4,-1),(1,0,0),W)
  Runder=contact('r',(-.28,.22,-.27),(0,-.4,-1),(1,0,0),W)
  Rbelt=contact('r',newPts[0]+Vector((0,0,.009)),(1,0,0),(0,0,-1),W)
  Rcover=contact('r',(0,.275,.10),(0,-1,0),(0,0,-1),W)
  RcoverMove=W@fit@coverM@fit.inverted()@W.inverted()@Rcover
  Rcharge=contact('r',(-.055,.115+charge,.023),(0,-.25,-1),(1,0,0),W)
 ...
 solve_arm('l',left,lopen,desired,bodyMove);solve_arm('r',right,ropen,desired,bodyMove)
...
report={'hand_source':str(SRC),'hand_mesh':hands.name,'donor_pose':'M4_idle frame 0','reference':'https://www.bilibili.com/video/BV1jHeA6sEmj/','fit_matrix':[list(v) for v in fit],'animations':actions,'old_new_box_and_belt':True,'arm_bone_lengths_scaled':False,'visual_tested':False,'game_integrated':False,'limitations':['Candidate author adaptation, not an exact capture of reference motion','No visual contact or intermediate-frame acceptance run','No audio or gameplay replenishment events integrated','Old/new duplicate box and belt visibility requires handling when integrated']}
