...
ROOT=Path(__file__).resolve().parents[1];ROOMS=ROOT.parent/'DungeonRoomShells20260922';BASE='/Game/Dungeons/CombatExpansionV3_20260922'
...
snapshot_path=ROOT/'Receipts/scene-before.json'
snapshot=json.loads(snapshot_path.read_text()) if snapshot_path.exists() else {'actors':[]}
if not any(row['label']=='DGN_Prop_GoddessStatue_01' for row in snapshot['actors']):
    snapshot={'world':target,'actors':[]}
    for source_actor in AA.get_all_level_actors():
        if not (source_actor.get_actor_label().startswith('DGN_RS_') or source_actor.get_actor_label()=='DGN_Prop_GoddessStatue_01'):continue
        p=source_actor.get_actor_location();r=source_actor.get_actor_rotation()
        snapshot['actors'].append(dict(label=source_actor.get_actor_label(),location=[p.x,p.y,p.z],rotation=dict(pitch=r.pitch,yaw=r.yaw,roll=r.roll)))
    snapshot_path.write_text(json.dumps(snapshot,indent=2))
actors={}
# Remove only the known previous anchor labels made stale by the two new flank anchors.
stale_labels={'DGN_RS_Anchor_encounter_7','DGN_RS_Anchor_side_discovery_8','DGN_RS_Anchor_future_exit_9'}
for old_anchor in list(AA.get_all_level_actors()):
    if old_anchor.get_actor_label() in stale_labels:AA.destroy_actor(old_anchor)
...
for item in FLUID['objects']:
    ...
    if is_surface:
        half=item['half_size_cm'];a.set_editor_property('half_size',u.Vector2D(half[0]*(.95 if 'Puddle' in name else 1),half[1]*(.95 if 'Puddle' in name else 1)));a.set_editor_property('damage_per_pulse',8);a.set_editor_property('damage_interval',.5)
        a.set_editor_property('tags',list(a.tags)+[u.Name('CorrosivePus'),u.Name('DungeonPermanentHazard')])
    if item.get('radial_footprint'):a.set_editor_property('tags',list(set(list(a.tags)+[u.Name('PusRadialFootprint')])))
...
before=json.loads((ROOT/'Receipts/scene-before.json').read_text());old=json.loads((ROOT/'Config/rooms-before.json').read_text(encoding='utf-8'))['rooms'][2];new=CFG['rooms'][2]
for data in before['actors']:
    ...
    a=own(matches[0]);p=data['location'];local=[p[0]/100-old['origin_m'][0],-p[1]/100-old['origin_m'][1],p[2]/100-old['origin_m'][2]];local[0]+=new['breach']['pocket'][0]-old['breach']['pocket'][0];local[1]+=new['breach']['pocket'][1]-old['breach']['pocket'][1]
