...
for item in items:
    if item.get('reuse_asset'):
        receipt['meshes'][item['name']]=item['reuse_asset'];continue
    ...
    previous_mesh=u.load_asset(path) if E.does_asset_exist(path) else None
    ...
    ns=previous_nanite if previous_nanite is not None else mesh.get_editor_property('nanite_settings')
    if previous_nanite is None or 'Pus' in item['name']:ns.enabled=False
    u.get_editor_subsystem(u.StaticMeshEditorSubsystem).set_nanite_settings(mesh,ns,True)
