"""Import a scoped family and save each changed animation at its current runtime path."""
import unreal as u,json,shutil,hashlib
from pathlib import Path
O=Path('D:/FPS3D/FPSGAME/SourceAssets/SVDHandRepair20260923');PROJECT=O.parents[1]
family=globals().get('BATCH_FAMILY','base');auth=json.loads((O/'authoring.json').read_text());receipt=json.loads((O/'import_receipt.json').read_text()) if (O/'import_receipt.json').exists() else {}
jobs=[(k,a) for k,a in auth.items() if a['family']==family and k not in receipt]
if u.get_editor_subsystem(u.UnrealEditorSubsystem).get_game_world():raise RuntimeError('Stop the active play session before importing animation assets')
dirty={p.get_path_name() for p in u.EditorLoadingAndSavingUtils.get_dirty_content_packages()}
conflicts=[a['path'] for _,a in jobs if a['path'] in dirty]
if conflicts:raise RuntimeError('Unsaved target animation changes: '+str(conflicts))
...
try:
 for key,info in jobs:
  ...
  if not old:raise RuntimeError('Current runtime animation not found '+path)
  source=Path(info['source'])
  if not source.exists():raise RuntimeError('FBX not produced '+str(source))
  ...
  receipt[key]={'asset':anim.get_path_name(),'source':str(source),'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'saved':True,'before':str(backup),'before_sha256':hashlib.sha256(backup.read_bytes()).hexdigest(),'game_tested':False}
  (O/'import_receipt.json').write_text(json.dumps(receipt,indent=2));print('SVD_HAND_SAVED',key,flush=True)
finally:u.SystemLibrary.execute_console_command(None,flag+' '+str(prior))
print('SVD_HAND_IMPORT_BATCH',family,len(jobs),'TOTAL',len(receipt),flush=True)
