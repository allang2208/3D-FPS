...
for prop in CFG['props']:
    ...
    obj.name='SM_V2_'+prop['id'];obj.data.calc_loop_triangles();original=len(obj.data.loop_triangles)
    high=obj.copy();high.data=obj.data.copy();bpy.context.collection.objects.link(high)
    high.name=prop['id']+'_SourceHigh'
    budget=100000 if prop['id']!='collapsed_tile_rubble' else 85000
    if original>budget:
    ...
    for slot in obj.material_slots:
        mat=slot.material
        if not mat:continue
        mat.name='V2_'+prop['id']+'_'+re.sub(r'[^A-Za-z0-9_]','_',mat.name)
