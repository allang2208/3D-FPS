...
$statePath = Join-Path $dungeonRoot 'Receipts/tile-polish-capture-state.json'
$receiptPath = Join-Path $dungeonRoot ('Receipts/bridge-tile-capture-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff') + '.txt')
...
try {
    ...
    & 'D:/FPS3D/FPSGAME/Tools/AssetPipeline/mcp_call_codex.ps1' -PythonScript (Join-Path $PSScriptRoot 'capture_tile_polish.py') -OutputFile $receiptPath -MaxOutputChars 3000
