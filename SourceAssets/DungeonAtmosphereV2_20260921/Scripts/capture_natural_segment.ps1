...
$statePath = Join-Path $dungeonRoot 'Receipts/natural-segment-capture-state.json'
$receiptPath = Join-Path $dungeonRoot ('Receipts/bridge-natural-capture-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff') + '.txt')
...
try {
    ...
    & 'D:/FPS3D/FPSGAME/Tools/AssetPipeline/mcp_call_codex.ps1' -PythonScript (Join-Path $PSScriptRoot 'capture_natural_segment.py') -OutputFile $receiptPath -MaxOutputChars 3000
