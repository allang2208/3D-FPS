...
$statePath = Join-Path $dungeonRoot 'Receipts/capture-state.json'
$receiptPath = Join-Path $dungeonRoot ('Receipts/bridge-capture-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff') + '.txt')
...
try {
    ...
    & 'D:/FPS3D/FPSGAME/Tools/AssetPipeline/mcp_call_codex.ps1' -PythonScript (Join-Path $PSScriptRoot 'capture_v2.py') -OutputFile $receiptPath -MaxOutputChars 3000
