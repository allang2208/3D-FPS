...
bpy.ops.import_scene.gltf(filepath=str(P.parent/'seed_91813/textured_master_00001_.glb'))
high=next(o for o in bpy.context.scene.objects if o.type=='MESH');high.data.transform(high.matrix_world);high.matrix_world=Matrix.Identity(4);high.data.transform(Matrix.Rotation(math.pi/2 if high.dimensions.x>high.dimensions.y else 0,4,'Z'));high.name='Resonance_Selected91813_Master'
...
low.data.calc_loop_triangles();d=low.modifiers.new('GameMesh','DECIMATE');d.ratio=min(1,40000/len(low.data.loop_triangles));d.use_collapse_triangulate=True;bpy.ops.object.modifier_apply(modifier=d.name)
