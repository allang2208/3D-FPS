...
opt.static_mesh_import_data.combine_meshes = False
...
mat_poly = u.load_asset('/Game/Weapons/ExtMagUniversal20260917/M_ExtMag_Polymer')
mat_metal = u.load_asset('/Game/Weapons/ExtMagUniversal20260917/M_ExtMag_Metal')
slots = mesh.get_editor_property('static_materials')
names = [str(s.material_slot_name) for s in slots]
for i, s in enumerate(slots):
    key = str(s.material_slot_name)
    if key == 'M_ExtMag_Metal' and mat_metal:
        s.material_interface = mat_metal
    elif mat_poly:
        s.material_interface = mat_poly
    slots[i] = s
mesh.set_editor_property('static_materials', slots)
ok = u.EditorAssetLibrary.save_loaded_asset(mesh, False)
b = mesh.get_bounds()
u.log('EXTMAG_REIMPORT slots=%s saved=%s extent=(%s)' % (names, ok,
      ','.join(str(round(v,2)) for v in (b.box_extent.x, b.box_extent.y, b.box_extent.z))))
