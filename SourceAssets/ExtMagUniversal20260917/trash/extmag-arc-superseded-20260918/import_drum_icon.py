...
if not isinstance(asset, u.Texture2D):
    raise RuntimeError('drum icon texture import failed')
u.EditorAssetLibrary.save_loaded_asset(asset, False)
(P / 'import_drum_icon_receipt.json').write_text(json.dumps({'icon': asset.get_path_name()}, indent=2))
u.log('DRUM_ICON_IMPORTED')
