...
import sys
P=Path(__file__).parent / ('seed_'+sys.argv[sys.argv.index('--')+1])
...
next(n for n in s.world.node_tree.nodes if n.type=='BACKGROUND').inputs[0].default_value=(.18,.21,.25,1)
next(n for n in s.world.node_tree.nodes if n.type=='BACKGROUND').inputs[1].default_value=.5
...
bpy.ops.wm.save_as_mainfile(filepath=str(P/'BalancedRearGrip_Preview.blend'))
