"""Author V5's connected drives and brake on the accepted two-hand rig."""
import json
import math
import sys
...
action = bpy.data.actions.new('A_RuneSword_WhirlwindV5')
...
select(r)
bpy.ops.export_scene.fbx(filepath=str(OUT / 'A_RuneSword_WhirlwindV5.fbx'), use_selection=True,
...
(P/'authoring.json').write_text(json.dumps({'clip':'A_RuneSword_WhirlwindV5','fps':FPS,'seconds':ATTACK_END,'ready':READY_END,'spin_end':SPIN_END,'loop':False,'source':str(source),'donor':str(donor),'camera_turn_degrees':T['turnDegrees'],'camera_turn_in_runtime':True,'knots_seconds':KNOTS,'windup_curve':'C2 quintic Hermite with shared velocity; continuous pull/load/drive through the release boundary','runtime_entry_seconds':.1},indent=2),encoding='utf-8')
