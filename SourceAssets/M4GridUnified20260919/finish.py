...
for gun in ['M4']:
 ...
 # Preserve transformed factory corner normals. Recalculating the full lower
 # shell destroyed its weighted face/bevel shading and exposed triangulation.
 if gun=='M4':
 ...
 ob.name='SM_M4_ExtMag40_Grid';bpy.ops.object.select_all(action='DESELECT');ob.select_set(True);bpy.context.view_layer.objects.active=ob
 (O/'FBX').mkdir(exist_ok=True);export(mesh,O/'FBX'/(ob.name+'.fbx'));bpy.context.preferences.filepaths.save_version=0;bpy.ops.wm.save_as_mainfile(filepath=str(O/('M4_ExtMag40_Grid_Editable.blend')))
