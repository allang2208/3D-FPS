...
for gun,src,frames in [('M4','ExtMagContact20260919/A_M4_ExtContact_reload.blend',[76]),('AKM','ExtMagContact20260919/A_AKM_ExtContact_reload.blend',[148])]:
 ...
 path=O/'AKM_ExtMag_Closed_Editable.blend' if gun=='AKM' else SA/'ExtMagPattern20260919/M4_ExtMag_Editable.blend'
 with bpy.data.libraries.load(str(path),link=False) as (a,b):b.objects=['SM_ExtMag_AKM40_Closed' if gun=='AKM' else 'SM_ExtMag_M440']
 ...
 for f in frames:
  ...
  for view,offset in [('palm',(-.38,-.3,.09)),('side',(.4,-.12,.08))]:
   cam.location=center+Vector(offset);cam.rotation_euler=(center-cam.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(O/('after_'+gun+'_'+str(f)+'_'+view+'.png'));bpy.ops.render.render(write_still=True)
(O/'after_contact.json').write_text(json.dumps(reports,indent=2))
