...
best=None
for z in [-.04,-.06,-.08,-.10,-.12,-.16]:
 for x,y in [(fit['release_vector'][0],fit['release_vector'][1]),(0,.12),(-.04,.12),(.04,.16),(-.06,.16)]:
  ...
  if hits==0:break
 if best[0]==0:break
assert best[0]==0,best;fit['release_vector']=best[1].tolist();(pdir/'fit_final.json').write_text(json.dumps(fit,indent=2));(pdir/'withdrawal_fit.json').write_text(json.dumps({'crossings':best[0],'release_vector':best[1].tolist(),'samples':len(samples),'fingers':'common unchanged'},indent=2));print('WITHDRAWAL_PASS')
