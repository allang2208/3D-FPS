...
for match in re.finditer(stamp + r'.*M4_AUDIO_CUE empty=(\d) index=(\d).*late=([\d.]+) sound=S_HK416_(\w+)', log):
    ...
    name = names[match[5]]
    template = np.frombuffer(subprocess.check_output([str(FFMPEG), '-v', 'error', '-i', str(ROOT.parent/'M4HK416Replica20260910/Audio'/f'{name}.wav'), '-ac', '1', '-ar', str(rate), '-f', 'f32le', '-']), dtype='<f4').astype(float)
