...
O=Path(__file__).resolve().parent;run=O.parents[1]/'Saved/GunplayUpgrade/m4-mat-repair-equip'
...
template=np.frombuffer(subprocess.check_output([ff,'-v','error','-i',str(O.parent/'M4HK416Replica20260910/Audio/equip.wav'),'-ac','1','-ar',str(rate),'-f','f32le','-']),dtype='<f4').astype(float)
...
report={'events':events,'peak':float(abs(pcm).max()),'duration':len(pcm)/rate,'note':'Actual mixer recording correlated with original HK416 equip.mp3 decoded to WAV. Interrupted equips are checked by runtime voice/queue assertions.'}
