...
try:
 for key,info in jobs:
  ...
  (O/'import_receipt.json').write_text(json.dumps(receipt,indent=2));print('SVD_MAG_ANIMATION_SAVED',key,flush=True)
finally:u.SystemLibrary.execute_console_command(None,flag+' '+str(prior))
print('SVD_MAG_ANIMATION_IMPORT_COMPLETE',len(receipt),flush=True)
