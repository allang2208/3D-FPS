param([string]$Script='import_model.py')
$ErrorActionPreference='Stop'
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/SVDMagazineFit20260923'
...
try {
 ...
 if($running.Count){$bridge=$true}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(SVD_MAG|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
