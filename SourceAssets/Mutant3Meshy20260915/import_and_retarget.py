"""Import only Mutant3 assets, author native IK rigs, and bake source motions."""
...
target=mesh(ROOT/'prepared/SK_Mutant3_Meshy.fbx','SK_Mutant3_Meshy',DEST)
source=mesh(ROOT/'prepared/SK_M2M_Source.fbx','SK_M2M_Source',DEST+'/Sources')
clips=[]
for role in ['Idle','Scratch','Rage','Stagger','Death']:
    o=u.FbxImportUI();o.automated_import_should_detect_type=False
...
mat=create('M_Mutant3_Meshy',DEST+'/Materials',u.Material,u.MaterialFactoryNew())
...
for index,(role,suffix,prop) in enumerate(maps):
    tex=imp(next(texbase.rglob('*'+suffix)),'T_Mutant3_'+role,DEST+'/Textures')
...
dst=ik('IK_Mutant3_Meshy',target,'Hips',dst_chains)
rtg=create('RTG_M2M_Mutant3',DEST+'/Rig',u.IKRetargeter,u.IKRetargetFactory())
...
params.search='A_M2M_';params.replace='A_Mutant3_Raw_';params.target_path=DEST+'/RetargetedRaw'
...
u.log('MUTANT3_NATIVE_RETARGET_COMPLETE '+json.dumps(report))


exec(compile((ROOT/'prepare_damage_collision.py').read_text(encoding='utf-8-sig'), str(ROOT/'prepare_damage_collision.py'), 'exec'))
