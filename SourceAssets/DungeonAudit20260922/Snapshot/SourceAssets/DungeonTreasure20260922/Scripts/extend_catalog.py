...
def extend(catalog):
    ...
    chest=json.loads((PROJECT/'Content/ColdSteelData/warehouse_assets.json').read_text(encoding='utf-8-sig'))
    m['props']=[dict(skeletal_mesh=chest['mesh'],closed_animation=chest['close'],material_overrides=chest['materials'],
                     position=[0,-512,.8],yaw=-90,scale=[1,1,1],role='treasure_chest')]
