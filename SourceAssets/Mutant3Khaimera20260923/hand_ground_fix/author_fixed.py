...
SOURCE = ROOT.parent
BASE = ROOT / 'Mutant3_Claw_Source.blend'
OUT = ROOT / 'final'
FPS = 60
META = json.loads((SOURCE / 'native_retarget.json').read_text())
...
for name in SOURCES:
    ...
    bpy.ops.import_scene.fbx(filepath=str(SOURCE / 'native_retarget' / ('A_Mutant3_KhaiRaw_' + name + '.fbx')))
...
for mesh in meshes:
    ...
    foot_vertices[mesh.name] = [v.index for v in mesh.data.vertices
        if sum(g.weight for g in v.groups if 'Foot' in group_names[g.group] or 'Toe' in group_names[g.group]) > .5]
ground_report = {}
for pb in ordered:
...
def sample(name, time):
    ...
    weight = index-lo
    return {n: data['frames'][lo][n].lerp(data['frames'][hi][n], weight) for n in rest if n in data['rest']}

def blend(a, b, w):
    return {n: a[n].lerp(b[n], w) for n in a}
...
def set_pose(data, src, role, phase):
    mats = {}
    for pb in ordered:
        name = pb.name
        bone = pb.bone
        r = rest[name]
        if name not in data:
            # Added finger bones inherit the animated hand and retain their
            # relaxed-claw neutral transforms, with no weapon-grip tracks.
            mats[name] = mats[bone.parent.name] @ (rest[bone.parent.name].inverted() @ r)
        ...
        if name == 'Hips':
            # In the Blender source Hips is the top bone (the UE Mutant3Root is
            # the exported armature object). It must translate even without a
            # bone parent; locking this branch was the V1 floating-crouch bug.
            # UE's animation-only FBX has a centimetre-sized bind skeleton but
            # its animated world translations are already converted to metres.
            # Use the target's real bind position, never subtract that tiny FBX
            # bind offset (which would add almost one extra metre of elevation).
            offset = data[name].translation - world_rest[name].translation
            if role == 'PounceFlight':
                offset.z = launch_pose[name].translation.z - world_rest[name].translation.z
    ...
    # Signed foot-sole correction both lowers floating feet and lifts penetration.
    # Using the lowest body point or a positive-only correction cannot plant feet.
    if True:
        ...
        for mesh in meshes:
            ...
            low = min(low, min((obj.matrix_world @ data_mesh.vertices[i].co).z for i in foot_vertices[mesh.name]))
            obj.to_mesh_clear()
        hips = rig.pose.bones['Hips']
        m = hips.matrix.copy()
        if role in ['FeralIdle','ClawA','ClawB','ClawC','PounceWindup','PounceLand']:
            correction = .003-low
        elif role == 'PounceFlight':
            # The air pose meets the grounded windup and landing at its endpoints.
            weight = max(1-smooth(phase/.16), smooth((phase-.65)/.35))
            correction = (.003-low)*weight
        else:
            correction = max(0,.003-low)
        m.translation += inv.to_3x3() @ Vector((0,0,correction))
        hips.matrix = m
        bpy.context.view_layer.update()
        ground_report.setdefault(role,[]).append({'phase':phase,'raw_sole_z_m':low,'correction_m':correction,'sole_z_m':low+correction})
...
for role, (src, duration, looping) in CLIPS.items():
    ...
    if old:
        old.name = 'BeforeClawGrounding_' + old.name
...
scene.frame_set(0)
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT / 'Mutant3_Khaimera_ClawGrounded.blend'))
(ROOT/'grounding_bake.json').write_text(json.dumps(ground_report,indent=2),encoding='utf-8')
