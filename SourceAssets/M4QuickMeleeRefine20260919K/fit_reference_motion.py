...
refine=plan['refinement']
frames=[]
for f in range(round(plan['duration_seconds']*120)+1):
    t=f/120;m=np.eye(4);m[:3,3]=positions(t);m[:3,:3]=rotations([t]).as_matrix()[0]
    weight=smooth(t/refine['entry_seconds'])*(1-smooth((t-refine['exit_start_seconds'])/(refine['exit_end_seconds']-refine['exit_start_seconds'])))
    m[1,3]-=refine['maximum_pullback_m']*weight
