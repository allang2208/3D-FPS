...
P=Path('D:/FPS3D/FPSGAME/SourceAssets/StableAntiSlipRearGrip20260913'); seed=91727; C=P/f'seed_{seed}'; C.mkdir(exist_ok=True)
base='http://192.168.3.142:8188'
if sys.argv[1]=='submit':
 w=json.loads((P/'seed_91713/workflow.json').read_text())
 w['4']['inputs']['seed']=seed
 for node in w.values():
  if 'filename_prefix' in node.get('inputs',{}): node['inputs']['filename_prefix']=node['inputs']['filename_prefix'].replace('seed_91713',f'seed_{seed}')
 (C/'workflow.json').write_text(json.dumps(w,indent=2))
 if not (C/'receipt.json').exists():
  req=urllib.request.Request(base+'/prompt',data=json.dumps({'client_id':'StableGripReroll','prompt':w}).encode(),headers={'Content-Type':'application/json'})
 ...
 if h['status']['status_str']=='success':
  for f in ['raw_00001_.glb','textured_master_00001_.glb']:
   if not (C/f).exists(): urllib.request.urlretrieve(base+'/view?'+urllib.parse.urlencode({'filename':f,'subfolder':f'StableGripMV/seed_{seed}','type':'output'}),C/f)
   print(C/f)
