...
if sys.argv[1]=='submit':
    if not (P/'upload.json').exists():
        ...
        body=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="stable_grip_three_views.png"\r\nContent-Type: image/png\r\n\r\n').encode()+(P/'three_views.png').read_bytes()+f'\r\n--{boundary}--\r\n'.encode()
    ...
    for seed in SEEDS:
        ...
        for i in range(3):
            ...
            w[crop]={'class_type':'ImageCrop','inputs':{'image':['10',0],'x':[0,600,930][i],'y':0,'width':[600,330,606][i],'height':1024}}
            w[pre]={'class_type':'Trellis2PreProcessImage','inputs':{'image':[crop,0],'padding':24,'remove_background':True,'max_size':2048}}
            w[str(130+i)]={'class_type':'SaveImage','inputs':{'images':[pre,0],'filename_prefix':f'StableGripMV/seed_{seed}/view{i}'}}
        pp=dict(params)
        ...
        w.pop('20',None)
        for key,name in [('51','raw'),('53','textured_master')]:w[key]['inputs']['filename_prefix']=f'StableGripMV/seed_{seed}/{name}'
        (p/'workflow.json').write_text(json.dumps(w,indent=2))
        r=req('/prompt',json.dumps({'client_id':'StableGripMV','prompt':w}).encode(),{'Content-Type':'application/json'})
    ...
    for seed in SEEDS:
        ...
        for filename in ['textured_master_00001_.glb','raw_00001_.glb']:
            if not (p/filename).exists():
                urllib.request.urlretrieve(BASE+'/view?'+urllib.parse.urlencode({'filename':filename,'subfolder':f'StableGripMV/seed_{seed}','type':'output'}),p/filename)
