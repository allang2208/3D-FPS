...
O=Path(__file__).parent; D='/Game/Weapons/RearGripFinish20260913'
...
base=texture(O/'T_Balanced_BaseColor.png',True);mr=texture(O/'T_Balanced_MetalRough.png',False)
phantom=load('/Game/Weapons/PhantomRearGrip/M_PhantomRearGrip')
metaltex={
...
for key,info in auth.items():
 ...
 report[key]={'mesh':path,'reference':refs[info['family']],'coating_uv':1,'polymer_uv':0,'slots':{str(s.material_slot_name):s.material_interface.get_path_name() for s in mesh.static_materials}}
 (O/'installed.json').write_text(json.dumps(report,indent=2));u.log('REAR_GRIP_FINISH_INSTALLED '+key)
u.log('REAR_GRIP_FINISH_COMPLETE')
