...
mesh=import_fbx('SK_Bow_BareArmsV7',u.FBXImportType.FBXIT_SKELETAL_MESH)
...
# Record the reference transforms needed to maintain the authoring bind contract.
...
def tr(t):
    p=u.MathLibrary.transform_location(t,u.Vector(0,0,0))
    return {'p':vec(p),'axes':[vec(u.MathLibrary.transform_location(t,u.Vector(*a))-p) for a in ((1,0,0),(0,1,0),(0,0,1))]}
(P/'imported_reference.json').write_text(json.dumps({str(b.name):tr(b.world_transform) for b in bones}),encoding='utf-8')
print('BOW_V4_BIND_AND_ACTIONS_SAVED',len(receipt['saved']))
