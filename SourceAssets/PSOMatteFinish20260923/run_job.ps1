param([string]$Script='read_inputs.py')
$ErrorActionPreference='Stop'
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/PSOMatteFinish20260923'
...
try {
 ...
 if($running.Count){$bridge=$true}
  ...
  if($Script -eq 'end_play.py'){Write-Output 'PSO_NO_EDITOR_RUNNING';return}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(PSO_|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
