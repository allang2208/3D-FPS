...
coremat = u.load_asset(BASE+'/Materials/M_CeramicFractureCore')
if not coremat:
    raise RuntimeError('Build ceramic fracture material first')
...
for entry in MAN['objects']:
    ...
    for index, slot in enumerate(mesh.get_editor_property('static_materials')):
        ...
        if name == 'V2_CeramicFractureCore':
            material = coremat
        else:
    ...
    write()
    print('CERAMIC_WALL_MESH_SAVED '+entry['name'])

receipt.update(stage='meshes_saved', saved_at=datetime.now().isoformat())
write()
print('CERAMIC_WALL_MESHES_SAVED '+json.dumps(receipt))
