"""Fit the current laser and Hunyuan flashlight to the M1911 dust cover."""
...
fit=json.loads((O/'fit_source.json').read_text())
try:
...
for kind in ['laser','flashlight']:
    ...
    # Remove only the rifle-specific saddle. The accepted device body keeps its size.
    ...
    for face in ob.data.polygons:
        for li in face.loop_indices:
            ...
            if kind=='flashlight' and p.y>low_y+.008:metal=1.
            aperture=float(kind=='laser' and (p-optical_center).length<.0065 and metal<.5)
    ...
    for iy in range(ny+1):
        for ix in range(nx+1):
            x=(ix/nx-.5)*.016;y=-.086+(iy/ny-.5)*.032
    ...
    report[kind]={'fbx':str(file),'source':source['source'],'body_scale':1.,'reference_frame':'M1911 WPN_root authoring metres; FBX centimetres, +Y forward',
                  'uv_index':1,'physical_tile_m':.1,'emitter_blender_m':list(emitter),'emitter_ue_cm':[emitter.x*100,-emitter.y*100,emitter.z*100],
                  'tail_y_m':-.064,'saddle_center_m':[0,-.086],'saddle_bridged_samples':bridged,'slots':[m.name for m in ob.data.materials]}
...
bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(O/'M1911_Tactical_Assembly_Editable.blend'))
