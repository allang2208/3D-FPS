...
O=Path(__file__).parent; D='/Game/Weapons/TacticalDevices20260913'
...
profiles=json.loads((O.parent/'WeaponAttachmentFinish20260913/profiles.json').read_text())
...
for name,alpha in [('M_LaserDot',1.),('M_LaserBeam',.65)]:
 path=D+'/Effects/'+name
 m=u.load_asset(path) or A.create_asset(name,D+'/Effects',u.Material,u.MaterialFactoryNew())
 m.set_editor_property('shading_model',u.MaterialShadingModel.MSM_UNLIT)
 if alpha<1:m.set_editor_property('blend_mode',u.BlendMode.BLEND_TRANSLUCENT)
 c=node(m,u.MaterialExpressionConstant3Vector,constant=u.LinearColor(3.,.045,.015,1.));output(c,'',u.MaterialProperty.MP_EMISSIVE_COLOR)
 if alpha<1:output(constant(m,alpha),'',u.MaterialProperty.MP_OPACITY)
 L.recompile_material(m);save(m)
u.log('TACTICAL_EFFECT_MATERIALS_COMPLETE')

# Keep the user-requested brighter red optics on subsequent model reimports.
import runpy
runpy.run_path(str(O/'brighten_laser.py'), run_name='__main__')
