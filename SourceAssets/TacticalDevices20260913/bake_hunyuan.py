...
O=Path(__file__).parent/'HunyuanV3';S=O.parent.parent;T=O/'Textures';T.mkdir(exist_ok=True)
...
normal_image=next(i for i in bpy.data.images if 'normal' in i.name.lower());normal_image.filepath_raw=str(O/'flashlight/T_flashlight_Normal.png');normal_image.file_format='PNG';normal_image.save()
...
report={};sources={k:v for k,v in json.loads((O/'authoring.json').read_text()).items() if v['family']=='QBZ191'}
for key,info in sources.items():
 before=set(s.objects);bpy.ops.import_scene.fbx(filepath=info['fbx']);socket_objects=[x for x in s.objects if x not in before and x.type=='EMPTY' and x.name.startswith('SOCKET_')];objects=[x for x in s.objects if x not in before and x.type=='MESH']
 bpy.ops.object.select_all(action='DESELECT')
 for x in objects:x.select_set(True)
 bpy.context.view_layer.objects.active=objects[-1]
 if len(objects)>1:bpy.ops.object.join()
 ob=bpy.context.object;ob.name=info['mesh_name'];bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)
 ...
 for m in original:
  ...
  for nd in m.node_tree.nodes:
   if nd.type=='TEX_IMAGE' and nd.image:
    kindtex='BaseColor' if nd.image.colorspace_settings.name=='sRGB' else 'MetalRough'
    src=O/info['kind']/('T_'+info['kind']+'_'+kindtex+'.png')
    nd.image=bpy.data.images.load(str(src),check_existing=True)
 ...
 for sock in socket_objects:sock.hide_set(True);sock.name=key+'_'+sock.name
...
bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(O/'QBZ191_TacticalCoating_Editable.blend'));print('QBZ_COATING_BAKE_COMPLETE',flush=True)
