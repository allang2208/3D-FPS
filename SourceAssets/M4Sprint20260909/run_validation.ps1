...
$arguments=@(
    ...
    '-game','-windowed','-RenderOffscreen','-ResX=1280','-ResY=720','-unattended','-nosplash',
    '-GunplayAudit',('-ColdSteelProfile='+$Label),('-GunplayLabel='+$Label),'-FixedSeed','-UseFixedTimeStep',('-FPS='+$Fps),
    $rhi,('-abslog="'+$logPath+'"')
)
...
$result=[ordered]@{label=$Label;simulation_hz=$Fps;rhi=$rhi;process_exit=$testProcess.ExitCode;pass=$passed;fail=$failed;completion_marker=$complete;assertions=$assertions;runtime_log=$logPath;note='Real rendered game, isolated clear floor, simulated controller inputs. Fixed simulation rate, not measured FPS.'}
