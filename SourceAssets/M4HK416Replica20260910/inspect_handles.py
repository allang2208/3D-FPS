...
partmap={v:color for ident,color in [(39,1),(36,2),(37,3),(38,4)] for v in parts[ident]['vertices']}
...
target=root@Vector((0,-.13,.06))
d=bpy.data.cameras.new('Handle');c=bpy.data.objects.new('Handle',d);s.collection.objects.link(c);s.camera=c;d.type='ORTHO';d.ortho_scale=.28
for x in [-1,1]:
...
for label,offset in [('rear',(.16,-.25,.17)),('side',(.3,0,.05))]:
 c.location=root@Vector((0,-.13,.06))+root.to_3x3()@Vector(offset);c.rotation_euler=(target-c.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(out/f'handle_{label}.png');bpy.ops.render.render(write_still=True)
