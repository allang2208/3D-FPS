...
for clip,frames in [('reload_empty',list(range(120,138))),('reload',[29,50,76,95]),('equip_charge',[9,15,22,30])]:
 ...
 for t in frames:
  ...
  result.append({'frame':t,'hand_local':list(inv@r.pose.bones['hand_l'].head),'finger_tips':{n:list(inv@r.pose.bones[n+'_03_l'].tail) for n in ['thumb','index','middle','ring','pinky']},'receiver_slab_vertices':len(inside),'closest_catch':list(min(points,key=lambda p:(p-Vector((.0185,-.0934,.055))).length)), 'min_side_cm':min([p.x*100 for p in inside],default=999)})
  obj.to_mesh_clear()
 report[clip]=result
