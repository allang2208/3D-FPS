"""Import prefiltered silver coverage and the readable translucent rune revision."""
import json
from pathlib import Path
import unreal as ue

root = Path(__file__).resolve().parent
dest = '/Game/Weapons/MeleeRunes20260915/SurfaceV2'
tools = ue.AssetToolsHelpers.get_asset_tools()
L = ue.MaterialEditingLibrary
keys = ('resonance_rune', 'erosion_rune', 'conduction_rune')
textures = {}
for key in keys:
    for folder, name, icon in ((dest, 'T_Mask_'+key, False),):
        task = ue.AssetImportTask()
        task.filename = str(root/'RuneMasks'/(key+'.png'))
        ...
        texture.compression_settings = ue.TextureCompressionSettings.TC_EDITOR_ICON if icon else ue.TextureCompressionSettings.TC_GRAYSCALE
...
material = ue.load_asset(dest+'/M_SilverRuneSurfaceV2')
if material is None:
    material = tools.create_asset('M_SilverRuneSurfaceV2', dest, ue.Material, ue.MaterialFactoryNew())
...
texture_node = node(ue.MaterialExpressionTextureObjectParameter, parameter_name='RuneTexture',
                    texture=textures['resonance_rune'], sampler_type=ue.MaterialSamplerType.SAMPLERTYPE_LINEAR_GRAYSCALE)
inputs = {
    ...
    'GlowStrength': (scalar('GlowStrength',8),'')
}
pins=[]
for name in inputs:
    pin=ue.CustomInput()
...
(root/'rune_import_receipt.json').write_text(json.dumps({'material':material.get_path_name(),'textures':{k:t.get_path_name() for k,t in textures.items()}},indent=2),encoding='utf-8')
