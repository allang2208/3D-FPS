...
P=Path(__file__).parent;D='/Game/Weapons/PanoramicRedDot';lib=unreal.MaterialEditingLibrary;assets=unreal.AssetToolsHelpers.get_asset_tools()
...
t=unreal.AssetImportTask();t.filename=str(P/'SM_PanoramicRedDot.fbx');t.destination_path=D;t.destination_name='SM_PanoramicRedDot';t.automated=True;t.replace_existing=True;t.save=True;t.options=o;assets.import_asset_tasks([t]);mesh=unreal.load_asset(D+'/SM_PanoramicRedDot');assert mesh
textures={}
for name in ['BaseColor','Roughness','Metallic','Normal']:
 t=unreal.AssetImportTask();t.filename=str(P/('T_Panoramic_'+name+'.png'));t.destination_path=D;t.automated=True;t.replace_existing=True;t.save=True;assets.import_asset_tasks([t]);tex=unreal.load_asset(D+'/T_Panoramic_'+name);assert tex
...
body=material('M_Panoramic_Body')
...
glass=material('M_Panoramic_Glass');glass.set_editor_property('blend_mode',unreal.BlendMode.BLEND_TRANSLUCENT);glass.set_editor_property('two_sided',True)
...
reticle=material('M_Panoramic_Reticle');reticle.set_editor_property('shading_model',unreal.MaterialShadingModel.MSM_UNLIT);reticle.set_editor_property('two_sided',True);constant(reticle,(40,.04,.006),unreal.MaterialProperty.MP_EMISSIVE_COLOR)
...
b=mesh.get_bounding_box();size=b.max-b.min;assert 5.8<size.x<6.2 and 5.2<size.y<5.9 and 4.5<size.z<5.0,str(size)
...
(P/'import_report.json').write_text(json.dumps(report,indent=2));unreal.log('PANORAMIC_IMPORT_PASS '+json.dumps(report))
