...
foreach($phase in @('write','reload')){
    ...
    $arguments='"{0}/FPSGAME.uproject" /Game/GameMaps/DayNight_Lighting -game -windowed -ResX={1} -ResY={2} -ForceRes -unattended -nosplash -nosound -M4GunsmithAudit -PanoramicRedDotAudit -ColdSteelProfile=M4GunsmithAudit_Panoramic_{3} -abslog="{4}"{5}' -f $root,$Width,$Height,$RunId,$log,$extra
    ...
    foreach($name in @('m4-gunsmith-panel','m4-holographic-ads','m4-holographic-hip','m4-holographic-fire','m4-holographic-reload','m4-iron-restored','panoramic-side','panoramic-rotated','panoramic-gameplay-ads','panoramic-inventory-icon')){Copy-Item -LiteralPath "$root/Saved/PanoramicRedDotAudit/$name.png" -Destination (Join-Path $out "$phase-$name.png")}
    Write-Output "PASS panoramic $phase : $log"
}
