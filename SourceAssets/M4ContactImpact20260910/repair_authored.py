...
for clip in ['reload','reload_empty','equip_charge']:
 ...
 if not equip:
  ...
  escape=(initial[holdframe]['WPN_root'].inverted()@initial[holdframe]['WPN_SOCKET_Magazine']).to_3x3()@Vector((0,0,-.085))
  ...
  if empty:
   anchors=[(88,grasp),(97,shifted(grasp,escape)),
   ...
   anchors=[(98,grasp),(106,shifted(grasp,escape)),
            (110,shifted(grasp,escape+Vector((.095,0,0)))),
            (119,shifted(held,(.085,0,0))),
            (123,shifted(held,(.045,0,0))),(126,held)]
  fingerframes=[(88,88),(97,88),(111,130),(135,130),(151,162),(162,162)] if empty else [(98,98),(106,98),(118,126),(126,126)]
  # Approach the magazine from below its base, then lift into the fitted grasp.
  if empty:
   early=[(0,held),(4,shifted(held,(.085,0,0))),(10,handrel(10))]
   for f in range(11):
    k=next(i for i in range(len(early)-1) if early[i][0]<=f<=early[i+1][0]);a,A=early[k];b,B=early[k+1];target=initial[f]['WPN_root']@mix(A,B,smooth((f-a)/(b-a)));delta=target@initial[f]['hand_l'].inverted()
    for n in names:
     if n.endswith('_l'):out[f][n]=delta@initial[f][n]
  entryend=44 if empty else 23;entrystart=28 if empty else 0
  entrygrasp=handrel(entryend);entryescape=(initial[entryend]['WPN_root'].inverted()@initial[entryend]['WPN_SOCKET_Magazine']).to_3x3()@Vector((0,0,-.075))
  entryanchors=[(28,handrel(28)),(31,shifted(handrel(28),(.10,0,0))),(35,shifted(entrygrasp,entryescape+Vector((.085,0,0)))),(39,shifted(entrygrasp,entryescape)),(43,entrygrasp),(44,entrygrasp)] if empty else [(0,held),(6,shifted(held,(.085,0,0))),(12,shifted(entrygrasp,entryescape+Vector((.085,0,0)))),(18,shifted(entrygrasp,entryescape)),(23,entrygrasp)]
  for f in range(entrystart,entryend+1):
   ...
   for n in names:
    if n.endswith('_l') and n.startswith(('thumb','index','middle','ring','pinky')):
     m=mix(initial[entrystart][parent[n]].inverted()@initial[entrystart][n],initial[entryend][parent[n]].inverted()@initial[entryend][n],u);out[f][n]=out[f][parent[n]]@m
 ...
 for k in range(end*4+1):
  ...
  if not equip and ((43<=f<=88) if empty else (23<=f<=100)):
