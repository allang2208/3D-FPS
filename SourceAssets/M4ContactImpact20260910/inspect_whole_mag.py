...
O=Path(__file__).parent;bpy.ops.wm.open_mainfile(filepath=str(O.parent/'M4ReloadPolish20260910/M4_Hand_MAT_Editable.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene;hand=bpy.data.objects['SK_Manny_Arms_Export'];mag=next(o for o in r.children if o.type=='MESH' and 'Magazine' in o.name)
...
report={}
for clip,lo,hi in [('reload',0,126),('reload_empty',0,162)]:
 a=bpy.data.actions['M4_MAT_'+clip];act(a);rows=[];baseline=None;drift=0
 for k in range(lo*2,hi*2+1):
  f=k/2;frame(f);dep=bpy.context.evaluated_depsgraph_get();inv=r.pose.bones['WPN_SOCKET_Magazine'].matrix.inverted();wrist=inv@r.pose.bones['hand_l'].head
 ...
 (O/'before_whole_mag.json').write_text(json.dumps(report,indent=2))
 print('WHOLE_MAG',clip,report[clip]['maximum_envelope_depth_mm'])
