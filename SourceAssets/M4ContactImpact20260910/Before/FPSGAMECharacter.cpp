...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    const FVector HipFraming = bUsingM4Infima
        ? FMath::Lerp(HipViewmodelLocation, M4ActionViewmodelLocation + ((bDrumInstalled && FParse::Param(FCommandLine::Get(), TEXT("DrumGripCandidate"))) ? FVector(12.0f, 0.0f, 0.0f) : FVector::ZeroVector), M4ActionFramingAlpha)
...
UAnimSequence* AFPSGAMECharacter::LoadAKMAnimation(const TCHAR* AssetName)
{
    if (bUsingM4Infima)
    {
        ...
        if (Clip) return LoadObject<UAnimSequence>(nullptr, *FString::Printf(TEXT("/Game/Weapons/M4ReloadPolish/A_M4_HK416_%s.A_M4_HK416_%s"), Clip, Clip));
    }
...
USoundBase* AFPSGAMECharacter::LoadAKMSound(const TCHAR* AssetName)
{
    if (bUsingM4Infima)
    {
        ...
        if (FCString::Strcmp(AssetName, TEXT("S_AKM_Fire")) == 0) HKName = TEXT("S_HK416_Fire");
            ...
            return LoadObject<USoundBase>(nullptr, TEXT("/Game/Weapons/M4ReloadPolish/S_HK416_Equip.S_HK416_Equip"));
