...
report={}
for clip,frames in [('reload',[0,5,10,15,20,25,30,40])]:
 a=bpy.data.actions['M4_MAT_'+clip];r.animation_data.action=a;r.animation_data.action_slot=a.slots[0];rows=[]
 for f in frames:
  bpy.context.scene.frame_set(0);bpy.context.view_layer.update();bone=r.pose.bones['clavicle_l'];m=bone.matrix.copy();m.translation+=r.pose.bones['WPN_root'].matrix.to_3x3()@__import__('mathutils').Vector((0,-f/1000,0));bone.matrix=m;bpy.context.view_layer.update();dg=bpy.context.evaluated_depsgraph_get();trees=[];polys=[];verts=[]
  for obj in [hand,mag]:
 ...
 report[clip]=rows
(O/'base_grip_fit.json').write_text(json.dumps(report,indent=2));print(report)
