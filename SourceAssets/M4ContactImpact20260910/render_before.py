...
bpy.ops.wm.open_mainfile(filepath=str(O.parent/'M4ReloadPolish20260910/M4_Hand_MAT_Editable.blend'))
 ...
 for version in ['M4_MAT_']:
  ...
  for f in fs:
   ...
   for view in (['fps'] if continuous else ['fps','palm']):
    ...
    folder=O/('Frames' if continuous else 'Review')/(version+clip)/view;folder.mkdir(parents=True,exist_ok=True)
