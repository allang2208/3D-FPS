"""Install the two overhead reach corrections through the editor batch bridge."""
import unreal as u,json,hashlib,shutil
from pathlib import Path
P=Path(__file__).parent;ROOT=Path(u.Paths.project_dir()).resolve()
patches=[(P/v/'Overhead_patch.json',json.loads((P/v/'Overhead_patch.json').read_text())) for v in ('Standard','LongGrip')]
receipt_path=P/'import_receipt.json'
receipt=json.loads(receipt_path.read_text()) if receipt_path.exists() else {'revision':'DowncutReachV5','saved':{},'runtime_tested':False}
targets={p['asset'].split('.')[0] for _,p in patches}
if u.get_editor_subsystem(u.LevelEditorSubsystem).is_in_play_in_editor():
    raise RuntimeError('PIE active: target animations preserved; end play before installing.')
dirty=[p.get_name() for p in u.EditorLoadingAndSavingUtils.get_dirty_content_packages() if p.get_name() in targets]
if dirty:raise RuntimeError('Unsaved target assets: '+str(dirty))
for file,patch in patches:
    if file.parent.name in receipt['saved']:continue
    ...
    if hashlib.sha256(disk.read_bytes()).hexdigest()!=patch['source_sha256']:
        raise RuntimeError('Target changed since source capture: '+patch['asset'])
for file,patch in patches:
    variant=file.parent.name
    if variant in receipt['saved']:continue
    asset=u.load_asset(patch['asset']);disk=ROOT/'Content'/(patch['asset'].split('.')[0].removeprefix('/Game/')+'.uasset')
    backup=P/'Before'/variant/disk.name;backup.parent.mkdir(parents=True,exist_ok=True)
    ...
    controller.open_bracket('Open sword downswing arc while preserving original grasp',False)
    try:
    ...
    u.EditorAssetLibrary.set_metadata_tag(asset,'SwordElbow.Revision','DowncutReachV5')
    ...
    receipt['saved'][variant]={'asset':asset.get_path_name(),'seconds':asset.get_play_length(),'backup':str(backup),
        'source_sha256':patch['source_sha256'],'saved_sha256':hashlib.sha256(disk.read_bytes()).hexdigest(),
        'editable_keys':str(file),'fbx':str(export)}
    receipt_path.write_text(json.dumps(receipt,indent=2),encoding='utf-8')
    u.log('SWORD_DOWNCUT_IMPORTED '+variant)
