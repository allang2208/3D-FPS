...
BASE=os.environ.get('COMFY_BASE_URL','http://127.0.0.1:18189')
REMOTE='EnhancementMaterials20260911'
...
def submit(asset,quality):
    ...
    v.update(pipeline=['1',0],front_image=['10',0],right_image=['11',0],back_image=['12',0],pipeline_type='1024_cascade' if high else '512',seed=91101+['enhancement_stone','magic_dust'].index(asset),sparse_structure_steps=16 if high else 12,shape_steps=32 if high else 16,texture_steps=24 if high else 12,max_num_tokens=999999 if high else 49152,generate_texture_slat=True,fill_holes=False)
    ...
    try: result=req('/prompt',json.dumps({'client_id':'enhancement-materials-20260911','prompt':w}).encode(),{'Content-Type':'application/json'})
