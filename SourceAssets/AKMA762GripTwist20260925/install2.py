"""Import the aimed-up AKM / A762 reload thumbs (v2) over their existing paths.
...
are the round-1 twist repair and are copied to Before2/ before being overwritten,
...
CHUNK_FILE = O / 'install2_chunk.json'
if CHUNK_FILE.exists():
    ...
    receipt_name = 'install2_receipt_%02d.json' % start
else:
    jobs = SOURCES
    receipt_name = 'install2_receipt.json'
...
for job in jobs:
    ...
    fbx = O / 'v2' / job['gun'] / job['magazine'] / job['family'] / (stem + '.fbx')
    if not fbx.exists():
    ...
    backup = O / 'Before2' / (asset.removeprefix('/Game/') + '.uasset')
    ...
    print('THUMB2_IMPORTED', key, anim.get_play_length(), 'written=%s returned=%s' % (written, returned), flush=True)
print('THUMB2_IMPORT_COMPLETE', len(report), receipt_name)
