...
for job in SOURCES:
    ...
    dst = O / 'v3' / gun / magaz / family / (stem + '.blend')
    if not dst.exists():
    ...
    print('%-42s visible %5.1f mm  angle %5.2f  rise %+6.1f  web_gap %6.2f (ship %6.2f, %+5.2f)  tip_to_mag_axis %6.2f (ship %6.2f, %+5.2f)  pad %s  thumb_err %.4f%s'
          % (row['clip'], row['vis_len_mm'], row['angle_to_mag_deg'], row['rise_mm'],
             row['web_gap_mm'], row['web_gap_shipping_mm'], row['web_gap_delta_vs_shipping_mm'],
             row['tip_to_mag_axis_mm'], row['tip_to_mag_axis_shipping_mm'], row['tip_moved_away_mm'],
             row['pad_min_mm'], max(thumb_err.values()), '  NOTE' if notes else ''), flush=True)
(O / 'verify3.json').write_text(json.dumps({'rows': rows, 'problems': problems, 'notes': all_notes}, indent=1))
print('VERIFY3_OK clips=%d problems=%d' % (len(rows), len(problems)), flush=True)
for p in problems:
