...
for job in SOURCES:
    ...
    tag = u.EditorAssetLibrary.get_metadata_tag(anim, 'GripRefinement3')
    key = '/'.join((job['gun'], job['magazine'], job['family'], job['clip']))
    expect = str(O / 'v3' / job['gun'] / job['magazine'] / job['family'] /
                 (Path(asset).name + '.fbx'))
...
(O / 'readback3.json').write_text(json.dumps({'rows': rows, 'problems': problems}, indent=1), encoding='utf-8')
print('THUMB3_READBACK clips=%d problems=%d' % (len(rows), len(problems)), flush=True)
