...
for job in SOURCES:
    ...
    tag = u.EditorAssetLibrary.get_metadata_tag(anim, 'GripRefinement4')
    key = '/'.join((job['gun'], job['magazine'], job['family'], job['clip']))
    expect = str(O / 'v4' / job['gun'] / job['magazine'] / job['family'] /
                 (Path(asset).name + '.fbx'))
...
(O / 'readback4.json').write_text(json.dumps({'rows': rows, 'problems': problems}, indent=1), encoding='utf-8')
print('THUMB4_READBACK clips=%d problems=%d' % (len(rows), len(problems)), flush=True)
