# Import the thumb-refined AKM / A762 reload clips through a headless UE commandlet.
...
try {
    ...
    & $engine $root/FPSGAME.uproject -run=pythonscript "-script=$script" -unattended -nop4 -nosplash `
        -NullRHI -ModelContextProtocolPort=18012 "-abslog=$log" 2>&1 | Select-Object -Last 30
    $code = $LASTEXITCODE
