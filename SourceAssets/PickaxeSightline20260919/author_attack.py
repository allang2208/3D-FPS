"""Near-sightline overhead pickaxe with an extended supporting left arm.
...
SOURCE = ROOT/'SourceAssets/PickaxeElbowArc20260919/Pickaxe_ElbowArc_Editable.blend'
REFERENCE = SOURCE
OUT = HERE/'Export'
...
def return_frame(start, u):
    return bezier([start, pitch_frame((-.005, .29, -.35), 54),
...
def swing_frame(t):
    if t <= CFG['raise_end_seconds']:
        return bezier([ready, centered(READY_CENTER+Vector((-.025, -.055, .035)), ready.to_quaternion()),
                       pitch_frame((-.015, .28, .34), -42), RAISED], ease(t/CFG['raise_end_seconds']))
    ...
    if t <= CFG['contact_seconds']:
        ...
        return bezier([TOP, pitch_frame((-.015, .27, .405), -62),
            pitch_frame((-.015, .37, -.135), 49), IMPACT], u**1.65)
...
def hand_frame(wpn, side, grip_weight, down, support_weight, lift):
    ...
    def cost(degrees):
        ...
        guide = Vector(CFG['elbow_pole_top'][side]).lerp(Vector(CFG['elbow_pole_impact'][side]), down)
        guide.x *= sign
...
def solve_arm(pose, side, hand, weight, down, lift, state):
    ...
    guide.x *= sign
    desired = (guide-direction*guide.dot(direction)).normalized()
    # Each elbow has its own anatomical pole; the left overhead elbow now stays
    # below the extended forearm. No wrist-driven search may
    ...
    # V2 left 130-180 degrees of axial rotation to the forearm. Share a bounded
    # part through the humerus, then derive the elbow base from that SAME frame.
    # The elbow is not an independent rotating joint, and the grip stays fixed.
    fore_hinge = (upper_deform @ b).rotation_difference(fore_axis) @ upper_deform
    requested_roll = unwrap(axial_delta(full_fore, fore_hinge, fore_axis), state.get(side+'_raw_twist'))
    state[side+'_raw_twist'] = requested_roll
    limit = math.radians(CFG['upper_roll_limit_degrees'][side])
    humerus_roll = max(-limit, min(limit, requested_roll*CFG['upper_roll_share']))*weight
...
def pose_frame(wpn, t, hit, state):
    ...
    if CFG['release_seconds'] < t <= CFG['contact_seconds']:
        # Keep elevated shoulder support while the accelerating hands are still
        # high; the old phase-only shoulder dropped ahead of the weapon arc.
        lift = max(0., min(1., ((wpn@PIVOT).z-CFG['impact']['center_m'][2])/
                              (CFG['top']['center_m'][2]-CFG['impact']['center_m'][2])))
...
for clip, duration in (('Swing', CFG['source_swing_seconds']), ('HitRecover', CFG['source_recovery_seconds'])):
    ...
    old.name = 'REF_BeforeSightline4_'+name
    ...
    print('PICKAXE_SIGHTLINE_EXPORTED', clip, flush=True)
