...
def restore_runtime_finish(ob,row,P):
 ...
 for index,old in enumerate(ob.data.materials):
  ...
  if 'Mouth' in mat.name:
   ...
   continue
  if row['gun']=='QBZ' or 'Magazine_Base' in mat.name:continue
 ...
 # Match the live ASH surface shader: the old source anime BaseColor is not used.
 for mat in ob.data.materials:
  bs=next(n for n in mat.node_tree.nodes if n.type=='BSDF_PRINCIPLED')
  for link in list(bs.inputs['Base Color'].links):mat.node_tree.links.remove(link)
  bs.inputs['Base Color'].default_value=(.044,.044,.046,1) if 'Magazine_Base' in mat.name else (.05,.05,.052,1)
