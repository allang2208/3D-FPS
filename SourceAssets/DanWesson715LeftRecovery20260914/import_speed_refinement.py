"""Import only the six speedloader clips changed by the final stow refinement."""
...
for kind,entry in manifest['clips'].items():
    if kind.startswith('speed_'):
        entry['destination']='/Game/Weapons/DanWesson715/LeftRecovery20260914/LoaderStow'
...
receipt={}
for kind,entry in manifest['clips'].items():
    if not kind.startswith('speed_'): continue
    ...
    receipt[kind]=clip.get_path_name()
    u.log('DW715_SPEED_RECOVERY_FINAL_IMPORTED '+kind)
(O/'import-speed-final.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
u.log('DW715_SPEED_RECOVERY_FINAL_COMPLETE')
