...
SOURCE = ROOT/'SourceAssets/PickaxeNaturalArms20260919/Pickaxe_NaturalArms_Editable.blend'
REFERENCE = SOURCE
OUT = HERE/'Export'
...
def swing_frame(t):
    if t <= CFG['raise_end_seconds']:
        return bezier([ready, centered(READY_CENTER+Vector((-.025, -.055, .035)), ready.to_quaternion()),
                       pitch_frame((-.015, .30, .225), -7), RAISED], ease(t/CFG['raise_end_seconds']))
    ...
    if t <= CFG['contact_seconds']:
        ...
        return bezier([TOP, pitch_frame((-.015, .315, .285), -10),
...
for clip, duration in (('Swing', CFG['source_swing_seconds']), ('HitRecover', CFG['source_recovery_seconds'])):
    ...
    old.name = 'REF_BeforeElbowArc3_'+name
...
scene.frame_set(0)
blend_file = HERE/'Pickaxe_ElbowArc_Editable.blend'
