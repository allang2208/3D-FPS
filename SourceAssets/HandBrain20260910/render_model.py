...
bpy.ops.import_scene.gltf(filepath=str(root / 'handbrain_detailed_v01.glb'))
...
bpy.ops.export_scene.fbx(filepath=str(root/'handbrain_detailed_v01.fbx'), use_selection=True, path_mode='COPY', embed_textures=True, add_leaf_bones=False)
...
for name,loc in [('front',(0,-6,1.15)),('right',(6,0,1.15)),('back',(0,6,1.15)),('hero',(4,-6,2.7))]:
    camera.location=loc
    camera.rotation_euler=(Vector((0,0,1))-camera.location).to_track_quat('-Z','Y').to_euler()
    scene.render.filepath=str(root/('render_'+name+'.png'))
    bpy.ops.render.render(write_still=True)
bpy.ops.file.pack_all()
bpy.ops.wm.save_as_mainfile(filepath=str(root/'handbrain_detailed_v01.blend'))
