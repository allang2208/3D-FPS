...
for obj in [o for o in bpy.context.scene.objects if o.type=='MESH']:
 ...
 for index,mat in enumerate(obj.data.materials):
  bs=next(n for n in mat.node_tree.nodes if n.type=='BSDF_PRINCIPLED');maps={'material':mat.name,'slot':index}
  for semantic,socket in [('BaseColor','Base Color'),('Normal_DirectX','Normal'),('Roughness','Roughness')]:
   src=find_image(bs.inputs[socket])
   if src is None:
    if socket!='Normal':
     v=bs.inputs[socket].default_value;maps[semantic]=list(v) if hasattr(v,'__len__') else float(v)
    continue
   dst=src.copy();dst.name=obj.name+'_'+str(index)+'_'+semantic
   ...
   dst.pixels.foreach_set(data);dst.filepath_raw=str(out/(dst.name+'.png'));dst.file_format='PNG';dst.save()
   maps[semantic]={'file':'textures/'+dst.name+'.png','sRGB':semantic=='BaseColor'}
  maps['Metallic']=0;slots.append(maps)
 manifest[obj.name]=slots
(R/'delivery/ue_materials.json').write_text(json.dumps(manifest,indent=2))
print('SINGLE_FACE_MATERIALS_EXPORTED')
