"""Make full-level AKM suppressor voices solely from the user-selected AKM fire WAV.
...
SOURCE = PROJECT / 'SourceAssets/AKMVideoAudio20260921/S_AKM_Fire.wav'
x, rate = sf.read(SOURCE, always_2d=True, dtype='float64')
if rate != 48000:
    raise RuntimeError('This authoring recipe uses the existing 48 kHz AKM source.')
...
def weighted(signal):
    ...
    # 400 ms loudness gate (the source itself is only 395 ms long).
...
manifest = {
    'source': str(SOURCE),
    'original_source': 'https://www.bilibili.com/video/BV17VtT6REZr/ (373.040-373.435s)',
    'source_sha256': hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
    'rights': 'Derived from user-selected LAZERH video; original rights retained, no public redistribution license established.',
    'recipe': 'AKM source only; filtered blast, retained low body/mechanics, short chamber returns; compensated source level.',
    'sample_rate': rate, 'channels': x.shape[1], 'duration_seconds': len(x) / rate,
    'pitch_multiplier': 1.0, 'runtime_volume_multiplier': 'Same as normal AKM fire.',
    'source_rms': source_rms, 'source_weighted_rms': source_weighted,
    'outputs': outputs,
    'listening_or_gameplay_tested': False,
}
(HERE / 'provenance.json').write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding='utf-8')
