...
for k,entry in slots.items():
 if k not in ('prism_scope_2x','lpvo_1_6x','ring'):continue
 for slot in entry['materials']:
  ...
  vc=E.create_material_expression(m,u.MaterialExpressionVertexColor);region=E.create_material_expression(m,u.MaterialExpressionMultiply);assert E.connect_material_expressions(mask,'',region,'A');assert E.connect_material_expressions(vc,'R',region,'B');mask=region
...
for k,entry in slots.items():
 if k not in ('prism_scope_2x','lpvo_1_6x','ring'):continue
 f=O/geo[k]['fbx'];opt=u.FbxImportUI();opt.automated_import_should_detect_type=False;opt.mesh_type_to_import=u.FBXImportType.FBXIT_STATIC_MESH;opt.import_materials=False;opt.import_textures=False;opt.static_mesh_import_data.combine_meshes=True;opt.static_mesh_import_data.normal_import_method=u.FBXNormalImportMethod.FBXNIM_IMPORT_NORMALS;opt.static_mesh_import_data.generate_lightmap_u_vs=False;opt.static_mesh_import_data.vertex_color_import_option=u.VertexColorImportOption.REPLACE
...
(O/'import-optical-interior.json').write_text(json.dumps(report,indent=2));u.log('AKM_OPTIC_STEEL_IMPORT_PASS')
