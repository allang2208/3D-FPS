"""Author the 0.25-second carry-to-strike windup for dash attacks.
...
for variant in ('Standard','LongGrip'):
    ...
    clips = [('SprintOverhead',round(data['clips']['Overhead']['seconds']*FPS))]
    actions = {}
    ...
    rig.animation_data.action = actions['SprintOverhead']
    rig.animation_data.action_slot = actions['SprintOverhead'].slots[0]
    scene.frame_start,scene.frame_end = round(CFG['attack_source_start']*FPS),intervals
    scene.frame_set(0)
    bpy.ops.wm.save_as_mainfile(filepath=str(out/'Sword_DashWindup_Editable.blend'))
