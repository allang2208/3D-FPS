...
for role in ['Idle','Walk','Attack','Death']:
    ...
    clip.set_editor_property('loop',role in ['Idle','Walk'])
    clip.set_editor_property('enable_root_motion',False)
    clip.set_editor_property('force_root_lock',True)
    lib.set_metadata_tag(clip,'Source','Mesh2Motion CC0: '+contract['clips'][role]['source'])
...
lib.set_metadata_tag(mesh,'Source','User-provided Meshy_AI_Mutant_Zombie_Charact_biped.zip')
...
(ROOT/'ue_delivery.json').write_text(json.dumps({'mesh':mesh.get_path_name(),
    ...
    'content_folder':DEST,'stage_project':str(ROOT/'UEAuthoring/FatZombieAuthoring.uproject'),
    'state':'Imported asset package. No runtime, render, or gameplay testing performed.'},indent=2),encoding='utf-8')
u.log('FAT_ZOMBIE_FINAL_IMPORT_COMPLETE '+json.dumps(outputs))
