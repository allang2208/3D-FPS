...
foreach($taskWidth in $Widths){
    ...
    $taskArgs=@('D:/FPS3D/FPSGAME/FPSGAME.uproject','/Game/GameMaps/DayNight_Lighting','-game','-windowed',"-ResX=$taskWidth","-ResY=$taskHeight",'-ForceRes','-unattended','-nosound','-NoSplash','-ExecCmds="fov 48"','-ColdSteelWarehouseAudit',"-ColdSteelProfile=$taskProfile","-abslog=$taskLog")
