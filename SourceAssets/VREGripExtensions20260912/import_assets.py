...
build=read(OUT/'build.json');assert len(build)==36;report={};A=u.AssetToolsHelpers.get_asset_tools()
for key,info in build.items():
 w,v,clip=key.split(':');name=prefix(w,v)+clip
 ...
 task=u.AssetImportTask();task.filename=info['output'];task.destination_path=asset_dir(w,v);task.destination_name=name;task.options=opt;task.automated=True;task.replace_existing=True;task.save=False;A.import_asset_tasks([task])
 asset=u.load_asset(info['asset']);assert asset
 ...
 report[key]={'asset':asset.get_path_name(),'duration':asset.get_play_length(),'source':task.filename};(OUT/'import.json').write_text(json.dumps(report,indent=2));u.log('GRASP_EXTENSION_IMPORTED '+key)
u.log('GRASP_EXTENSION_IMPORT_PASS 36')
