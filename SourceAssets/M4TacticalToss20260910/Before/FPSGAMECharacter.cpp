...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    {
        // The throw reload is authored around a fixed right-hand grip.
        // Do not add a second forward/back translation through action framing.
        ActionFramingTarget = 0.0f;
    }
    ...
    if (bUsingM4Infima)
    {
        ...
        if (Clip)
        {
            const FString Path = FString::Printf(TEXT("/Game/Weapons/%s/A_M4_HK416_%s.A_M4_HK416_%s"),
                FCString::Strcmp(Clip, TEXT("reload_empty")) == 0 ? TEXT("M4SlapImpactFinal") : TEXT("M4WrapGripFinal"), Clip, Clip);
