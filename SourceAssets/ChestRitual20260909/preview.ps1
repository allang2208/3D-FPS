...
foreach($taskWidth in $Widths){
    ...
    $taskArgs=@('D:/FPS3D/FPSGAME/FPSGAME.uproject','/Game/GameMaps/DayNight_Lighting','-game','-windowed',"-ResX=$taskWidth","-ResY=$taskHeight",'-ForceRes','-unattended','-nosound','-NoSplash','-NoTextureStreaming','-ExecCmds="fov 55,slomo 0.15"','-ColdSteelWarehouseAudit',"-ColdSteelProfile=$taskProfile","-abslog=$taskLog")
