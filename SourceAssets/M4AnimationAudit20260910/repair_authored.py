...
for clip in ['reload','reload_empty','equip_charge']:
 ...
 for f in range(end+1):
  ...
  root=p['WPN_root']
  if empty and 21<=f<=44:
   if f<=33:
    m=magrest.copy();m.translation.z-=.015;origin=(raw[21]['WPN_root']@m).translation
    u=(f-21)/12;p['WPN_SOCKET_Magazine'].translation=origin+Vector((.03*u,0,-.72*u*u))
   else:
    lowmag=Vector((-.10,.25,-.75));p['WPN_SOCKET_Magazine'].translation=lowmag.lerp(p['WPN_SOCKET_Magazine'].translation,smooth((f-35)/9))
  for side in ['l','r']:
 ...
 if not equip:
  ...
  for f in range(entrystart,entryend+1):
   ...
   if empty and f>=35:
    ...
    below=(initial[f]['WPN_root'].inverted()@initial[f]['WPN_SOCKET_Magazine']).to_3x3()@Vector((0,0,-.085))
