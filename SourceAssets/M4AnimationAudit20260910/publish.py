...
O=Path(__file__).resolve().parent;project=O.parents[1];destination='/Game/Weapons/M4AnimationAuditFinal';report={}
...
for clip in ['reload','reload_empty','equip_charge']:
 name='A_M4_HK416_'+clip;file=project/'Content/Weapons/M4AnimationAuditFinal'/(name+'.uasset');saved=backup/file.name
 if not saved.exists():shutil.copy2(project/'Content/Weapons/M4ContactImpactFinal'/file.name,saved)
...
for name,wav in [('S_HK416_Equip','equip'),('S_HK416_BoltRelease','bolt_release')]:
 t=unreal.AssetImportTask();t.filename=str(O/'Audio'/(wav+'.wav'));t.destination_path=destination;t.destination_name=name;t.automated=True;t.replace_existing=True;t.save=True
 unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([t]);sound=unreal.load_asset(destination+'/'+name);assert sound
 sound.set_editor_property('loading_behavior',unreal.SoundWaveLoadingBehavior.FORCE_INLINE);assert unreal.EditorAssetLibrary.save_loaded_asset(sound,False)
 report[name]={'path':sound.get_path_name(),'duration':sound.duration}
(O/'publish_report.json').write_text(json.dumps(report,indent=2));unreal.log('M4_ANIMATION_AUDIT_PUBLISHED')
