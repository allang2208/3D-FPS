...
void AFPSGAMECharacter::InitializeWeaponVisuals()
{
    ...
    DrumReloadAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumGripRebuilt/A_M4_DrumGrip_reload.A_M4_DrumGrip_reload"));
    DrumReloadEmptyAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumGripRebuilt/A_M4_DrumGrip_reload_empty.A_M4_DrumGrip_reload_empty"));
...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    const FVector HipFraming = bUsingM4Infima
        ? FMath::Lerp(HipViewmodelLocation, M4ActionViewmodelLocation + (bDrumInstalled ? FVector(12.0f, 0.0f, 0.0f) : FVector::ZeroVector), M4ActionFramingAlpha)
...
UAnimSequence* AFPSGAMECharacter::LoadAKMAnimation(const TCHAR* AssetName)
{
    if (bUsingM4Infima)
    {
        ...
        if (Clip)
        {
            const bool bEquip = FCString::Strcmp(Clip, TEXT("equip_charge")) == 0;
            const FString Path = FString::Printf(TEXT("/Game/Weapons/%s/A_M4_HK416_%s.A_M4_HK416_%s"),
                TEXT("M4ContactImpactFinal"), Clip, Clip);
            UAnimSequence* Animation = LoadObject<UAnimSequence>(nullptr, *Path);
            if (!bEquip) UE_LOG(LogTemp, Display, TEXT("M4_CONTACT_IMPACT_ACTIVE clip=%s loaded=%d duration=%.4f"),
...
void AFPSGAMECharacter::EmitMechanicalCue(int32 CueIndex)
{
    ...
    if (Sound && Lateness < Sound->GetDuration()) PlayMechanicalSound(Sound, ContactVolume);
