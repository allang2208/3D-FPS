...
d=json.loads((O/'inputs.json').read_text());prior=json.loads((S/'SVDMagazineFit20260923/grasp_fit.json').read_text())
...
H0=np.array(d['hand_in_mag']);Q0={n:np.array(m)[:3,:3] for n,m in d['basis'].items()}
...
def posed(x):
 ...
 for j,f in enumerate(digits):
  ...
  Q[f+'_01_l']=R.from_euler('x',spread,degrees=True).as_matrix()@rz(mcp)
  Q[f+'_02_l']=rz(pip)
...
def objective(x):
 ...
 for n,ids in pads.items():
  ...
  value+=np.mean((dist[ids]-1.2)**2)*(.55 if '_02_' in n else 3.)
  c=v[ids].mean(0);front,rear=shell.front_rear(c[2])
  value+=(.25 if n.startswith('thumb') else .12)*((c[1]-(rear+.001 if n.startswith('thumb') else front-.001))*1000)**2
 value+=4*np.mean((dist[palm]-1.2)**2)
 value+=.012*np.dot(x[:3],x[:3])+.018*np.dot(x[3:6],x[3:6])+.025*np.dot(x[22:25],x[22:25])
 for j in range(4):
  mcp,pip,ratio,spread=x[6+4*j:10+4*j]
  value+=.018*(mcp-50)**2+.015*(pip-50)**2+.2*spread**2+20*(ratio-.65)**2
 return float(value)

bounds=[(-24,16),(-30,24),(-24,20)]+[(-25,25)]*3
for j in range(4):bounds += [(25,78),(32,72),(.5,.8),(-9,9)]
bounds += [(-22,22)]*3+[(20,58),(8,35)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[50,50,.65,0]
x[25:]=[40,22]
if (O/'fit_working.json').exists():x=np.array(json.loads((O/'fit_working.json').read_text())['parameters'])

def penalty(x):
 H,Q,p,v=posed(x);c=shell.clearance(v);wrong=np.minimum(semantic(v),0)
 return objective(x)+200*np.mean(np.minimum(c-.6,0)**2)+25*min(0,c.min()-.6)**2+20*np.dot(wrong,wrong)
coarse=minimize(penalty,x,method='L-BFGS-B',bounds=bounds,options={'maxiter':200,'maxfun':7000,'ftol':1e-7})
x=coarse.x
(O/'fit_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
print('SVD_NATURAL_COARSE',round(coarse.fun,3),str(coarse.message),flush=True)

def constraints(x):
 v=posed(x)[3];return np.r_[shell.clearance(sampled_surface(v,True))-.35,semantic(v)]
...
for j,f in enumerate(digits):
 ...
 opened[f+'_01_l']=R.from_euler('x',spread,degrees=True).as_matrix()@rz(max(12,mcp-16))
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'method':'Current SVD geometry; fixed metacarpals; flexion-only PIP/DIP, coupled distal curl, bounded MCP abduction; rear thumb opposition',
 'game_tested':False}
(O/'grasp_fit.json').write_text(json.dumps(out,indent=2))
(O/'fit_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
print('SVD_NATURAL_FIT',{k:out[k] for k in ['objective','optimizer_success','optimizer_message','production_clearance_mm','pad_means_mm','palm_mean_mm','joint_flexion_deg']},flush=True)
