...
ui=(O/'ui_runtime.log').read_text(encoding='utf-8-sig');assert 'VERTICAL_UI_AUDIT COMPLETE checks=24 failures=0' in ui
...
record={'runtime_run':run,'runtime_checks':log.count('FOREGRIP_AUDIT: PASS '),'runtime_failures':0,'ui_checks':24,'ui_failures':0,'animation_variants':9,'geometry_samples':len(geo),'sampled_intersections':0,'source_contract':source,'import_clips':clips,'material_assignment':imp['assigned'],'import_exit_code':1,'import_note':'Import and readback passed; process exit 1 includes existing GameFeatureData configuration and occupied localhost:8000 listener errors. New game process independently passed.','native_build':'Succeeded 2026094138','packaged_build_verified':False,'preview_note':'Actual game screenshots assembled in capture order using bounded capture-time intervals; silent visual preview, not audio verification.','preview_frames':len(entries),'fbx_sha256':hashes,'visual_review':'Game palm/front/wrist views and gunsmith side/all-attachments views reviewed; four fingers wrap straight grip, thumb opposes; preserved original arm mesh and source action contacts.'}
(O/'acceptance.json').write_text(json.dumps(record,indent=2),encoding='utf-8');print({k:record[k] for k in ['runtime_checks','runtime_failures','ui_checks','geometry_samples','sampled_intersections','preview_frames']})
