...
for clip,frames in checks:
 bpy.ops.wm.open_mainfile(filepath=str(O/('A_M4_Vertical_'+clip+'.blend')));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene
 a=bpy.data.actions['A_M4_Vertical_'+clip];r.animation_data.action=a;r.animation_data.action_slot=a.slots[0]
 ...
 for f in frames:
  ...
  for grip in [x for x in s.objects if x.name.startswith('VG_')]:
   ...
   e.to_mesh_clear()
  inv=(r.pose.bones['WPN_root'].matrix@Matrix(json.loads((O/'fit_final.json').read_text())['grip_in_root'])).inverted()
