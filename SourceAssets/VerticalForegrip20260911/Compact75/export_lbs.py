...
bpy.ops.wm.open_mainfile(filepath=str(O/'Compact_Baseline.blend'));r=bpy.data.objects['SK_M4_Infima'];hand=bpy.data.objects['SK_Manny_Arms_Export'];s=bpy.context.scene;bpy.context.view_layer.update()
...
G=np.array(json.loads((O/'fit_baseline.json').read_text())['grip_matrix']);grip=next(o for o in s.objects if o.name.startswith('VG_')) if False else next(o for o in bpy.context.scene.objects if o.name.startswith('VG_'))
