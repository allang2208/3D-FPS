...
O=Path(__file__).parent;DEST='/Game/Weapons/M4VerticalGripCompact75'
mesh=unreal.load_asset('/Game/Weapons/M4HK416Replica/SK_M4_FoldingSights_HK416');assert mesh
options=unreal.FbxImportUI();options.automated_import_should_detect_type=False;options.mesh_type_to_import=unreal.FBXImportType.FBXIT_STATIC_MESH;options.import_materials=False;options.import_textures=False;options.import_animations=False
options.static_mesh_import_data.combine_meshes=True;options.static_mesh_import_data.generate_lightmap_u_vs=False;options.static_mesh_import_data.auto_generate_collision=False
task=unreal.AssetImportTask();task.filename=str(O/'SM_VerticalForegrip.fbx');task.destination_path=DEST;task.destination_name='SM_VerticalForegrip';task.automated=True;task.replace_existing=True;task.save=True;task.options=options
unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task]);sm=unreal.load_asset(DEST+'/SM_VerticalForegrip');assert sm
materials={str(x.material_slot_name):x.material_interface for x in mesh.materials};report={'m4_materials':{n:m.get_path_name() if m else None for n,m in materials.items()},'assigned':{}}
for i,slot in enumerate(sm.static_materials):
 name=str(slot.material_slot_name);key='Grip' if 'Grip' in name or 'Recess' in name else 'Body'
 candidates=[m for n,m in materials.items() if key.lower() in n.lower() and m];assert candidates,(key,materials)
 sm.set_material(i,candidates[0]);report['assigned'][name]=candidates[0].get_path_name()
assert unreal.EditorAssetLibrary.save_loaded_asset(sm,False)
report['static_bounds_cm']=str(sm.get_bounds())
actual=sm.get_bounds().box_extent*2; expected=json.loads((O/'model_scale.json').read_text())['after_m']; assert max(abs(getattr(actual,k)-v*100) for k,v in zip(['x','y','z'],expected))<.001; report['scale_ratio']=[.75,.75,.75]
references={
...
unreal.log('VERTICAL_IMPORT_PASS')
