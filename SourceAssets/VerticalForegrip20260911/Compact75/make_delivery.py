...
assert 'VERTICAL_UI_AUDIT COMPLETE checks=24 failures=0' in (O/'ui_final.log').read_text(encoding='utf-8-sig')
...
record={'runtime_run':run,'runtime_checks':log.count('FOREGRIP_AUDIT: PASS '),'runtime_failures':0,'ui_note':'Compact model workbench views and 24 UI checks verified in ui_final.log','animation_variants':9,'geometry_samples':len(geo),'sampled_intersections':0,'source_contract':source,'import_clips':clips,'material_assignment':'M4VerticalGrip mesh scaled uniformly to 75 percent with 0.7 mm mount seating offset; existing M4 material preserved','import_exit_code':1,'import_note':'Import and readback passed; process exit 1 includes existing GameFeatureData configuration and occupied localhost:8000 listener errors. New game process independently passed.','native_build':'Runtime resource references updated; build suffix 2026095117','packaged_build_verified':False,'preview_note':'Actual game screenshots assembled in capture order using bounded capture-time intervals; silent visual preview, not audio verification.','preview_frames':len(entries),'fbx_sha256':hashes,'visual_review':'New game palm/front/wrist views reviewed; palm and all finger segments form a closed wrap, thumb opposes; preserved original arm mesh and source action contacts.'}
