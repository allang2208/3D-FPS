...
x0=np.array(json.loads((O.parent/'TightGrip/fist_solution.json').read_text())['parameters']);x0[:6]=0;x0[18:]=0
best=None;rng=np.random.default_rng(75)
for k in range(12):
 x=x0+rng.normal(0,4,len(x0));x=np.clip(x,np.array(low)+.001,np.array(high)-.001)
 sol=least_squares(residual,x,bounds=(low,high),max_nfev=180,diff_step=.001)
 if best is None or sol.cost<best.cost:best=sol
 print('FIT75',k,sol.cost,flush=True)
p=pose(best.x);(O/'fist_solution.json').write_text(json.dumps({'cost':float(best.cost*2),'parameters':best.x.tolist(),'pose':{n:p[i].tolist() for i,n in enumerate(names)}},indent=2))
