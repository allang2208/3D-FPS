...
def calc(x):
 ...
 sdf=np.linalg.norm(points[:,:,:2],axis=2)-np.interp(points[:,:,2],profile[:,0],profile[:,1]);sdf=np.maximum(sdf,np.maximum(-.1035-points[:,:,2],points[:,:,2]))
 return angles,sdf
...
for k in range(4):
 sol=least_squares(residual,(np.array(json.loads((O.parent/'TightGrip/thumb_trajectory.json').read_text())['controls']) if k==0 else rng.uniform(-25,25,9)),bounds=(-85,85),max_nfev=140,diff_step=.001)
 if best is None or sol.cost<best.cost:best=sol
 print('TRY',k,sol.cost,flush=True)
sol=best;a,s=calc(sol.x);print('THUMB_SMOOTH',sol.cost,'min clearance',s.min(),'max step',np.max(np.linalg.norm(np.diff(a,axis=0),axis=1)),sol.x,flush=True)
for e,angle in zip(entries,a):
 q=Rotation.from_matrix(base[thumb[0],:3,:3]@Rotation.from_euler('xyz',angle,degrees=True).as_matrix()).as_quat();e['basis']['thumb_01_l']=[q[3],q[0],q[1],q[2]]
 e.pop('hits',None)
(O/'release_profile.json').write_text(json.dumps(entries,indent=2));(O/'thumb_trajectory.json').write_text(json.dumps({'controls':sol.x.tolist(),'minimum_clearance':float(s.min()),'angles':a.tolist()},indent=2))
