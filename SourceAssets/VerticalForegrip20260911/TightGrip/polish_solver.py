...
rest=D['rest'];old=D['pose'];parents=D['parents'];weights=D['weights'];vertices=D['vertices'];fit=json.loads((O.parent/'Integration/fit_final.json').read_text());G=np.array(fit['grip_matrix']);Gi=np.linalg.inv(G);profile=np.array(json.loads((O/'grip_profile.json').read_text()))
...
def residual(x):
 ...
 sdf=np.maximum(sdf,np.maximum(-.138-points[:,2],points[:,2]))
