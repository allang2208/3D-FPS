...
with bpy.data.libraries.load(str(O/'VerticalForegrip_Refined.blend'),link=False) as (src,dst):dst.objects=['SM_VerticalForegrip']
parts=[]
for ob in dst.objects:
 s.collection.objects.link(ob);ob.name='VG_Grip';ob.matrix_world=G;parts.append(ob)
''' + source[end:]
...
source=source.replace('middle=G@Vector((.16,0,.035));middle-=side*.022','middle=G@Vector((-.023,.037,-.050))')
start=source.index('tb=r.pose.bones[')
end=source.index('for ob in s.objects:ob.hide_render',start)
source=source[:start]+source[end:]
source=source.replace("'fit_pose.json'","'fit_final.json'").replace('M4_Foregrip_Pose.blend','M4_Vertical_Pose.blend')
