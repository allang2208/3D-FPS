...
for x,y,z in itertools.product([-.006,-.003,0,.003,.006],[-.006,-.003,0,.003,.006],[-.003,0,.003]):
 H=base.copy();H.translation+=G.to_3x3()@Vector((x,y,z));r.pose.bones['hand_l'].matrix=H
 count,near,_=measure('palm');score=count*100+near*3000+Vector((x,y,z)).length*10
 if best is None or score<best[0]:best=(score,H.copy(),count,near)
r.pose.bones['hand_l'].matrix=best[1];bpy.context.view_layer.update();data['hand_in_root']=[list(row) for row in r.pose.bones['WPN_root'].matrix.inverted()@best[1]];print('PALM',best[2:],flush=True)
