...
bpy.ops.wm.open_mainfile(filepath=str(O/'M4_Vertical_Pose.blend'))
...
for d in ['index','middle','ring','pinky']:
 ...
 for mcp,pip,dip in itertools.product([-30,-20,-10,0],[45,55,65,75,85],[10,20,30,40]):
  ...
  score=hits*3+near*300+abs(mcp+10)*.03+abs(pip-65)*.015+abs(dip-30)*.015+(0 if enclosed else 30)
...
for iteration in range(2):
 ...
 for x,y,z in itertools.product([-.003,0,.003],repeat=3):
  ...
  score=sum(v[0] for v in readings.values())*3+sum(readings[d][1] for d in ['index','middle','thumb','palm'])*500+Vector((x,y,z)).length*100+sum(0 if readings[d][2] else 30 for d in ['index','middle'])
...
for d in ['index','middle','ring','pinky']:
 ...
 for mcp,pip,dip in itertools.product([-30,-20,-10,0],[45,55,65,75,85],[10,20,30,40]):
  for j,ang in [(1,mcp),(2,pip),(3,dip)]:r.pose.bones[f'{d}_{j:02}_l'].rotation_quaternion=Quaternion((0,0,1),math.radians(ang))
  hits,near,enclosed=measure(d);score=hits*10+near*(3000 if d in ['index','middle'] else 150)+abs(mcp+10)*.02+abs(pip-65)*.01+abs(dip-30)*.01+(0 if enclosed else 30)
...
print('VERTICAL_CONTACT_FIT_PASS',report,flush=True)
