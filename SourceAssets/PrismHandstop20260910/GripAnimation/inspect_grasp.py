...
bpy.ops.wm.open_mainfile(filepath=str(O/'M4_Prism_Fitted.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene
fit=json.loads((O/'fit_final.json').read_text());G=Matrix(fit['grip_matrix']);inv=G.inverted();dg=bpy.context.evaluated_depsgraph_get()
grip=next(o for o in s.objects if o.name.startswith('PH_'));e=grip.evaluated_get(dg);m=e.to_mesh();m.calc_loop_triangles();gv=[inv@e.matrix_world@v.co for v in m.vertices];tree=BVHTree.FromPolygons(gv,[tuple(t.vertices) for t in m.loop_triangles],all_triangles=True);e.to_mesh_clear()
