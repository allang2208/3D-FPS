...
bpy.ops.wm.open_mainfile(filepath=str(O/'A_M4_Prism_idle.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene;s.frame_set(0);bpy.context.view_layer.update();r.animation_data.action=None
...
dg=bpy.context.evaluated_depsgraph_get();grip=next(o for o in s.objects if o.name.startswith('PH_'));e=grip.evaluated_get(dg);m=e.to_mesh();m.calc_loop_triangles();tree=BVHTree.FromPolygons([inv@e.matrix_world@v.co for v in m.vertices],[tuple(t.vertices) for t in m.loop_triangles],all_triangles=True);e.to_mesh_clear()
def smooth(x):x=max(0,min(1,x));return x*x*(3-2*x)
best=None
for delay in [0,.35,.65]:
 samples=[]
 for k in range(1,9):
  t=k/8;opening=smooth((t-delay)/max(.001,1-delay))
  for n,b in r.pose.bones.items():
   if n.startswith(('index','middle','ring','pinky')) and n.endswith('_l'):
    ...
    if '_02_' in n or '_03_' in n or n=='pinky_01_l':
 ...
 for x,y,z in itertools.product([-.06,-.03,0,.03,.06],[-.06,-.03,0,.03,.06],[-.08,-.04,0]):
...
data['release_vector']=best[1];data['release_open_delay']=best[2]
