...
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/SVDReloadArm20260923'
...
try {
 ...
 if($running.Count){$bridge=$true}
  ...
  if($Script -eq 'end_play.py'){Write-Output 'SVD_ARM_NO_EDITOR_RUNNING';return}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(SVD_ARM_|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
