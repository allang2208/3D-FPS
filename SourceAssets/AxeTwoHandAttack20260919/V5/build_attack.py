"""Author V5: H4 diagonal hold with the V4 wrist-sparing left arm chain.

KayKit's CC0 Slice/Chop are timing references, not retargeted skin or contacts.
H4 keeps H3 tool-local grips and V3 timing. The left forearm carries its full skin
segment, with the upper arm transported from the same frame and shoulder support.
"""
import bpy
import copy
import json
...
idle_dir = ROOT / 'SourceAssets/AxeTwoHandPose20260919/H4'
receipt = json.loads((idle_dir / 'authoring.json').read_text(encoding='utf-8'))
IDLE = receipt['config']
...
def support(side,hand,weight):
    ...
    if side=='l':
        # The right/rear load previously left the shoulder static while the
        # wrist bent to reach across the body. Protract the shoulder toward the
        # neutral elbow, smoothly bounded and faded into the accepted H3 pose.
        neutral_elbow=target-neutral*l2
        shoulder_to_elbow=neutral_elbow-shoulder
        excess=max(0.,shoulder_to_elbow.length-l1)
        limit=CFG['left_shoulder_assist_max_m']
        advance=limit*math.tanh(excess/limit)*weight
        shoulder+=shoulder_to_elbow.normalized()*advance
    ...
    preference=CFG['left_elbow_preference'] if side=='l' else CFG['elbow_preference']
    preferred=desired+natural*radius*preference
...
def pose_frame(wpn,t,state,end):
    ...
    for side in ('r','l'):
        ...
        if side=='l':
            # Carry the same anatomical frame through the bend. V3 untwisted
            # lowerarm_l at the elbow and reapplied fractions on helpers, so a
            # small wrist bend could still have a large axial wrist rotation.
            # This Manny skin already has a working whole-segment H3 grasp.
            matched_upper=(fore_deform@rest_upper).rotation_difference(up_axis) @ fore_deform
            up_deform=base_upper.slerp(matched_upper,weight)
            pose['clavicle_l'].translation+=shoulder-rest[upper].translation
            ...
            for segment in (upper,fore):
                for index in ('01','02'):
                    helper=segment.replace('_l','_twist_'+index+'_l')
...
report={'runtime_tested':False,'rendered':False,'config':CFG,'source':receipt['blend'],
        ...
        'skin_stations':stations,'clips':{},'left_arm_authoring':{}}
contact_state=None
for clip,duration in [('Swing',.68),('HitRecover',.44)]:
    ...
    for frame in range(scene.frame_end+1):
        ...
        if frame%10==0 or (clip=='Swing' and frame==round(.24*CFG['fps'])):
            fore_axis=(pose['hand_l'].translation-pose['lowerarm_l'].translation).normalized()
            neutral=(pose['hand_l'].to_quaternion()@rest['hand_l'].to_quaternion().inverted()) @ (
                rest['hand_l'].translation-rest['lowerarm_l'].translation).normalized()
            left_authored.append({'runtime_s':t,'wrist_bend_deg':math.degrees(fore_axis.angle(neutral)),
                                  'shoulder':list(pose['upperarm_l'].translation),
                                  'elbow':list(pose['lowerarm_l'].translation)})
        apply_pose(pose)
        for b in rig.pose.bones:
    ...
    report['left_arm_authoring'][clip]=left_authored
    print('AXE_V5_EXPORTED '+clip,flush=True)
