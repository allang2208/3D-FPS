...
for match in re.finditer(stamp + r'.*M4_AUDIO_CUE empty=(\d) index=(\d).*late=([\d.]+) sound=S_HK416_(\w+)', log):
    ...
    name = names[match[5]]
    template = np.frombuffer(subprocess.check_output([str(FFMPEG), '-v', 'error', '-i', str(ROOT/'Audio'/f'{name}.wav'), '-ac', '1', '-ar', str(rate), '-f', 'f32le', '-']), dtype='<f4').astype(float)
...
fire = np.frombuffer(subprocess.check_output([str(FFMPEG), '-v', 'error', '-i', str(ROOT/'Audio/fire.wav'), '-t', '0.08', '-ac', '1', '-ar', str(rate), '-f', 'f32le', '-']), dtype='<f4').astype(float)
...
report = dict(recording=str(RUN/'GunplayAudio.wav'), duration=frames/rate, rate=rate, channels=channels, fire_attack_correlation=fire_corr,
              ...
              note='Waveform correlation against unchanged source clips; audio versus log includes render-buffer latency and recording-start offset. Frame export is 10 Hz.')
