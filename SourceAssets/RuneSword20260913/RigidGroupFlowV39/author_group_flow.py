"""Reorient the complete V36 pose and smooth its entry/exit group movement.
...
def nominal_hand(t):
    # Same V36 timeline and body path, with a smooth common turn layered on top.
    if t < ENTRY:
        weight = ease(t / ENTRY)
    elif t > REF_END:
...
r['RigidGroupFlowV39'] = 'All V36 bones share one rigid transform per frame; entrance/exit group velocity blends'
note = bpy.data.texts.new('RIGID_GROUP_FLOW_V39_README')
...
bpy.ops.wm.save_as_mainfile(filepath=str(P / 'AzureRunesword_RigidGroupFlowV39.blend'))
(P / 'authoring.json').write_text(json.dumps({
    ...
    'relationship_method': 'P39[n,t] = common_group[t] @ baked_V36[n,t] for every bone',
...
print('RIGID_GROUP_FLOW_V39_AUTHORING_COMPLETE', flush=True)
