...
P=Path(__file__).parent;request=json.loads((P/'review_request.json').read_text());OUT=P/request['output'];OUT.mkdir(exist_ok=True)
...
def finish(error=None):
    ...
    (OUT/('ue_review_error.txt' if error else 'ue_review_receipt.json')).write_text(error or json.dumps(receipt,indent=2))
...
try:
    ...
    readback=[]
    for clip,duration in [('Inspect',2.90)]:
        ...
        for sample in range(round(duration*120)+1):
            ...
            for bone in u.AnimPoseExtensions.get_bone_names(poses[0]):
    ...
    (OUT/'ue_animation_readback.json').write_text(json.dumps(readback,indent=2))
    shots=[(f"frame_{round(t*120):03d}",'Inspect',t,False) for t in request['times']]
    def tick(dt):
        global rt
        try:
            if time.monotonic()-state['started']<12:return
            if time.monotonic()-state['started']>480:finish('Offscreen arm review exceeded 480 seconds');return
            ...
            if f==1:
                ...
                if shot[3]:
                    pos=u.Vector(-5,-70,15);target=u.Vector(34,-18,-10)
                    capture.set_actor_location_and_rotation(pos,u.MathLibrary.find_look_at_rotation(pos,target),False,True);cc.set_editor_property('fov_angle',42.)
