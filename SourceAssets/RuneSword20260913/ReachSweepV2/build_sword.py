...
base=Matrix.Translation((.13,.48,-.08))@Matrix.Rotation(math.radians(-18),4,'X')@Matrix.Rotation(math.radians(-5),4,'Z')
...
# Contact stays 0.23--0.46 s so existing saves, hit sampling and audio remain aligned.
ready=(.13,.48,-.08,0,-18,-5)
sweep_keys={
    'Slash1':[
        (0,*ready),
        (.10,.28,.43,-.04,-42,-48,-9),
...
(P/'authoring.json').write_text(json.dumps({'revision':'ReachSweepV2','ready_guard_cm':[13,48,-8],'ready_change_cm':[-2,12,5],'contact_yaw_degrees':160,'sweep_keys':sweep_keys,'clips':report,'grips_cm':{'right_below_guard':9,'left_below_guard':21.5},'source':str(source),'donor':'BigAndCrispy/Unity-First-Person-Melee CC0 (one-handed phases reconstructed as two-handed sweeps)','source_triangles':count,'game_triangles':len(canonical.polygons)},indent=2))
