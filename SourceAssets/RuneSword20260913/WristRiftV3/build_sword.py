...
base=Matrix.Translation((.13,.48,-.08))@Matrix.Rotation(math.radians(-18),4,'X')@Matrix.Rotation(math.radians(85),4,'Z')
grip_source={'l':HL,'r':HR}
grip_depth={'l':-.215,'r':-.09}
grip_angle={};ready_angles={};twist_history={}
localrest={n:(rest[r.data.bones[n].parent.name].inverted()@m if r.data.bones[n].parent else m) for n,m in rest.items()}
def solve_support(side,H):
    un,fn,hn=[x+'_'+side for x in ['upperarm','lowerarm','hand']]
    anchor=rest[un].translation.copy();T=H.translation
    l1=(rest[fn].translation-anchor).length;l2=(rest[hn].translation-rest[fn].translation).length
    old=(rest[hn].translation-rest[fn].translation).normalized()
    d=(H.to_quaternion()@rest[hn].to_quaternion().inverted())@old
    # Support the neutral wrist with the entire chain. Move the shoulder on the
    # fixed-length upper-arm sphere instead of stretching a nearly straight arm.
    ideal_elbow=T-d*l2
    preferred=anchor+Vector((0,.075,-.02))
    near=ideal_elbow+(preferred-ideal_elbow).normalized()*l1
    offset=near-anchor
    offset.x=max(-.13,min(.13,offset.x));offset.y=max(-.04,min(.23,offset.y));offset.z=max(-.10,min(.10,offset.z))
    A=anchor+offset
    axis=(T-A).normalized();dist=(T-A).length
    if dist>l1+l2-.015:A+=axis*(dist-(l1+l2-.015));dist=(T-A).length
    pole=ideal_elbow-A;pole-=axis*pole.dot(axis)
    preferred_pole=Vector((-.5 if side=='l' else .5,-.15,-1))
    preferred_pole-=axis*preferred_pole.dot(axis);preferred_pole.normalize()
    if pole.length<.0001:pole=preferred_pole
    else:pole= (pole.normalized()*.94+preferred_pole*.06).normalized()
    along=(l1*l1-l2*l2+dist*dist)/(2*dist);E=A+axis*along+pole*math.sqrt(max(0,l1*l1-along*along))
    bend=(T-E).normalized().angle(d)
    cost=bend*bend*.045+(A-preferred).length_squared*1.8
    cost+=max(0,E.z+.08)**2*3+max(0,E.y-.48)**2*2
    return cost,A,E,T

def hand_frame(side,swordframe,settle):
    z=grip_depth[side];source=grip_source[side]
    previous=grip_angle.get(side,math.radians(-25 if side=='l' else -155))
    def candidate(angle):
        H=swordframe@Matrix.Translation((0,0,z))@Matrix.Rotation(angle,4,'Z')@source
        cost,*_=solve_support(side,H)
        # Wrist position remains on the same grip; retain a continuous grasp roll
        # instead of sending all sword orientation change into the wrist joint.
        cost+=(angle-previous)**2*.003
        return cost,H
    best=min((previous+math.radians(i*5) for i in range(-36,37)),key=lambda a:candidate(a)[0])
    lo,hi=best-math.radians(5),best+math.radians(5)
    for _ in range(15):
        a=lo+(hi-lo)/3;b=hi-(hi-lo)/3
        if candidate(a)[0]<candidate(b)[0]:hi=b
        else:lo=a
    angle=(lo+hi)*.5
    if side in ready_angles:
        ready=ready_angles[side]
        while ready-angle>math.pi:ready-=2*math.pi
        while ready-angle<-math.pi:ready+=2*math.pi
        angle=angle*(1-settle)+ready*settle
    else:ready_angles[side]=angle
    grip_angle[side]=angle
    return candidate(angle)[1]

def arm(p,side,H):
    un,fn,hn=[x+'_'+side for x in ['upperarm','lowerarm','hand']]
    _,A,E,T=solve_support(side,H)
    ...
    while twist-previous>math.pi:twist-=2*math.pi
    while twist-previous<-math.pi:twist+=2*math.pi
    twist_history[side]=twist
    # Give the main forearm the wrist's axial orientation. Interpolate twist
    # helper bones by their actual position along the forearm, not a hard-coded
    # pair disconnected from the hand's parent bone.
    neutral_forearm=p[fn].copy()
    p[fn]=Matrix.LocRotScale(E,Quaternion(v,twist)@neutral_forearm.to_quaternion(),Vector((1,1,1)))
    for prefix,parent in [('upperarm',un),('lowerarm',fn)]:
        for idx,weight in [('01',.75),('02',.35)]:
            ...
            frame=neutral_forearm if prefix=='lowerarm' else p[parent]
            m=frame@rest[parent].inverted()@rest[n]
            if prefix=='lowerarm':
                weight=(rest[n].translation-rest[fn].translation).length/(rest[hn].translation-rest[fn].translation).length
                m=Matrix.LocRotScale(m.translation,Quaternion(v,twist*weight)@m.to_quaternion(),Vector((1,1,1)))
            p[n]=m
    p[hn]=H
    for n,m in (left_rel if side=='l' else right_rel).items():p[n]=H@m
def apply(swordframe,settle=0):
    p={n:m.copy() for n,m in rest.items()}
    for side in ['l','r']:arm(p,side,hand_frame(side,swordframe,settle))
...
ready=(.13,.48,-.08,0,-18,85)
sweep_keys={
    'Slash1':[
        (0,*ready),
        (.10,.28,.43,-.04,-42,-48,-9),
        (.18,.34,.40,.015,-100,-77,-12),
        (.23,.33,.44,-.015,-80,-84,-6),
        (.335,.025,.56,-.11,0,-90,0),
        (.46,-.33,.44,-.20,80,-95,9),
        (.55,-.37,.40,-.23,106,-102,14),
        (.67,-.16,.45,-.15,61,-62,3),
        (20/24,*ready)],
    'Slash2':[
        (0,*ready),
        (.10,-.18,.44,-.10,55,-65,8),
        (.18,-.34,.41,-.13,102,-104,12),
        (.23,-.33,.44,-.14,80,-100,9),
        (.34,0,.57,-.075,0,-88,0),
        (.46,.34,.44,-.03,-80,-82,-8),
        (.54,.38,.40,.005,-108,-73,-15),
        (.67,.26,.45,-.045,-54,-43,-8),
        (20/24,*ready)]}
...
for clip in ['Slash1','Slash2']:
    ...
    for i in range(segments+1):
        ...
        for j in range(across+1):
            v=j/across;verts.append(tuple(sf@Vector((0,0,.25+.83*v))));uvs.append((u,v))
...
for name,donor,duration in clips:
    ...
    s.frame_start=0;s.frame_end=round(duration*240);previous={};grip_angle=dict(ready_angles);twist_history.clear()
    for f in range(s.frame_end+1):
        ...
        settle=1 if name in ['Idle','Walk','Sprint'] or f==0 else max(0,min(1,(t/duration-.76)/.24))
...
(P/'authoring.json').write_text(json.dumps({'revision':'WristRiftV3','ready_guard_cm':[13,48,-8],'idle_blade_roll_delta_degrees':90,'fast_phase_seconds':[.23,.345],'fast_phase_speed_multiplier':2,'ready_grasp_degrees':{k:math.degrees(v) for k,v in ready_angles.items()},'contact_yaw_degrees':160,'sweep_keys_in_v2_source_time':sweep_keys,'clips':report,'grips_cm':{'right_below_guard':9,'left_below_guard':21.5},'source':str(source),'donor':'BigAndCrispy/Unity-First-Person-Melee CC0 (one-handed phases reconstructed as two-handed sweeps)','source_triangles':count,'game_triangles':len(canonical.polygons)},indent=2))
