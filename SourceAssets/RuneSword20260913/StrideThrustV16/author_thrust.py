...
source=ROOT/'ChargedHeavyV12/AzureRunesword_Manny_Editable.blend'
...
# The V12 editable source already carries all original clips at 480 Hz.
s.render.fps=FPS;s.render.fps_base=1
paired=[poses(f/FPS) for f in range(round(ATTACK_END*FPS)+1)]
frames=[p[0] for p in paired];hold=(math.ceil(LOAD_END*FPS),math.floor(CONTACT_START*FPS))
print('THRUST_SOLVING_GRASP',flush=True)
...
print('THRUST_SOLVING_BONES',flush=True)
r.animation_data.action=None;baked=[]
...
action=bpy.data.actions.new('A_RuneSword_Thrust');action.use_fake_user=True
r.animation_data.action=action;s.frame_start=0;s.frame_end=len(baked)-1;previous={}
for f,pose in enumerate(baked):
    s.frame_set(f)
    for b in r.pose.bones:
...
select(r)
bpy.ops.export_scene.fbx(filepath=str(OUT/'A_RuneSword_Thrust.fbx'),use_selection=True,object_types={'ARMATURE'},axis_forward='-Y',axis_up='Z',add_leaf_bones=False,bake_anim=True,bake_anim_use_all_actions=False,bake_anim_use_nla_strips=False,bake_anim_simplify_factor=0)
print('THRUST_ANIMATION_EXPORTED',flush=True)

# Two narrow optical ribbons follow the authored sword-tip path. U reveals
# time; V feathers the edges using the already installed refraction material.
verts=[];faces=[];uvs=[];segments=96;across=6
for ribbon in range(2):
    offset=len(verts)
    for i in range(segments+1):
        u=i/segments;_,sf=poses(CONTACT_START+(EXTENSION_END-CONTACT_START)*u)
        tip=sf@Vector((0,0,.80));axis=sf.to_3x3()@Vector((1,0,0) if ribbon==0 else (0,1,0))
        width=.04+.05*math.sin(math.pi*u)
        for j in range(across+1):
            v=j/across;verts.append(tuple(tip+axis*((v*2-1)*width)));uvs.append((u,v))
    for i in range(segments):
        for j in range(across):
            a=offset+i*(across+1)+j;faces.append((a,a+1,a+across+2,a+across+1))
mesh=bpy.data.meshes.new('RuneRift_Thrust');mesh.from_pydata(verts,[],faces);mesh.update()
ob=bpy.data.objects.new('SM_RuneRift_Thrust',mesh);s.collection.objects.link(ob)
uv=mesh.uv_layers.new(name='UVMap')
for loop in mesh.loops:uv.data[loop.index].uv=uvs[loop.vertex_index]
select(ob)
bpy.ops.export_scene.fbx(filepath=str(OUT/'SM_RuneRift_Thrust.fbx'),use_selection=True,object_types={'MESH'},axis_forward='-Y',axis_up='Z',bake_anim=False,mesh_smooth_type='FACE',use_tspace=True)
ob.hide_set(True);ob.hide_render=True
r.animation_data.action=action;r.animation_data.action_slot=action.slots[0]
s.frame_start=0;s.frame_end=round(ATTACK_END*FPS);s.frame_set(round(CONTACT_END*FPS))
bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(P/'AzureRunesword_Manny_Editable.blend'))
(P/'authoring.json').write_text(json.dumps({'revision':'StrideThrustV16','source':str(source),'donor':str(donor),'clip':'A_RuneSword_Thrust','seconds':ATTACK_END,'fps':FPS,'loop':False,'load_end':LOAD_END,'extension_end':EXTENSION_END,'contact_seconds':[CONTACT_START,CONTACT_END],'arrest_end':ARREST_END,'return_corner':RETURN_CORNER,'lunge_seconds':[LUNGE_START,LUNGE_END],'lunge_cm':LUNGE_CM,'load_guard_cm':[10,30,-12],'extended_guard_cm':[3,79,-7],'rift_max_width_cm':18,'retained':'Manny mesh, weights, bones, two-hand hilt contacts, V8 normal slashes and V12 charged heavy','arm_solution':'continuous grasp plus joint elbow-plane optimisation; fixed held load; quaternion-continuous bake'},indent=2),encoding='utf-8')
print('RUNESWORD_V16_AUTHORED',flush=True)
