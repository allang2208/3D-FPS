"""Import the added heavy clips and rift into the current sword asset folder."""
...
mesh=task('SM_RuneRift_Heavy',opt,FX);mesh.set_material(0,u.load_asset(FX+'/M_RuneRift'))
if not u.EditorAssetLibrary.save_loaded_asset(mesh):raise RuntimeError('Heavy rift save failed')
(P/'import_receipt.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
u.log('RUNESWORD_V12_IMPORT_COMPLETE')
