...
for name in ['S_RuneSword_Block','S_RuneSword_Parry']:
    t=u.AssetImportTask();t.filename=str(P/'Audio'/(name+'.wav'));t.destination_path=D;t.destination_name=name
    t.automated=True;t.replace_existing=True;t.save=False
    tools.import_asset_tasks([t]);asset=u.load_asset(D+'/'+name)
    if not asset or not t.imported_object_paths:raise RuntimeError('Audio import failed: '+name)
    if not u.EditorAssetLibrary.save_loaded_asset(asset):raise RuntimeError('Audio save failed: '+name)
    receipt.append({'source':t.filename,'asset':asset.get_path_name()})
(P/'import_receipt.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
u.log('RUNESWORD_V18_IMPORT_COMPLETE')