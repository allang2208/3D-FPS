...
try:
    ...
    readback=[]
    for clip,duration in [('Guard',.20),('GuardHit',106/480),('GuardBreak',.40)]:
    ...
    shots=[('guard_hold','Guard',.20,False),('guard_left_joint','Guard',.20,True),
           ('guard_raise','Guard',.10,False),('guard_hit','GuardHit',.044,False),
           ('guard_break','GuardBreak',.10,False),('guard_regrip','GuardBreak',.30,False)]
    def tick(dt):
        global rt
        try:
