...
face_flip=Matrix.Rotation(math.radians(50),4,'Z')
WINDUP_END=.30;CONTACT_START=.38;CONTACT_END=.495;ATTACK_END=20/24+.15
...
HL=face_flip.inverted()@HL;HR=face_flip.inverted()@HR
solver=ArmSolver(rest,{'l':HL,'r':HR},{'l':left_rel,'r':right_rel})
solver.ready_pose(base@face_flip)
...
# Keys retain V2 source time; the new clock adds anticipation and a held pose.
...
def source_time(t):
    if t<=WINDUP_END:
        u=max(0,t/WINDUP_END)
        # Ease to zero velocity before the 80 ms load hold.
        return .23*(u*u*(3-2*u))
    if t<=CONTACT_START:return .23
    if t<=CONTACT_END:return .23+(t-CONTACT_START)*2
    return .46+(t-CONTACT_END)*((20/24-.46)/(20/24-.345))
...
def sweep_pose(name,t):
    ...
    # Keep the load pose still, then drive both hands forward together. The
    # support solver preserves bone lengths and gives the shoulders room.
    reach=.06*smooth((authored_time-CONTACT_START)/.035)*(1-smooth((authored_time-CONTACT_END)/.20))
    return Matrix.Translation((x,y+reach,z))@Matrix.Rotation(math.radians(yaw),4,'Z')@Matrix.Rotation(math.radians(tilt),4,'X')@Matrix.Rotation(math.radians(roll),4,'Z')@face_flip
...
for clip in ['Slash1','Slash2']:
    ...
    for i in range(segments+1):
        u=i/segments;sf=sweep_pose(clip,CONTACT_START+(CONTACT_END-CONTACT_START)*u)
...
def pose_at(name,donor,t,duration):
    ...
    return sf if name in sweep_keys else sf@face_flip

clips=[('Idle','Sword_Idle',80/24),('Walk','Sword_Walk',22/24),('Slash1','Sword_Slash1',ATTACK_END),('Slash2','Sword_Slash2',ATTACK_END),('Equip',None,.55),('Sprint','Sword_Walk',22/24)]
s.render.fps=240;s.render.fps_base=1
apply(base@face_flip,solver.ready)
for name,donor,duration in clips:
    ...
    hold=(math.ceil(WINDUP_END*240),math.floor(CONTACT_START*240)) if name in sweep_keys else None
    grasp_path=solver.fit_path(frames,name in ['Idle','Walk','Sprint'],hold)
    ...
    report[name]={'duration':duration,'fps':240,'frames':s.frame_end+1,'loop':name in ['Idle','Walk','Sprint'],'reference':donor,'contact_window':[CONTACT_START,CONTACT_END] if name.startswith('Slash') else None}
...
(P/'authoring.json').write_text(json.dumps({'revision':'WeightLeftV5','geometry_scale_from_v3':.8,'blade_roll_from_v4_degrees':50,'idle_blade_roll_degrees':115,'grasp_counter_roll_degrees':-50,'ready_guard_cm':[13,48,-8],'windup_seconds':WINDUP_END,'hold_seconds':CONTACT_START-WINDUP_END,'fast_phase_seconds':[CONTACT_START,CONTACT_END],'fast_phase_speed_multiplier':2,'swing_forward_extension_cm':6,'ready_grasp_degrees':{k:math.degrees(v) for k,v in solver.ready.items()},'contact_yaw_degrees':160,'sweep_keys_in_v2_source_time':sweep_keys,'clips':report,'grips_cm':{'right_below_guard':7.2,'left_below_guard':17.2},'source':str(source),'donor':'BigAndCrispy/Unity-First-Person-Melee CC0 (one-handed phases reconstructed as two-handed sweeps)','source_triangles':count,'game_triangles':len(canonical.polygons)},indent=2))
