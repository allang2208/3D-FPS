"""Install the V45 hold-shift clips at the existing runtime paths.

V45 moves the whole charged hold along the left wrist-to-shoulder line so the
left elbow can bend (29.4 -> 65.7 deg at full charge). The live assets are the
V44 shoulder-carried revision, so they are backed up to BeforeV44/ first.
"""
import json
import shutil
...
for clip in ('HeavyCharge', 'HeavyRelease', 'Slash1'):
    ...
    replacement = P / 'BeforeV44' / (name + '.uasset')
    if current.exists() and not replacement.exists():
    ...
    task.filename = str(P / 'ExportV45' / (name + '.fbx'))
...
(P / 'import_receipt_v45.json').write_text(json.dumps(receipt, indent=2), encoding='utf-8')
u.log('RUNESWORD_V45_IMPORT_COMPLETE')
