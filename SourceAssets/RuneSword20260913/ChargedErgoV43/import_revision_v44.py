"""Install the V44 shoulder-carried clips at the existing runtime paths."""
import json
import shutil
...
for clip in ('HeavyCharge', 'HeavyRelease', 'Slash1'):
    ...
    replacement = P / 'BeforeV43' / (name + '.uasset')
    ...
    task.filename = str(P / 'ExportV44' / (name + '.fbx'))
...
(P / 'import_receipt_v44.json').write_text(json.dumps(receipt, indent=2), encoding='utf-8')
u.log('RUNESWORD_V44_IMPORT_COMPLETE')
