"""Render V44 (straight left elbow) against V45 (hold brought in, elbow bent)."""
import bpy
...
SHOTS = [('HeavyCharge', 1.60), ('HeavyCharge', 2.00), ('HeavyRelease', 0.60)]
...
def build(label, path):
    ...
    world = bpy.data.worlds.new('V45World' + label)
    scene.world = world
    ...
    data = bpy.data.cameras.new('V45Camera' + label)
...
for label, path in BLENDS.items():
    build(label, path)
print('COMPARE_V45_DONE', flush=True)
