...
source=P/(asset+'.glb') if (P/(asset+'.glb')).exists() else next(P.glob(asset+'_textured_master*.glb'))
prefix=asset
if 'clay' in sys.argv: prefix+='_clay'
...
if 'clay' in sys.argv:
    mat=bpy.data.materials.new('Geometry diagnostic');mat.use_nodes=True
    bs=mat.node_tree.nodes.get('Principled BSDF');bs.inputs['Base Color'].default_value=(.22,.24,.26,1);bs.inputs['Roughness'].default_value=.65
    for o in objects:
