"""Apply the six V4 locomotion clips; SprintOverhead is owned by V5."""
import json
...
receipt = json.loads(receipt_path.read_text()) if receipt_path.exists() else {'revision':'DashReadyStrideV4','saved':{},'runtime_tested':False}
patches = [(p,json.loads(p.read_text())) for variant in ('Standard','LongGrip') for p in sorted((P/variant).glob('*_keys.json')) if p.stem != 'SprintOverhead_keys']
...
for file,patch in patches:
    ...
    if L.does_asset_exist(target):
        ...
        previous_source = P.parent/'MeleeTacticalSprintClearView20260921'/file.parent.name/file.name
    ...
    controller.open_bracket('Dash-ready half-second tactical sprint V4',False)
    try:
