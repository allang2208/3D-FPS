...
helpers(S/'SVDReloadHandRepair20260923/author_arm.py',['repair'])
...
previous=json.loads((S/'SVDReloadHandRepair20260923/authoring.json').read_text())
...
def hand_targets(poses):
 ...
 for f,old in enumerate(poses):
  ...
  if 18<f<302:
   ...
   if f<=49:
    approach=held.copy();approach.translation+=M.to_3x3()@Vector((.025,-.012,0))*(1-ease((f-34)/15))
    ...
    H=held.copy();H.translation+=M.to_3x3()@Vector((.060,-.016,-.012))*ease((f-254)/14)
...
for family in ['base','vertical','canted','prism','angled']:
 source=S/'SVDReloadHandRepair20260923'/f'SVD_{family}_Editable.blend'
 ...
 for clip in ['reload','reload_empty']:
  ...
  poses,arm_rows=repair(poses,rest)
  a.name='REFERENCE_BEFORE_HAND_SHAPE_'+name
  ...
  report[key]={**info,'source':str(D/(name+'.fbx')),'blend':str(O/f'SVD_{family}_Editable.blend'),'previous_blend':str(source),
   'changed':'Grasp the front half only, thumb rests beside the front edge on the near broad face; no rear-edge reach; natural local thumb hinges, full-arm support and front-outward release; preserve joint translations, scale, mechanics and right-hand tracks',
   'grasp_fit':str(O/'grasp_fit.json'),'game_tested':False}
  (O/'authoring.json').write_text(json.dumps(report,indent=2));print('SVD_HAND_SHAPE_ANIMATION_AUTHORED',key,flush=True)
...
print('SVD_HAND_SHAPE_ANIMATIONS_COMPLETE',len(report),flush=True)
