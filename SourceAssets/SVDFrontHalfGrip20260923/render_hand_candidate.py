...
if draft:
 fit=json.loads((O/('grasp_balanced.json' if balanced else 'grasp_pulp.json' if pulp else 'grasp_contact.json' if contact else 'grasp_side.json' if side else 'grasp_candidate.json')).read_text());rest={b.name:b.matrix_local.copy() for b in r.data.bones}
...
left_groups={g.index for g in arms.vertex_groups if g.name.endswith('_l') and g.name.startswith(('lowerarm','hand','thumb','index','middle','ring','pinky'))}
ids=[v.index for v in arms.data.vertices if any(g.group in left_groups and g.weight>.01 for g in v.groups)]
