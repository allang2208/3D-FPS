...
record={'contact_patches':{k:v for k,v in json.loads((O/'fist_solution.json').read_text()).items() if k!='pose'},'runtime_run':run,'runtime_checks':log.count('FOREGRIP_AUDIT: PASS '),'runtime_failures':0,'ui_note':'UI and canted mesh unchanged; prior 24-check report retained in Integration','animation_variants':9,'geometry_samples':len(geo),'sampled_intersections':0,'source_contract':source,'import_clips':clips,'material_assignment':'Existing M4CantedForegrip mesh and M4 material unchanged','import_exit_code':int((O/'import_exit.txt').read_text(encoding='utf-8-sig').strip()),'import_note':'Import and readback passed; process exit 1 includes existing GameFeatureData configuration errors. New game process independently passed.','native_build':'Animation resource path updated; build suffix 2026095439','packaged_build_verified':False,'preview_note':'Actual game screenshots assembled in capture order using bounded capture-time intervals; silent visual preview, not audio verification.','preview_frames':len(entries),'fbx_sha256':hashes,'visual_review':'New game wrist, palm, front and first-person views reviewed; palm and thumb pulled inward and finger middle/distal joints curled while preserving accepted left cant and original arm mesh.'}
