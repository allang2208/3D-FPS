...
low=np.r_[low,[-22]*6];high=np.r_[high,[22]*6];seed=np.array(json.loads((O/'fist_solution.json').read_text())['parameters'])
best=None;rng=np.random.default_rng(9131)
for k in range(8):
 x=seed.copy() if k==0 else best.x+rng.normal(0,1.5,len(x0));x=np.clip(x,low+.001,high-.001);sol=least_squares(residual,x,bounds=(low,high),max_nfev=180,diff_step=.001)
