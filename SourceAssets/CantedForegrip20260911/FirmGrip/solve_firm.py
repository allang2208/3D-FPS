...
strength=np.array([1.3,1.8,1.2]*4+[4,2])
def distances(p):
 points=np.einsum('bij,bnj->ni',p[active],pre)@Gi.T;radius=np.interp(points[:,2],profile[:,0],profile[:,1]);sdf=np.linalg.norm(points[:,:2],axis=1)-radius
...
def residual(x):
 sdf=distances(pose(x));penetration=np.minimum(sdf-.00045,0)*16000;contact=(contacts(sdf)-.00055)*strength*2200
 return np.r_[penetration,contact,(x[6:18]-x0[6:18])*.015,x[:3]*.03,x[3:6]*.08,x[18:21]*.025,x[21:]*.06]
low=np.r_[[-12,-12,-5],[-7]*3,x0[6:18]-15,[-18]*3,[-5]*8];high=np.r_[[12,12,5],[7]*3,x0[6:18]+25,[18]*3,[5]*8]
best=None;rng=np.random.default_rng(9131)
for k in range(3):
 x=x0.copy() if k==0 else best.x+rng.normal(0,.8,len(x0));x=np.clip(x,low+.001,high-.001);sol=least_squares(residual,x,bounds=(low,high),max_nfev=100,diff_step=.001)
 if best is None or sol.cost<best.cost:best=sol
 print('FIRM',k,sol.cost,'contacts mm',contacts(distances(pose(best.x)))*1000,flush=True)
p=pose(best.x);(O/'fist_solution.json').write_text(json.dumps({'cost':float(best.cost*2),'parameters':best.x.tolist(),'before_patch_distance_mm':(contacts(distances(old))*1000).tolist(),'after_patch_distance_mm':(contacts(distances(p))*1000).tolist(),'pose':{n:p[i].tolist() for i,n in enumerate(names)}},indent=2))
