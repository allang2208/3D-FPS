...
void AFPSGAMECharacter::InitializePrismGripAnimations()
{
    ...
    {
        const FString Path=AKMSoviet::Matches(AKMViewmodel)?FString::Printf(TEXT("/Game/Weapons/AKMIntegration/SovietFab/Attachments/prism/A_AKM_prism_%s"),Pair.Value):FString::Printf(TEXT("/Game/Weapons/M4PrismGrip/A_M4_Prism_%s"),Pair.Value);
        const FString ResolvedPath=AKMSoviet::Matches(AKMViewmodel)?Path.Replace(TEXT("/Attachments/"),TEXT("/ArmSupportFinal/")):Path;
...
void AFPSGAMECharacter::SetGunsmithHandstop(const FString& Variant)
{
    ...
    if(!PrismHandstop)
    {
        auto* HandstopMesh=LoadObject<UStaticMesh>(nullptr,TEXT("/Game/Weapons/PrismHandstopV1/SM_PrismHandstop"));
