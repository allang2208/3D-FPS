...
void AFPSGAMECharacter::RunForegripAudit()
{
    ...
    const TCHAR* Variant=bVerticalAudit?TEXT("vertical_foregrip"):bPrismAudit?TEXT("prism_handstop"):TEXT("angled_foregrip");
    const TCHAR* Prefix=bVerticalAudit?TEXT("A_M4_Vertical_"):bAKM?(bPrismAudit?TEXT("A_AKM_prism_"):TEXT("A_AKM_angled_")):(bPrismAudit?TEXT("A_M4_Prism_"):TEXT("A_M4_Foregrip_"));
    const auto& GripAnimations=bVerticalAudit?VerticalGripAnimations:bPrismAudit?PrismGripAnimations:ForegripAnimations;
    auto HasGrip=[&](){return bVerticalAudit?HasVerticalForegrip():bPrismAudit?HasPrismHandstop():HasAngledForegrip();};
    ...
    {
        Check(HasGrip()&&((int32)HasAngledForegrip()+(int32)HasPrismHandstop()+(int32)HasVerticalForegrip()==1),TEXT("grip installed exclusively"));
        Check(GunplayAnimation->IdleClip==GripAnimations.FindRef(IdleAnimation),TEXT("runtime idle uses selected grasp"));
        if(!bAKM&&!bPrismAudit&&!bVerticalAudit&&HasAngledForegrip())
        {
    ...
    {
        Check(!HasAngledForegrip()&&!HasPrismHandstop()&&!HasVerticalForegrip()&&GunplayAnimation->IdleClip==IdleAnimation,TEXT("removal restores original mesh and animation"));
    ...
    if(ForegripAuditStage==16)
    {
        ...
        {
            const FVector Focus=(bVerticalAudit?VerticalForegrip:bPrismAudit?PrismHandstop:AngledForegrip)->Bounds.Origin;
    ...
    if(ForegripAuditStage==21){FScreenshotRequest::RequestScreenshot(Out/TEXT("wrist_review.png"),false,false);ForegripAuditStage=22;ForegripAuditNextTime=Now+.5f;return;}
