...
void AFPSGAMECharacter::InitializeWeaponVisuals()
{
    ...
    for(auto* Part:{LPVORing.Get(),HolographicOptic.Get(),LargeDrum.Get(),MuzzleAttachment.Get(),PrismHandstop.Get(),AngledForegrip.Get()})if(Part)Part->DestroyComponent();
    LPVORing=nullptr;LPVOMagnification=1.f;HolographicOptic=nullptr;LargeDrum=nullptr;MuzzleAttachment=nullptr;PrismHandstop=nullptr;AngledForegrip=nullptr;
    ...
    USkeletalMesh* ViewmodelMesh = LoadObject<USkeletalMesh>(nullptr, TEXT("/Game/Weapons/AKMIntegration/SovietFab/Attachments/SK_AKM_MannyNative.SK_AKM_MannyNative"), nullptr, LOAD_NoWarn);
...
void AFPSGAMECharacter::Tick(float DeltaSeconds)
{
    ...
    if (FParse::Param(FCommandLine::Get(), TEXT("ForegripAudit")) || FParse::Param(FCommandLine::Get(), TEXT("PrismGripAudit")) || FParse::Param(FCommandLine::Get(), TEXT("VerticalGripAudit"))) RunForegripAudit();
...
void AFPSGAMECharacter::FireShot()
{
    ...
    const FVector Muzzle = GetEffectiveMuzzleLocation(); // Same visible pose as the aim ray, before restarting animation.
    ...
    if (bHit && Hit.GetActor())
    {
        ...
        if(!bLastShotMuzzleBlocked&&ProjectileSpeedCM<=0){
            ...
            while(Remaining-->0&&Cast<APawn>(NextHit.GetActor())){
                ...
                if(NextHit.GetActor())NotifyConfirmedWeaponHit(NextHit.GetActor(),UGameplayStatics::ApplyPointDamage(NextHit.GetActor(),DamagePerShot,TraceDirection,NextHit,Controller,this,nullptr));
...
void AFPSGAMECharacter::UpdateADSPose()
{
    ...
    if (bHolographicOptic)
    {
        ...
        Front=Rear+Mount.GetRotation().RotateVector(AKMSoviet::Matches(AKMViewmodel)?FVector::RightVector:FVector::ForwardVector)*10.f;
...
float AFPSGAMECharacter::ReloadSourceTime(float RuntimeTime) const
{
    if (bUsingM4Infima)
    {
        ...
        if(bDrumInstalled)return M4DrumReloadTiming::SourceSeconds(Phase,bPendingEmptyReload);
