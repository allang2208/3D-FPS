...
assert 'CANTED_SOURCE_VALIDATION_PASS' in (O/'validate_source_final.log').read_text(encoding='utf-8-sig')
...
r=subprocess.run([str(ff),'-y','-v','error','-safe','0','-f','concat','-i',str(concat),'-r','30','-c:v','libx264','-crf','19','-pix_fmt','yuv420p',str(O/'M4_CantedForegrip_Gameplay.mp4')],capture_output=True);assert r.returncode==0,r.stderr
hashes={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in O.glob('*.fbx')}
assert 'CANTED_UI_AUDIT COMPLETE checks=24 failures=0' in (O/'ui_final.log').read_text(encoding='utf-8-sig')
assert 'Result: Succeeded' in (O/'native_build.log').read_text(encoding='utf-8-sig')
record={'runtime_run':run,'runtime_checks':log.count('FOREGRIP_AUDIT: PASS '),'runtime_failures':0,'ui_checks':24,'ui_failures':0,'animation_variants':9,'geometry_samples':len(geo),'sampled_intersections':0,'source_contract':source,'import_clips':clips,'model':json.loads((O/'model_report.json').read_text()),'mesh_arm':json.loads((O/'mesh_arm_report.json').read_text()),'material_assignment':json.loads((O/'static_import_report.json').read_text()),'import_exit_code':int((O/'import_exit.txt').read_text(encoding='utf-8-sig').strip()),'native_build':'2026095345 succeeded','packaged_build_verified':False,'preview_note':'Silent preview assembled from actual game captures; capture intervals bounded to 16-250ms, no audio validation claimed.','preview_frames':len(entries),'fbx_sha256':hashes,'direction':'Shooter left/down 45 degrees, +Y is left in author mount; arm solved without skeleton mirroring'}
