...
source=source.replace('A_M4_Foregrip_','A_M4_Canted_').replace('FOREGRIP_','CANTED_')
source=source.replace('M4_Foregrip_Fitted.blend','M4_Canted_Fitted.blend').replace("'FG_'","'CG_'")
source=source.replace("if f<end/2:opening=smooth(f/4);retreat=smooth((f-4)/5)\n    else:opening=1-smooth((f-(end-6))/6);retreat=1-smooth((f-(end-16))/10)", "if f<end/2:u=max(0,min(1,f/9))\n    else:u=1-max(0,min(1,(f-(end-12))/12))\n    retreat=smooth(u)*(.8+.2*smooth((u-.20)/.2444444444));opening=smooth(u*fit.get('release_open_speed',1))")
...
"""+source[end:]
source=source.replace("   A=old[un].translation.copy();target=H.translation;", "   support_offset=Vector((.06,.12,0))*w\n   p['clavicle_l'].translation+=support_offset\n   A=old[un].translation+support_offset;target=H.translation;")
