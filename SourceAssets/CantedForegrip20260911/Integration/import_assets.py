...
O=Path(__file__).parent;DEST='/Game/Weapons/M4CantedForegrip'
...
task=unreal.AssetImportTask();task.filename=str(O/'SM_CantedForegrip.fbx');task.destination_path=DEST;task.destination_name='SM_CantedForegrip';task.automated=True;task.replace_existing=True;task.save=True;task.options=options
unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task]);sm=unreal.load_asset(DEST+'/SM_CantedForegrip');assert sm
...
(O/"static_import_report.json").write_text(json.dumps(report,indent=2))
report={}
references={
