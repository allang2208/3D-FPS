...
root=(P/'remote_root.txt').read_text(encoding='utf-8');remote=root+'\\output\\CantedForegrip20260911'
...
for f in files:
 dst=P/f['Name']
 if not dst.exists():urllib.request.urlretrieve(B+'/view?'+urllib.parse.urlencode({'filename':f['Name'],'subfolder':'CantedForegrip20260911','type':'output'}),dst)
