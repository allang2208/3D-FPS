...
seed=np.array(json.loads((O/'fist_solution.json').read_text())['parameters']);free=np.array([18,19,20,29,30,31,32,33,34]);initial_pose=pose(seed)
def objective(y):
 x=seed.copy();x[free]=y;sdf=distances(pose(x));c=contacts(sdf)
 return np.r_[np.minimum(sdf-.0008,0)*24000,(c[-1]-.00065)*50000,(y-seed[free])*.1,(y[5]-seed[31]-5)*8,(y[8]-seed[34]-7)*8]
sol=least_squares(objective,seed[free],bounds=(seed[free]-8,seed[free]+8),max_nfev=300,diff_step=.0001,ftol=None,xtol=1e-9,gtol=1e-8)
x=seed.copy();x[free]=sol.x;p=pose(x)
(O/'fist_solution.json').write_text(json.dumps({'cost':float(sol.cost*2),'parameters':x.tolist(),'thumb_change_degrees':(x[free]-seed[free]).tolist(),'before_patch_distance_mm':(contacts(distances(initial_pose))*1000).tolist(),'after_patch_distance_mm':(contacts(distances(p))*1000).tolist(),'pose':{n:p[i].tolist() for i,n in enumerate(names)}},indent=2))
print('THUMB_SOLUTION',x[free]-seed[free],contacts(distances(p))[-1]*1000,flush=True)
