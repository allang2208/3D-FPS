...
# Whole palm starts at the current SVD contact, with the accepted M4 thumb shape.
...
def semantic(v):
 ...
 for n,ids in pads.items():
  ...
  out.extend([1000*(c[1]-(front+.006) if n.startswith('thumb') else middle-c[1]),(c[2]+.038)*1000,(.048-c[2])*1000])
 return np.array(out)
...
def objective(x):
 ...
 for n,ids in pads.items():
  ...
  if n.startswith('thumb'):
   right=np.interp(c[2],shell.z,shell.support[:,0])
   value+=8*((c[0]-right)*1000)**2+1.5*((c[1]-(front+rear)*.5)*1000)**2+1.0*((c[2]-.018)*1000)**2
  elif '_02_' in n:
   value+=2*((c[1]-front)*1000)**2
  else:
...
bounds += [(-40,40)]*3+[(-12,40),(-10,35)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[18,45,.7,0]
x[25:]=[10,10]
if (O/'side_working.json').exists():x=np.array(json.loads((O/'side_working.json').read_text())['parameters'])
...
x=coarse.x
(O/'side_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'method':'Current evaluated SVD; local X finger length, local Y abduction, local Z flexion; accepted M4 natural thumb adapted to SVD broad-side pinch; middle finger pads on front edge, distal finger pads on opposite face, thumb on palm-side face; bounded 0.55 mm glove compression',
 'game_tested':False}
(O/'grasp_side.json').write_text(json.dumps(out,indent=2))
(O/'side_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
