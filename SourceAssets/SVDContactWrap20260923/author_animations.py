...
previous=json.loads((S/'SVDNaturalGrasp20260923/authoring.json').read_text())
...
for family in ['base','vertical','canted','prism','angled']:
 source=S/'SVDNaturalGrasp20260923'/f'SVD_{family}_Editable.blend'
 ...
 for clip in ['reload','reload_empty']:
  ...
  a.name='REFERENCE_BEFORE_CONTACT_WRAP_'+name
  ...
  report[key]={**info,'source':str(D/(name+'.fbx')),'blend':str(O/f'SVD_{family}_Editable.blend'),'previous_blend':str(source),
   'changed':'Rebuild wrap with actual finger joint axes, central palmar pad regions and coupled palm/thumb/finger contact; preserve original mechanical clocks',
   'grasp_fit':str(O/'grasp_fit.json'),'game_tested':False}
  (O/'authoring.json').write_text(json.dumps(report,indent=2));print('SVD_CONTACT_ANIMATION_AUTHORED',key,flush=True)
...
print('SVD_CONTACT_ANIMATIONS_COMPLETE',len(report),flush=True)
