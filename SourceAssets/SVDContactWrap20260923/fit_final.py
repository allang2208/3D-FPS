...
for j in range(4):bounds += [(20,78),(12,72),(.25,.85),(-14,14)]
bounds += [(-65,65)]*3+[(10,75),(0,35)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[50,50,.65,0]
x[25:]=[40,22]
if (O/'final_working.json').exists():x=np.array(json.loads((O/'final_working.json').read_text())['parameters'])
...
coarse=minimize(penalty,x,method='L-BFGS-B',bounds=bounds,options={'maxiter':60,'maxfun':2500,'ftol':1e-7})
x=coarse.x
(O/'final_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
...
def constraints(x):
 ...
 return np.r_[shell.clearance(sampled_surface(v,True))+.55,semantic(v),touch,3.3-dist[palm].mean()]
solution=minimize(objective,x,method='SLSQP',bounds=bounds,constraints=[{'type':'ineq','fun':constraints}],options={'maxiter':240,'ftol':.01})
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'game_tested':False}
(O/'grasp_final.json').write_text(json.dumps(out,indent=2))
(O/'final_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
