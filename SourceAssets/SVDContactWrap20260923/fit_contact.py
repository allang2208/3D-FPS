...
def objective(x):
 ...
 for n,ids in pads.items():
  ...
  if '_02_' in n:
   value+=1.5*((c[1]-(rear if n.startswith('thumb') else front))*1000)**2
  else:
   value+=5*((c[0]-left)*1000)**2
   desired_y=rear-.009 if n.startswith('thumb') else front+.008
   value+=1.2*((c[1]-desired_y)*1000)**2
...
bounds += [(-45,45)]*3+[(20,68),(8,50)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[50,50,.65,0]
x[25:]=[40,22]
if (O/'contact_working.json').exists():x=np.array(json.loads((O/'contact_working.json').read_text())['parameters'])
...
x=coarse.x
(O/'contact_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'method':'Current evaluated SVD; local X finger length, local Y abduction, local Z flexion; mid-phalanx pads wrap front/rear edges while distal pads clasp the opposite broad face; bounded 0.55 mm glove surface compression',
 'game_tested':False}
(O/'grasp_contact.json').write_text(json.dumps(out,indent=2))
(O/'contact_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
