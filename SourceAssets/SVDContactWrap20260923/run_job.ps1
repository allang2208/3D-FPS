...
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/SVDContactWrap20260923'
...
try {
 ...
 if($running.Count){$bridge=$true}
  ...
  if($Script -eq 'end_play.py'){Write-Output 'SVD_CONTACT_NO_EDITOR_RUNNING';return}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(SVD_CONTACT_|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
