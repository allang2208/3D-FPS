...
H0=np.array(accepted_input['hand_in_mag']);Q0={n:R.from_quat([q[1],q[2],q[3],q[0]]).as_matrix() for n,q in accepted_input['finger_basis'].items()}
digits=['index','middle','ring','pinky']
pads={}
for n in prior['pad_regions']:
 ids=np.flatnonzero(labels==n);points=bound[n][ids,:3]
 length=np.percentile(points[:,0],95)
 use=(points[:,0]>length*.25)&(points[:,0]<length*.85)&(np.abs(points[:,2])<.0065)
 ids=ids[use];points=points[use]
 # Local +X is longitudinal; local +Y is the palmar surface.
 pads[n]=ids[np.argsort(points[:,1])[-max(12,len(ids)//3):]]
...
hand_local=bound['hand_l'][:,:3]
palm=np.flatnonzero((labels=='hand_l')&(hand_local[:,0]>.025)&(hand_local[:,0]<.070)&(np.abs(hand_local[:,2])<.020))
palm=palm[np.argsort(hand_local[palm,1])[-max(20,len(palm)//3):]]
...
def posed(x):
 ...
 Q['thumb_02_l']=rz(x[25]);Q['thumb_03_l']=rz(x[26])
...
def objective(x):
 _,_,posed_bones,v=posed(x);dist=shell.clearance(v);value=0.
 for n,ids in pads.items():
  ...
  value+=(.35 if n.startswith('thumb') else .10)*((c[1]-(rear if n.startswith('thumb') else front))*1000)**2
  support=np.array([np.interp(c[2],shell.z,shell.support[:,j]) for j in range(48)])
  signed=c[:2]@shell.N.T-support;weights_n=np.exp((signed-signed.max())/.002)
  normal=weights_n@shell.N;normal/=np.linalg.norm(normal)
  inward=posed_bones[n][:3,1];value+=25*(1+np.dot(inward[:2],normal))**2
 value+=8*np.mean((dist[palm]-.35)**2)
 value+=.012*np.dot(x[:3],x[:3])+.018*np.dot(x[3:6],x[3:6])+.025*np.dot(x[22:25],x[22:25])
 for j in range(4):
  mcp,pip,ratio,spread=x[6+4*j:10+4*j]
  value+=.018*(mcp-42)**2+.015*(pip-55)**2+.1*spread**2+20*(ratio-.65)**2
 return float(value)

bounds=[(-28,16),(-30,24),(-24,20)]+[(-30,30)]*3
for j in range(4):bounds += [(20,78),(20,72),(.3,.85),(-14,14)]
bounds += [(-65,65)]*3+[(10,75),(0,42)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[50,50,.65,0]
x[25:]=[40,22]
if (O/'selected_working.json').exists():x=np.array(json.loads((O/'selected_working.json').read_text())['parameters'])
...
coarse=minimize(penalty,x,method='L-BFGS-B',bounds=bounds,options={'maxiter':1,'maxfun':100,'ftol':1e-7})
x=coarse.x
(O/'selected_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
print('SVD_CONTACT_COARSE',round(coarse.fun,3),str(coarse.message),flush=True)

def constraints(x):
 v=posed(x)[3];dist=shell.clearance(v)
 touch=np.array([(3.0 if n.startswith('thumb') and '_02_' in n else 2.0 if n.startswith('thumb') else 4.0 if n=='index_02_l' else 3.5 if '_02_' in n else 1.8)-dist[ids].mean() for n,ids in pads.items()])
 return np.r_[shell.clearance(sampled_surface(v,True))+.55,semantic(v),4.0-dist[palm].mean()]
solution=minimize(objective,x,method='SLSQP',bounds=bounds,constraints=[{'type':'ineq','fun':constraints}],options={'maxiter':180,'ftol':.01})
...
opened['thumb_02_l']=rz(max(12,solution.x[25]-20));opened['thumb_03_l']=rz(max(6,solution.x[26]-12))
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'pad_regions':{n:ids.tolist() for n,ids in pads.items()},'palm_region':palm.tolist(),'pad_means_mm':{n:float(shell.clearance(v[ids]).mean()) for n,ids in pads.items()},'palm_mean_mm':float(shell.clearance(v[palm]).mean()),
 ...
 'method':'Current evaluated SVD; local X finger length, local Y abduction, local Z flexion; actual central palmar pads instead of closest donor side-edge vertices; opposing surface normals and palm contact; bounded 0.55 mm glove surface compression',
 'game_tested':False}
(O/'grasp_selected.json').write_text(json.dumps(out,indent=2))
(O/'selected_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
