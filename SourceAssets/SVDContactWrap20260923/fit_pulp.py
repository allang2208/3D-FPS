...
def objective(x):
 ...
 for n,ids in pads.items():
  ...
  inward=posed_bones[n][:3,1];value+=80*(1+np.dot(inward[:2],normal))**2
...
x[25:]=[40,22]
if (O/'pulp_working.json').exists():x=np.array(json.loads((O/'pulp_working.json').read_text())['parameters'])
...
x=coarse.x
(O/'pulp_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'game_tested':False}
(O/'grasp_pulp.json').write_text(json.dumps(out,indent=2))
(O/'pulp_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
