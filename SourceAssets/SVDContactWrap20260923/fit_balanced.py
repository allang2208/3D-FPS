...
for j in range(4):bounds += [(20,78),(32,72),(.55,.85),(-14,14)]
bounds += [(-65,65)]*3+[(10,68),(5,48)]
x=np.zeros(27)
for j in range(4):x[6+4*j:10+4*j]=[50,50,.65,0]
x[25:]=[40,22]
if (O/'balanced_working.json').exists():x=np.array(json.loads((O/'balanced_working.json').read_text())['parameters'])
...
x=coarse.x
(O/'balanced_working.json').write_text(json.dumps({'parameters':x.tolist(),'stage':'coarse'}))
print('SVD_CONTACT_COARSE',round(coarse.fun,3),str(coarse.message),flush=True)

def constraints(x):
 v=posed(x)[3];dist=shell.clearance(v)
 touch=np.array([(4.0 if n.startswith('thumb') and '_02_' in n else 2.0 if n.startswith('thumb') else 4.8 if n=='index_02_l' else 3.5 if '_02_' in n else 1.8)-dist[ids].mean() for n,ids in pads.items()])
...
out={'hand_in_mag':H.tolist(),'finger_basis':{n:wxyz(q) for n,q in Q.items()},'open_basis':{n:wxyz(q) for n,q in opened.items()},
 ...
 'game_tested':False}
(O/'grasp_balanced.json').write_text(json.dumps(out,indent=2))
(O/'balanced_working.json').write_text(json.dumps({'parameters':solution.x.tolist(),'stage':'full_surface'}))
