...
O=Path(__file__).parent;bpy.ops.wm.open_mainfile(filepath=str(O.parent/'Revision4/M4_DrumDrop_Editable.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene;rest=r.data.bones
def smooth(t):
...
for clip,end,release,pickup,lock in [('reload',126,18,42,50),('reload_empty',162,14,29,35)]:
 a=bpy.data.actions['A_M4_DrumDrop_'+clip];r.animation_data.action=a;r.animation_data.action_slot=a.slots[0];raw=[]
 ...
 r.animation_data.action=None;reference=raw;W0=raw[0]['P']['WPN_root'];home=raw[0]['P']['hand_l'];hold=release+4
 ...
 def weapon(t,old):
  ...
  # An eight-degree lift keeps the initial grip. The larger reload cant follows only after release.
  initial=Quaternion(Vector((1,0,0)),math.radians(8*early))
  ...
  pos=W0.translation+Vector((0,0,.022*early))*finish
  pos+=((old.translation-W0.translation)*.40-Vector((0,0,.022)))*later*finish
 ...
 for phase in [0,1]:
  for k,src in enumerate(raw):
   ...
   if t<lock:
    ...
    for b in r.pose.bones:
     ...
     if n.endswith('_l') and n.startswith(('thumb','index','middle','ring','pinky')):
      ...
      b.matrix_basis=track([(0,B),(hold,B),(hold+5,openB),(pickup,openB),(lock,C)],t)
    update()
   ...
   if t<=release:r.pose.bones['WPN_SOCKET_Magazine'].matrix=gun[k]@W0.inverted()@reference[0]['P']['WPN_SOCKET_Magazine'];update()
   row={'frame':t}
   for side in ['l','r']:
 ...
 a.name='BeforeFlow_'+clip;a=bpy.data.actions.new('A_M4_DrumFlow_'+clip);a.use_fake_user=True;r.animation_data.action=a;prev={}
 ...
 report[clip]={'release_frame':release,'support_release_frame':hold,'pickup_frame':pickup,'grip_frame':lock,'rows':rows};r.animation_data.action=None
r.animation_data.action=bpy.data.actions['A_M4_DrumFlow_reload'];r.animation_data.action_slot=r.animation_data.action.slots[0];s.frame_start=0;s.frame_end=126;s.frame_set(0);bpy.ops.wm.save_as_mainfile(filepath=str(O/'M4_DrumFlow_Editable.blend'));(O/'motion_report.json').write_text(json.dumps(report,indent=2))
