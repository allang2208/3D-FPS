...
O=Path(__file__).parent;bpy.ops.wm.open_mainfile(filepath=str(O/'M4_DrumFlow_Editable.blend'));s=bpy.context.scene;r=bpy.data.objects['SK_M4_Infima']
...
for clip,frame in [('reload',15),('reload',76),('reload',106),('reload_empty',38),('reload_empty',126)]:
 for label,prefix in [('before','BeforeFlow_'),('after','A_M4_DrumFlow_')]:
