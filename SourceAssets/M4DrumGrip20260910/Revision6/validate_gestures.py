...
for clip,end,hold,lock in [('reload',126,22,50),('reload_empty',162,18,35)]:
 base=sample('BeforeThrow_'+clip,0);LH=base['WPN_root'].inverted()@base['hand_l'];RH=base['WPN_root'].inverted()@base['hand_r'];left=right=rot=contact=arm=armrot=0;steps=[];previous=None
 for k in range(end*2+1):
  t=k/2;pose=sample('A_M4_DrumThrow_'+clip,t);W=pose['WPN_root'];h=W.inverted()@pose['hand_l'];hr=W.inverted()@pose['hand_r'];right=max(right,(hr.translation-RH.translation).length*100)
