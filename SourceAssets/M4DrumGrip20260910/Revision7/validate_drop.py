...
O=Path(__file__).parent;bpy.ops.wm.open_mainfile(filepath=str(O/'M4_DrumMatch_Editable.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene;hm=bpy.data.objects['SK_Manny_Arms_Export'];rest=r.data.bones
...
for clip,end,release,grip in [('reload',126,18,36),('reload_empty',148,14,31)]:
 a=bpy.data.actions['A_M4_DrumMatch_'+clip];r.animation_data.action=a;r.animation_data.action_slot=a.slots[0];hinge=0;stretch=0;depth=0;steps=0;prev={};rows=[];fast=[]
