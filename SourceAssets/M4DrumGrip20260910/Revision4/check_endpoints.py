...
O=Path(__file__).parent;bpy.ops.wm.open_mainfile(filepath=str(O/'M4_DrumDrop_Editable.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene
for clip,end in [('reload',126),('reload_empty',162)]:
 for t in [0,end]:
  poses=[]
  for prefix in ['BeforeDrop_','A_M4_DrumDrop_']:
