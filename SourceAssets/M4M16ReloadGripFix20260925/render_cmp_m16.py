"""Normalised M16 before/after comparison in the magazine's own rest frame."""
import bpy, sys
...
def draw(tag, path, action, f):
    ...
    Di = D.inverted()
    rest = [Di @ (mag.evaluated_get(dg).matrix_world @ v.co) for v in mag.evaluated_get(dg).to_mesh().vertices] \
        if False else [mag.matrix_world @ v.co for v in mag.data.vertices]
    c, ax = G.pca(rest)
    ...
    e.to_mesh_clear()
    arm = bpy.data.meshes.new('Arm' + tag); arm.from_pydata(to_frame([Di @ p for p in verts], c, ax), [], polys); arm.update()
    ...
    for nm, i, sgn in [('T', 2, -1), ('T', 2, 1), ('W', 1, -1), ('W', 1, 1)]:
        cam.location = ax[i] * sgn * .4
        cam.rotation_euler = (Vector((0, 0, 0)) - cam.location).to_track_quat('-Z', 'Y').to_euler()
        sc.render.filepath = str(O / f'm16cmp_{tag}_{nm}{"p" if sgn > 0 else "n"}.png')
        bpy.ops.render.render(write_still=True)


draw('after', O / 'm16_cache/base/A_M16_reload.blend', 'A_M16_reload_GripPrecise', 76)
draw('before', S / 'M16RemovalMelee20260920/Animations/base/A_M16_reload.blend', None, 76)
print('M16CMP_OK', flush=True)
