...
fit=json.loads((O/'wrap_fit.json').read_text());fit['contact_refinement']=report;fit['bone_local_rotations']={b.name:list(b.rotation_quaternion) for b in r.pose.bones if b.name.endswith('_l') and b.name.startswith(('index','middle','ring','pinky','thumb'))};(O/'wrap_fit.json').write_text(json.dumps(fit,indent=2));a=bpy.data.actions.new('M4_WRAP_contact');r.animation_data.action=a
