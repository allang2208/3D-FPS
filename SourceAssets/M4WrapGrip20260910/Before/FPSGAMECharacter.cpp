...
void AFPSGAMECharacter::InitializeWeaponVisuals()
{
    ...
    DrumReloadAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumDrop/Final/A_M4_DrumDrop_reload.A_M4_DrumDrop_reload"));
    DrumReloadEmptyAnimation=LoadObject<UAnimSequence>(nullptr,TEXT("/Game/Weapons/M4DrumDrop/Final/A_M4_DrumDrop_reload_empty.A_M4_DrumDrop_reload_empty"));
...
void AFPSGAMECharacter::UpdateViewmodel(float DeltaSeconds)
{
    ...
    const FVector HipFraming = bUsingM4Infima
        ? FMath::Lerp(HipViewmodelLocation, M4ActionViewmodelLocation + (bDrumInstalled ? FVector(4.0f, 0.0f, 0.0f) : FVector::ZeroVector), M4ActionFramingAlpha)
    ...
    if (bUsingM4Infima)
    {
        ...
        if (Clip)
        {
            const FString Path = FString::Printf(TEXT("/Game/Weapons/%s/A_M4_HK416_%s.A_M4_HK416_%s"),
                TEXT("M4AnimationAuditFinal"), Clip, Clip);
