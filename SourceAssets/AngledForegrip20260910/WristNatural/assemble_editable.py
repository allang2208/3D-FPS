...
bpy.ops.wm.open_mainfile(filepath=str(O/'A_M4_Foregrip_idle.blend'));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene
for clip in json.loads((O/'animation_build.json').read_text()):
 if clip=='idle':continue
 name='A_M4_Foregrip_'+clip
 with bpy.data.libraries.load(str(O/(name+'.blend')),link=False) as (src,dst):dst.actions=[name]
 dst.actions[0].use_fake_user=True
a=bpy.data.actions['A_M4_Foregrip_idle'];r.animation_data.action=a;r.animation_data.action_slot=a.slots[0];s.frame_set(0)
for ob in s.objects:ob.hide_render=not(ob.name.startswith('FG_') or (ob.type=='MESH' and ob.parent==r))
...
bpy.ops.wm.save_as_mainfile(filepath=str(O/'M4_AngledForegrip_Integrated_Editable.blend'))
