...
imports=json.loads((O/'import_report.json').read_text());assert all('/M4ForegripWristNatural/' in imports[n]['asset'] for n in ['idle','aim','fire','aim_fire','equip','reload','reload_empty','drum_reload','drum_reload_empty'])
...
assert (frames/'wrist_review.png').exists()
...
lines.extend(["file '"+(frames/'wrist_review.png').as_posix()+"'",'duration 2',"file '"+(frames/'wrist_review.png').as_posix()+"'"])
