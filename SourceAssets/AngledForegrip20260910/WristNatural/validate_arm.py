...
for clip,info in json.loads((O/'animation_build.json').read_text()).items():
 name='A_M4_Foregrip_'+clip;bpy.ops.wm.open_mainfile(filepath=str(O/(name+'.blend')));r=bpy.data.objects['SK_M4_Infima'];s=bpy.context.scene;new=bpy.data.actions[name]
 ...
 def sample(a,f):
  ...
  return {n:r.pose.bones[n].matrix.copy() for n in points},math.degrees(v.angle(desired)),math.degrees((l.head-u.head).normalized().angle(v))
 delta=0;angles={}
 for frame in range(info['frames']+1):
  ...
  if frame in [0,info['frames']]:angles[str(frame)]={'old_wrist_axis_bend_deg':oldw,'new_wrist_axis_bend_deg':neww,'old_elbow_bend_deg':olde,'new_elbow_bend_deg':newe}
 assert delta<.0001,(clip,delta)
 report[clip]={'max_hand_finger_position_delta_m':delta,'angles':angles};(O/'arm_validation.json').write_text(json.dumps(report,indent=2))
