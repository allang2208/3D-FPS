...
imports=json.loads((O/'import_report.json').read_text());assert all('/M4AngledForegripFinal/' in imports[n]['asset'] for n in ['idle','aim','fire','aim_fire','equip','reload','reload_empty','drum_reload','drum_reload_empty'])
geometry=json.loads((O/'geometry_contact_full.json').read_text());assert len(geometry)==446 and all(v['crossing_hand_triangles']==0 for v in geometry.values())
...
report={'run':run,'runtime_passes':log.count('FOREGRIP_AUDIT: PASS'),'runtime_failures':0,'modules':sorted(set(module)),'geometry_samples':len(geometry),'finger_grip_intersections_in_sampled_poses':0,'source_validation':sources,'import_validation':imports,'game_frames':count,'preview':'M4_foregrip_gameplay.mp4','preview_note':'Actual runtime screenshots; variable frame holds follow capture file timestamps, clamped to 1/60..0.25 seconds. Silent visual review, not audio synchronization evidence.','editable':'M4_AngledForegrip_Integrated_Editable.blend','grasp_parameters':json.loads((O/'grasp_refinement.json').read_text()),'files_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [O/'SM_M4_AngledForegrip.fbx',O/'M4_AngledForegrip_Integrated_Editable.blend']}}
