...
specs=[
 ...
 ('reload','M4TacticalToss20260910','M4_MAT_reload',126,60),
...
for clip,folder,action,end,hz in specs:
 ...
 for k,old in enumerate(samples):
  ...
  if w>0:
   ...
   opening=0
   if 'reload' in clip:
    if f<end/2:opening=.5*smooth(f/4)+.5*smooth((f-8)/3);retreat=smooth((f-4)/5)
    else:opening=.5*smooth((end-f)/4)+.5*smooth((end-f-8)/3);retreat=smooth((end-f-4)/5)
   ...
   A=old[un].translation.copy();target=H.translation;l1=(rest[fn].translation-rest[un].translation).length;l2=(rest[hn].translation-rest[fn].translation).length
   ...
   natural=target-desired*l2-A;natural-=axis*natural.dot(axis);pole=pole.normalized().lerp(natural.normalized(),.8*w).normalized()
   ...
   for n in left:
    if any(n.startswith(d) for d in ['index','middle','ring','pinky','thumb']):
     ...
     if opening and not n.startswith(('thumb','pinky')):
      if '_02_' in n or '_03_' in n or n=='pinky_01_l':
       angle=(15 if '_02_' in n else 10 if '_03_' in n else 0) if n.startswith('pinky') else 0
       target=Quaternion((0,0,1),math.radians(angle));digitbasis=digitbasis.to_quaternion().slerp(target,opening).to_matrix().to_4x4()
