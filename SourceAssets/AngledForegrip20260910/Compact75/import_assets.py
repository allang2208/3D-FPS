...
O=Path(__file__).parent;DEST='/Game/Weapons/M4AngledForegripCompact75'
...
task=unreal.AssetImportTask();task.filename=str(O/'SM_M4_AngledForegrip.fbx');task.destination_path=DEST;task.destination_name='SM_M4_AngledForegrip';task.automated=True;task.replace_existing=True;task.save=True;task.options=options
unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task]);sm=unreal.load_asset(DEST+'/SM_M4_AngledForegrip');assert sm
...
unreal.log('FOREGRIP_IMPORT_PASS')
