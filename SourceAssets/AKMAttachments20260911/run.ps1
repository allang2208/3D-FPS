param([string]$Run=('akm-attachments-'+(Get-Date -Format 'yyyyMMdd-HHmmss')),[switch]$Angled)
$ErrorActionPreference='Stop'
if($Run -notmatch '^[a-zA-Z0-9_-]+$'){throw 'Use a simple run label'}
$gripRoot='D:/FPS3D/FPSGAME'
$gripArgs='"{0}/FPSGAME.uproject" /Game/Weapons/M4InfimaRigV4/Preview/L_M4RigValidation -game -windowed -RenderOffscreen -ResX=1000 -ResY=700 -unattended -nosplash -AKMAttachmentAudit -PrismGripAudit -ForegripRun={1} -ColdSteelProfile=ForegripAudit_{1} -ExecCmds="DisableAllScreenMessages,t.MaxFPS 60" -abslog="{2}/runtime-{1}.log"' -f $gripRoot,$Run,$PSScriptRoot
