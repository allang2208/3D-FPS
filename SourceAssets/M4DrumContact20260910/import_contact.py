...
O=Path('D:/FPS3D/FPSGAME/SourceAssets/M4DrumContact20260910');DEST='/Game/Weapons/M4DrumDrop/Contact'
mesh=unreal.load_asset('/Game/Weapons/M4HK416Replica/SK_M4_FoldingSights_HK416');assert mesh
report={}
for clip,length in [('reload',2.1),('reload_empty',148/60)]:
 name='A_M4_DrumContact_'+clip
 ...
 options.anim_sequence_import_data.set_editor_property('use_default_sample_rate',False);options.anim_sequence_import_data.set_editor_property('custom_sample_rate',240)
 ...
 for i in range(round(length*960)+1):
  opt.evaluation_type=unreal.AnimDataEvalType.RAW;p=unreal.AnimPoseExtensions.get_anim_pose_at_time(a,i/960,opt)
  opt.evaluation_type=unreal.AnimDataEvalType.COMPRESSED;q=unreal.AnimPoseExtensions.get_anim_pose_at_time(a,i/960,opt)
 ...
 assert err<.05
 report[clip]={'asset':a.get_path_name(),'duration':a.get_play_length(),'keys':a.get_editor_property('number_of_sampled_keys'),'compression_error_cm':err,'verification_hz':960,'source':task.filename}
