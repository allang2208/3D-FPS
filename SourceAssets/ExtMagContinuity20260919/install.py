...
O=Path(__file__).parent;D='/Game/Weapons/ExtMagContinuity20260919';E=u.EditorAssetLibrary;A=u.AssetToolsHelpers.get_asset_tools();L=u.MaterialEditingLibrary
...
def seam_material(gun,original):
 ...
 return result
jobs={
 'M4':('/Game/Weapons/M4InfimaV3/Magazine_Light_001','/Game/Weapons/MagazineMouthFinish20260919/SM_ExtMag_M440'),
 'AKM':('/Game/Weapons/AKMIntegration/SovietFab/M_AKM_Soviet_PBR','/Game/Weapons/MagazineMouthFinish20260919/SM_ExtMag_AKM40_Mouth'),
 'QBZ':('/Game/Weapons/QBZ191/Attachments20260913/Materials/M_QBZ191_Unified_M_QBZ191_Wear_Magazine_polymer','/Game/Weapons/MagazineMouthFinish20260919/SM_ExtMag_QBZ40')}
report={}
for gun,(matpath,old) in jobs.items():
 material=u.load_asset(matpath)
 if gun!='QBZ':material=seam_material(gun,material)
 name='SM_ExtMag_'+gun+'40_Continuous';t=u.AssetImportTask();t.filename=str(O/'FBX'/(name+'.fbx'));t.destination_path=D;t.destination_name=name;t.automated=True;t.replace_existing=True;t.save=False
 ...
 if not t.imported_object_paths:raise RuntimeError('No import '+name)
 mesh=u.load_asset(D+'/'+name);slots=mesh.get_editor_property('static_materials')
 for i in range(len(slots)):
  slot=slots[i];slot.material_interface=u.load_asset('/Game/Weapons/MagazineMouthFinish20260919/M_AKM_MagazineMouth') if 'Mouth' in str(slot.material_slot_name) else material;slots[i]=slot
 mesh.set_editor_property('static_materials',slots);save(mesh)
 report[gun]={'mesh':mesh.get_path_name(),'previous':old,'actual_slots':[{'slot':str(s.material_slot_name),'material':s.material_interface.get_path_name()} for s in mesh.static_materials]}
 (O/'installed.json').write_text(json.dumps(report,indent=2));u.log('CONTINUITY_INSTALLED '+gun)
wet={}
for gun,path in [('M4','/Game/Weather/NaturalV2/MI_Wet_Magazine_Light_001_612d2e6c95'),('AKM','/Game/Weather/NaturalV2/M_Wet_M_AKM_Soviet_PBR_74c2286707')]:
 source=u.load_asset(path)
 if not source:raise RuntimeError('Missing existing wet material '+path)
 wet[gun]={'source':path,'replacement':seam_material(gun+'Wet',source).get_path_name()}
(O/'wet_materials.json').write_text(json.dumps(wet,indent=2))
