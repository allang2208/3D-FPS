...
from author_impact_smoke_corrosion import pool_surface,TAG
...
for source,name in [('/Game/Dungeons/SlimeSheet20260922/Materials/M_PusConvergingSheet','M_RoutePusSheet'),('/Game/Dungeons/SlimeSheet20260922/Materials/M_PusConvergedDrops','M_RoutePusDrops'),('/Game/Dungeons/SlimeSheet20260922/Materials/M_PusSingleImpact','M_RoutePusImpact'),('/Game/Dungeons/CombatExpansion20260922/Materials/M_PusFluidV2','M_RoutePusChannel')]:
    ...
    for prop in [u.MaterialProperty.MP_NORMAL,u.MaterialProperty.MP_WORLD_POSITION_OFFSET]:
        current=L.get_material_property_input_node(mat,prop)
        # Existing route normals already include Local->World below our final
        # surface detail. New copies still need that transform around the source.
        if (not created and prop==u.MaterialProperty.MP_NORMAL and
            isinstance(current,u.MaterialExpressionCustom) and
            current.get_editor_property('description')==TAG+' Pool normal'):
            continue
    ...
    if name in ['M_RoutePusImpact','M_RoutePusChannel']:pool_surface(mat)
