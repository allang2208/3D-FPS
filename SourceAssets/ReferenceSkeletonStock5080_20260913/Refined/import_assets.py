...
P=Path(__file__).resolve().parent;D='/Game/Weapons/ReferenceStock5080/Refined91379'
...
for key in ['BaseColor','MetalRough','Normal']:
    t=import_file(P/'Textures'/(key+'.png'),'T_SkeletonStock_'+key,D+'/Textures')
...
for kind in ['Polymer','Rubber']:
    m=material('M_SkeletonStock_'+kind,D);uv=uvcoord(m,0)
...
for family in ['M4','AKM','QBZ191']:
    dest=D+'/'+family;metal=material('M_SkeletonStock_Metal_'+family,dest);finish(metal,family);normal(metal);M.recompile_material(metal);save(metal)
    adapter=material('M_SkeletonStock_Adapter_'+family,dest);finish(adapter,family);M.recompile_material(adapter);save(adapter)
    ...
    mesh=import_file(P/family/'SM_SkeletonStock.fbx','SM_SkeletonStock',dest,opt)
    for i,slot in enumerate(mesh.static_materials):
        name=str(slot.material_slot_name);mesh.set_material(i,nonmetal['Polymer'] if 'Polymer' in name else nonmetal['Rubber'] if 'Rubber' in name else adapter if 'Adapter' in name else metal)
    E.set_metadata_tag(mesh,'SourceCandidate','ReferenceSkeletonStock5080_20260913/seed_91379')
    E.set_metadata_tag(mesh,'AuthoringStage','Local repairs, low mesh, baked normals, per-rifle mounting; not runtime tested')
    save(mesh);report['assets'][family]=mesh.get_path_name();print('STOCK_REVISION_SAVED',family,flush=True)
(P/'import_results.json').write_text(json.dumps(report,indent=2));u.log('STOCK_REFINED91379_IMPORT_COMPLETE')
