...
void AFPSGAMECharacter::ApplyColdSteelProfile(UColdSteelStatusModel* Profile)
{
    ...
    bInventoryWeaponReady=I&&(I->Definition==TEXT("ue_m4a1")||I->Definition==TEXT("ue_akm"));
    ...
    if(auto* Gunsmith=GetGameInstance()->GetSubsystem<UGunsmithSystem>())
    {
        ...
        {const auto Stats=Gunsmith->Calculate(I->Definition,Parts);ADSInDuration=Stats.ADS;MagazineCapacity=Stats.Capacity;ReloadDuration=Stats.Reload;EmptyReloadDuration=Stats.EmptyReload;
            ...
            if(I->Definition==TEXT("ue_akm")){DamagePerShot=Stats.Damage+Profile->Derived(TEXT("atk"));FireInterval=Stats.Interval/FMath::Max(1.f,Profile->Derived(TEXT("aspd")));}}
    }
    if(I)if(auto* E=GetGameInstance()->GetSubsystem<UColdSteelEnhancementSystem>()){
        // Keep weapon and character contributions separate so republishing never compounds upgrades.
        DamagePerShot=E->ProcessedDamage(*I,DamagePerShot-Profile->Derived(TEXT("atk")),Profile->Derived(TEXT("atk")));
        FireInterval*=E->Effect(*I,TEXT("attackIntervalMul"),1);
    }
