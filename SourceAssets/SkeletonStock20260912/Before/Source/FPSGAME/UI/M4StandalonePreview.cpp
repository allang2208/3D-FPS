...
void UM4GunsmithWidget::SetStandaloneItem(const FColdSteelItem& Item)
{
    ...
    Rig->bUseM4Infima=Item.Definition==TEXT("ue_m4a1");Rig->InitializeWeaponVisuals();
    Rig->SetGunsmithOpticVariant(Parts.FindRef(TEXT("optic")));Rig->SetGunsmithDrum(Parts.FindRef(TEXT("magazine"))==TEXT("large_drum"));Rig->SetGunsmithMuzzle(Parts.FindRef(TEXT("muzzle")));Rig->SetGunsmithHandstop(Parts.FindRef(TEXT("underbarrel")));Rig->UpdateFoldingSights(1.f);
