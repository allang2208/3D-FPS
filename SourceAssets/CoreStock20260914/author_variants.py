...
bpy.ops.wm.open_mainfile(filepath=str(P/'CoreStock_Canonical_Editable.blend'))
...
def material(kind):
    ...
    if kind in {'Adapter','FrontPolymer','FrontMetal'}:
...
mats={k:material(k) for k in ['Metal','Polymer','Rubber','Adapter','FrontPolymer','FrontMetal']}
labels=[]
...
for f in low.data.polygons:
    ...
    labels.append(4 if f.material_index==1 else 5 if f.material_index==2 else 2 if rubber else 1 if polymer else 0)
report={'source':'CoreStock_Canonical_Editable.blend','opening_center_generator':list(origin),'body_length_m':.2,'source_uv_kept':1,'baked_uv':0,'receiver_uv':2,'variants':{},'tested':False,'preview_rendered':False}

for family in ['M4','AKM','QBZ191']:
    ob=bpy.data.objects.new('SM_CoreStock',base.copy());bpy.context.collection.objects.link(ob);ob.data.materials.clear()
    for key in ['Metal','Polymer','Rubber','Adapter','FrontPolymer','FrontMetal']:ob.data.materials.append(mats[key])
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(out/'CoreStock_Game_Editable.blend'))
    ...
    bpy.ops.export_scene.gltf(filepath=str(out/'CoreStock_Game.glb'),use_selection=True,export_apply=True)
    report['variants'][family]={'asset_dir':'/Game/Weapons/CoreStock20260914/'+family,'body_start_m':leading+.006,'adapter_front_m':leading,'triangles':sum(len(f.vertices)-2 for f in ob.data.polygons),'materials':[m.name for m in ob.data.materials]}
