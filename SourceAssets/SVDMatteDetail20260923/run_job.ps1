...
$jobDir='D:/FPS3D/FPSGAME/SourceAssets/SVDMatteDetail20260923'
...
try {
 ...
 if($running.Count){$bridge=$true}
  ...
  $result=$LASTEXITCODE
  Select-String -LiteralPath "$jobDir/$job.log" -Pattern 'LogPython:.*(SVD_|Error|Traceback)' | Select-Object -Last 8 | ForEach-Object {$_.Line}
