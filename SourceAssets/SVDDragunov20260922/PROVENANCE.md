...
## 派生件 UV 修正（2026-09-23）

GLB 内嵌图像与原包 4K 图像的垂直方向不同；派生件的 UV0 已做 `V = 1 - V` 转换以匹配原包。修改范围为八个分件和当前完整骨骼模型 UV，保留原几何位置、源 GLB 和原图。源备份、UV 前后散列与 UE 保存记录位于 `../SVDCompletion20260923/UVRepair/`。此前记录中的“未修改 UV”描述仅适用于初次拆分阶段。

## 派生表面重制（2026-09-23）

`../SVDSurface20260923/` 保留本轮制作源与四张新 4096² 派生图：枪体、PSO 各 BaseColor／ORM。按几何划分枪钢、聚合物枪托／护木、橡胶、棕色贴腮垫，重新制作底色、粗糙度及金属度；部分原色／刻字与源粗糙度小幅变化保留，AO 改为独立通道。原结构法线、修正后的 UV0 和几何继续沿用，没有重新烘焙法线或宣称修复源压缩损失。七个既有材质实例已接入新母材质；上文初版 `BaseColor × AO × Tint` 不再是当前这七个实例的连接方式。原作者署名与 CC BY 4.0 继续适用。
