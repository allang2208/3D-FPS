...
# Weapon performance authored from the reference clip instead of the accepted M4
# motion: the receiver tips back and opens toward the shooter while the magazine
# is worked, then closes as it seats. (fraction, pitch deg, yaw deg, back m, down m).
WEAPON_CURVE = {
    "reload_empty": [
        (0.00, 0.0, 0.0, 0.000, 0.000),
        (0.12, 6.0, 5.0, 0.006, 0.004),
        (0.30, 12.0, 9.0, 0.012, 0.008),
        (0.55, 10.0, 7.0, 0.010, 0.006),
        (0.72, 5.0, 3.0, 0.005, 0.003),
        (0.86, 1.5, 1.0, 0.002, 0.001),
        (1.00, 0.0, 0.0, 0.000, 0.000),
    ],
}

WELL_LOCAL = Vector((0.004, 0.181, -0.022))
# Charging handle travel and the hand that works it: (start fraction, pull metres,
# hold frames). Pulled straight back along the receiver axis, then released.
CHARGE = {"reload_empty": (0.86, 0.055, 6.0)}
RELOAD_PATH = {
    "reload_empty": [
        (0.00, (0.00, 0.00, 0.00), 0.0),
        (0.80, (0.00, 0.02, -0.05), 1.0),   # release the seated magazine
        # The charge phase (>= 0.84) is driven from the handle's fitted anchor.
...
def charge_handle(pose, kind, frame, length):
    """Work the charging handle for the empty reload; returns its pull metres."""
    spec = CHARGE.get(kind)
    if not spec:
        return 0.0
    start, pull, hold = spec
    t = frame / length
    if t < start:
        return 0.0
    into = min(1.0, (t - start) / 0.06)
    back = smooth(into)
    if t > start + 0.06 + hold / length:
        back = max(0.0, 1.0 - smooth((t - (start + 0.06 + hold / length)) / 0.02))
    travel = pull * back
    if travel <= 1e-5:
        return 0.0
    q = pose["WPN_root"].to_quaternion()
    offset = q @ Vector((0.0, travel, 0.0))
    for name in ("WPN_ChargingHandle", "WPN_bolt"):
        if name in pose:
            pose[name] = Matrix.Translation(offset) @ pose[name]
    return travel


def apply_weapon_curve(pose, kind, frame, length):
    """Author the receiver's own performance; the hands are re-driven after."""
    keys = WEAPON_CURVE.get(kind)
    if not keys:
        return None
    ...
    for k0, k1 in zip(keys, keys[1:]):
        if t <= k1[0]:
            ...
            a = smooth((t - k0[0]) / span)
            c = tuple(x0 + (x1 - x0) * a for x0, x1 in zip(k0[1:], k1[1:]))
            break
    pitch, yaw, back, down = c
    if abs(pitch) < 0.01 and abs(yaw) < 0.01 and back < 1e-5 and down < 1e-5:
        return None
    q = pose["WPN_root"].to_quaternion()
...
def path_hand_target(pose, kind, frame, length):
    ...
    t = max(0.0, min(1.0, frame / length))
    if t >= 0.84:
        # Working the charging handle: meet it, pull it back, let it fly forward.
        pull = 0.055 * smooth(min(1.0, (t - 0.89) / 0.05)) * (1.0 - smooth(max(0.0, (t - 0.96) / 0.03)))
        weight = 1.0 if t < 0.97 else 1.0 - smooth((t - 0.97) / 0.03)
        return pose["WPN_root"] @ (Vector(WELL_LOCAL) + HANDLE_LOCAL + Vector((0.0, pull, 0.0))), weight
...
for kind, (source_name, end) in CLIPS.items():
    ...
    for step in range(end * 2 + 1):
        ...
        charge_handle(pose, kind, frame, end)
        rigid = apply_weapon_curve(pose, kind, frame, end)
        if rigid:
            # The firing hand rides the grip, so it follows the receiver.
            shift_arm(pose, "r", (rigid @ pose["hand_r"].translation) - pose["hand_r"].translation)
