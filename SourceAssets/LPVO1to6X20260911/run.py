...
P=Path(__file__).parent;REMOTE='LPVO1to6X20260911'
def submit(mode):
 ...
 body=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="prismlpvo_white.png"\r\nContent-Type: image/png\r\n\r\n').encode()+(P/'reference_white.png').read_bytes()+f'\r\n--{boundary}--\r\n'.encode()
 ...
 if mode.startswith('generate'):
  ...
  v=defaults(info,'Trellis2MeshWithVoxelGenerator');v.update(pipeline=['1',0],image=['3',0],pipeline_type='1024_cascade',seed=911166,sparse_structure_steps=16,shape_steps=32,texture_steps=24,max_num_tokens=999999,generate_texture_slat=True,fill_holes=False)
  ...
  w['6']={'class_type':'Trellis2ExportMesh','inputs':{'trimesh':['5',0],'filename_prefix':REMOTE+'/lpvo_raw','file_format':'glb'}}
  w['7']={'class_type':'Trellis2OvoxelExportToGLB','inputs':{'mesh':['4',0],'resolution':1024,'texture_size':2048,'target_face_num':100000}}
  w['8']={'class_type':'Trellis2ExportMesh','inputs':{'trimesh':['7',0],'filename_prefix':REMOTE+'/lpvo_textured_master','file_format':'glb'}}
  if mode=='generate_filled':
   for key in ['6','8']:w[key]['inputs']['filename_prefix']=w[key]['inputs']['filename_prefix'].replace('lpvo_','lpvo_filled_')
 (P/(mode+'_workflow.json')).write_text(json.dumps(w,indent=2))
 r=req('/prompt',json.dumps({'prompt':w,'client_id':'panoramic-lpvo'}).encode(),{'Content-Type':'application/json'});r['submitted_at']=time.time();receipt.write_text(json.dumps(r,indent=2));print(r)
def status(mode):
