...
P=Path(__file__).parent;D='/Game/Weapons/LPVO1to6X';lib=unreal.MaterialEditingLibrary;assets=unreal.AssetToolsHelpers.get_asset_tools()
...
t=unreal.AssetImportTask();t.filename=str(P/'SM_LPVO1to6X.fbx');t.destination_path=D;t.destination_name='SM_LPVO1to6X';t.automated=True;t.replace_existing=True;t.save=True;t.options=o;assets.import_asset_tasks([t]);mesh=unreal.load_asset(D+'/SM_LPVO1to6X');assert mesh
t=unreal.AssetImportTask();t.filename=str(P/'SM_LPVORing.fbx');t.destination_path=D;t.destination_name='SM_LPVORing';t.automated=True;t.replace_existing=True;t.save=True;t.options=o;assets.import_asset_tasks([t]);ring=unreal.load_asset(D+'/SM_LPVORing');assert ring
textures={}
for name in ['BaseColor','Roughness','Metallic','Normal']:
 t=unreal.AssetImportTask();t.filename=str(P/('T_LPVO_'+name+'.png'));t.destination_path=D;t.automated=True;t.replace_existing=True;t.save=True;assets.import_asset_tasks([t]);tex=unreal.load_asset(D+'/T_LPVO_'+name);assert tex
...
glass=material('M_LPVO_Glass');glass.set_editor_property('blend_mode',unreal.BlendMode.BLEND_TRANSLUCENT);glass.set_editor_property('two_sided',True)
...
reticle=material('M_LPVO_Reticle');reticle.set_editor_property('shading_model',unreal.MaterialShadingModel.MSM_UNLIT);reticle.set_editor_property('two_sided',True);constant(reticle,(40,.04,.006),unreal.MaterialProperty.MP_EMISSIVE_COLOR)
...
ring.set_material(0,body);assert unreal.EditorAssetLibrary.save_loaded_asset(ring,only_if_is_dirty=False)
assert unreal.EditorAssetLibrary.save_loaded_asset(mesh,only_if_is_dirty=False)
b=mesh.get_bounding_box();size=b.max-b.min;assert 24.8<size.x<25.2 and 3<size.y<6 and 5<size.z<9,str(size)
report={'asset':mesh.get_path_name(),'bounds_cm':[str(b.min),str(b.max)],'triangles':mesh.get_num_triangles(0),'materials':[s.material_interface.get_path_name() for s in mesh.static_materials],'ring_triangles':ring.get_num_triangles(0),'ring_bounds':[str(ring.get_bounding_box().min),str(ring.get_bounding_box().max)]}
(P/'import_report.json').write_text(json.dumps(report,indent=2));unreal.log('LPVO_IMPORT_PASS '+json.dumps(report))
