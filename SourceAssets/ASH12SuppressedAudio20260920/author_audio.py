"""Make full-level ASH suppressor voices solely from the user's ASH fire WAV.
...
SOURCE = PROJECT / 'SourceAssets/ASH12CameraAudio20260919/S_ASH12_Fire.wav'
x, rate = sf.read(SOURCE, always_2d=True, dtype='float64')
if rate != 48000:
    raise RuntimeError('This authoring recipe uses the existing 48 kHz ASH source.')
...
def weighted(signal):
    ...
    # 400 ms loudness gate (the source itself is only 288 ms long).
...
manifest = {
    'source': str(SOURCE),
    'original_source': 'D:/FPS3D/资产/音效/ash-12-fire.mp3',
    'source_sha256': hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
    'rights': 'User-provided ASH source and derivatives retain original source rights; not CC0.',
    'recipe': 'ASH source only; filtered blast, retained low body/mechanics, short chamber returns; compensated source level.',
    'sample_rate': rate, 'channels': x.shape[1], 'duration_seconds': len(x) / rate,
    'pitch_multiplier': 1.0, 'runtime_volume_multiplier': 'Same as normal ASH fire.',
