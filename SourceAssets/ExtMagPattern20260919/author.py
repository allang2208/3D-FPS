...
JOBS={'M4':(.030,.0125),'AKM':(.030,.030),'QBZ':(.030,.031)}
reports={}
for gun,(cutheight,period) in JOBS.items():
 ...
 ob.hide_set(False);ob.hide_render=False;ob.name='SM_ExtMag_'+gun+'40'
 ...
 cut=float(candidates[int(np.argmin(costs))]);cut={'M4':.151,'AKM':cut,'QBZ':cut}[gun];start=cut-period
 ...
 def contour(vs):return sorted([(angle(v),v.co.copy()) for v in vs])
 def at(cont,phi):
  phi%=2*math.pi
  for i,(a,p) in enumerate(cont):
   b,q=cont[(i+1)%len(cont)];b=b if b>a else b+2*math.pi;t=phi if phi>=a else phi+2*math.pi
   if a-1e-10<=t<=b+1e-10:
    d=Vector((math.cos(phi),math.sin(phi)));e=Vector((q.x-p.x,q.y-p.y));p2=Vector((p.x,p.y))
    den=e.x*d.y-e.y*d.x
    fac=0 if abs(den)<1e-12 else -(p2.x*d.y-p2.y*d.x)/den
    return p.lerp(q,min(1,max(0,fac)))
  raise RuntimeError('No contour interval')
 ...
 # Distributed contour correction across a full donor period preserves actual
 # grooves/panels; it is not a rigid copy forced onto a different cut ring.
 def mapped(q,mode):
  p=q.copy()
  if mode==-1:p.z+=extension
   ...
   for k in range(len(ac)):
    a=at(ac[k],phi);b=at(bc[k],phi);mid=a.lerp(b,t)
    choices.append((math.hypot(mid.x-p.x,mid.y-p.y),a,b))
   _,a,b=min(choices,key=lambda x:x[0]);weight=1-t*t*(3-2*t)
 ...
 def dst(v,mode):
  ...
  if key not in vcache:
   p=mapped(v.co,mode);weld=tuple(round(x,7) for x in p)
 ...
 def normal(v,n,mode):
  ...
  if key not in ncache:
   original=roll(v.co);delta=1e-5;base=mapped(v.co,mode);cols=[]
   for axis in range(3):
    p=original.copy();p[axis]+=delta;cols.append((mapped(unroll(p),mode)-base)/delta)
 ...
 reports[gun]['adjacent_basis_corners']=export(newmesh,O/'FBX'/('SM_ExtMag_'+gun+'40.fbx'))
