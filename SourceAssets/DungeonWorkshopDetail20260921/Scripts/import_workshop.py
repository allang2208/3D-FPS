...
DEST='/Game/Dungeons/AtmosphereV2/RoomInteriors/WorkshopDetail'
...
for key,recipe in recipes.items():
    name='M_WSDetail_'+key;path=DEST+'/Materials/'+name;m=u.load_asset(path)
    if not m:
        m=A.create_asset(name,DEST+'/Materials',u.Material,u.MaterialFactoryNew())
        for ch,filename in recipe['maps'].items():
            t=texture(filename,'T_WSDetail_'+key+'_'+ch,ch)
        ...
        if 'Roughness' not in recipe['maps']:output(scalar(m,recipe.get('roughness',.6)),'ROUGHNESS')
...
for entry in manifest['objects']:
    ...
    for i,slot in enumerate(mesh.get_editor_property('static_materials')):
        key=re.sub(r'[._][0-9]{3}$','',str(slot.get_editor_property('material_slot_name'))).removeprefix('WSDetail_')
...
print('WORKSHOP_DETAIL_ASSETS_SAVED',len(receipt['meshes']),len(receipt['materials']))
