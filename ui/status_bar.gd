...
func _build() -> void:
	...
	hp_bg.color = Color(0.05, 0.06, 0.10, 0.72)
	hp_bg.position = Vector2(16, 12)
	hp_bg.size = Vector2(BAR_W, BAR_H)
	add_child(hp_bg)
	_hp_fill = ColorRect.new()
	_hp_fill.color = Color(0.5, 0.82, 0.42)
	_hp_fill.position = Vector2(18, 14)
	_hp_fill.size = Vector2(BAR_W - 4, BAR_H - 4)
	add_child(_hp_fill)
	_hp_label = _make_label("生命 100/100", Vector2(16, 34), 16, Color(0.92, 0.92, 0.96))
	_kill_label = _make_label("击杀: 0", Vector2(16, 58), 16, Color(0.96, 0.9, 0.7))
	# 右下：弹药 + 状态提示
	_ammo_label = _make_label("弹药: --", Vector2.ZERO, 22, Color(0.92, 0.92, 0.96))
	_ammo_label.set_anchors_and_offsets_preset(Control.PRESET_BOTTOM_RIGHT)
	_ammo_label.position = Vector2(-170, -42)
	_ammo_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_status_label = _make_label("", Vector2.ZERO, 16, Color(0.98, 0.75, 0.4))
	...
	# 中央：准星 + 命中标记
	_center(_make_label("+", Vector2.ZERO, 26, Color(0.95, 0.95, 0.9)))
	_hitmarker = _make_label("✕", Vector2.ZERO, 30, Color(0.98, 0.98, 0.95))
	_center(_hitmarker)
...
func set_hp(hp: int, max_hp: int) -> void:
	...
	if pct > 0.5:
		...
		_hp_fill.color = Color(0.88, 0.66, 0.31)
	else:
		_hp_fill.color = Color(0.85, 0.36, 0.29)
