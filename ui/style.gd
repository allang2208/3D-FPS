...
static var COLOR_SLOT_TINT_EMPTY: Color = Color(1.0, 1.0, 1.0, 0.42)      # 灰色毛玻璃
static var COLOR_SLOT_TINT_ITEM: Color = Color(1.0, 0.98, 0.94, 0.48)
static var COLOR_SLOT_TINT_HOVER: Color = Color(1.0, 0.90, 0.70, 0.58)    # 金色毛玻璃
static var COLOR_SLOT_TINT_DRAG: Color = Color(1.0, 0.84, 0.55, 0.64)
static var COLOR_EQUIP_TINT_EQUIPPED: Color = Color(1.0, 0.98, 0.94, 0.52)
static var COLOR_EQUIP_TINT_LOCKED: Color = Color(0.56, 0.56, 0.60, 0.42)
static var COLOR_OVERLAY_STRONG: Color = Color(0, 0, 0, 0.55)  # 命令面板/遮罩强压暗
...
static var COLOR_STATUS_CARD_BG: Color = Color(0, 0, 0, 0.15)     # 原项目 status-section 半透明暗卡
static var COLOR_ATTR_ROW_BG: Color = Color(0.353, 0.302, 0.247, 0.20)   # 原项目 attr-item 底
static var COLOR_ATTR_ROW_HOVER: Color = Color(0.353, 0.302, 0.247, 0.35)
static var COLOR_HUD_BG: Color = Color(0.165, 0.145, 0.125, 0.85)   # 原项目 top-bar #2a2520@0.85
static var COLOR_HUD_BORDER: Color = Color(0.353, 0.302, 0.247)     # #5a4d3f
static var COLOR_HUD_TRACK: Color = Color(0.118, 0.102, 0.086)      # #1e1a16
static var COLOR_HUD_TEXT: Color = Color(0.831, 0.773, 0.663)       # #d4c5a9
static var COLOR_HUD_DIM: Color = Color(0.541, 0.490, 0.420)        # #8a7d6b
static var COLOR_HUD_GOLD: Color = Color(1.0, 0.843, 0.0)           # #ffd700
static var COLOR_HUD_SLOT_BG: Color = Color(0.239, 0.204, 0.169)     # 原项目 quick-slot #3d342b
static var COLOR_HUD_SLOT_HOVER: Color = Color(0.290, 0.247, 0.208)  # #4a3f35
static var COLOR_HUD_SLOT_HOVER_BORDER: Color = Color(0.541, 0.490, 0.420)  # #8a7d6b
...
static func _apply_colors(p: Dictionary) -> void:
	...
	if p.has("COLOR_OVERLAY_STRONG"): COLOR_OVERLAY_STRONG = _hex_to_color(p.COLOR_OVERLAY_STRONG)
	...
	if p.has("COLOR_STATUS_CARD_BG"): COLOR_STATUS_CARD_BG = _hex_to_color(p.COLOR_STATUS_CARD_BG)
	if p.has("COLOR_ATTR_ROW_BG"): COLOR_ATTR_ROW_BG = _hex_to_color(p.COLOR_ATTR_ROW_BG)
	if p.has("COLOR_ATTR_ROW_HOVER"): COLOR_ATTR_ROW_HOVER = _hex_to_color(p.COLOR_ATTR_ROW_HOVER)
	if p.has("COLOR_HUD_BG"): COLOR_HUD_BG = _hex_to_color(p.COLOR_HUD_BG)
	if p.has("COLOR_HUD_BORDER"): COLOR_HUD_BORDER = _hex_to_color(p.COLOR_HUD_BORDER)
	if p.has("COLOR_HUD_TRACK"): COLOR_HUD_TRACK = _hex_to_color(p.COLOR_HUD_TRACK)
	if p.has("COLOR_HUD_TEXT"): COLOR_HUD_TEXT = _hex_to_color(p.COLOR_HUD_TEXT)
	if p.has("COLOR_HUD_DIM"): COLOR_HUD_DIM = _hex_to_color(p.COLOR_HUD_DIM)
	if p.has("COLOR_HUD_GOLD"): COLOR_HUD_GOLD = _hex_to_color(p.COLOR_HUD_GOLD)
	if p.has("COLOR_HUD_SLOT_BG"): COLOR_HUD_SLOT_BG = _hex_to_color(p.COLOR_HUD_SLOT_BG)
	if p.has("COLOR_HUD_SLOT_HOVER"): COLOR_HUD_SLOT_HOVER = _hex_to_color(p.COLOR_HUD_SLOT_HOVER)
	if p.has("COLOR_HUD_SLOT_HOVER_BORDER"): COLOR_HUD_SLOT_HOVER_BORDER = _hex_to_color(p.COLOR_HUD_SLOT_HOVER_BORDER)
