...
report={}
for folder,name in [('runtime-lit','Attack-review'),('runtime','Attack')]:
 paths=sorted((R/folder/'Attack').glob('*.png'))
 if not paths:continue
 assert len(paths)==37,(folder,len(paths))
 frames=[]
 for p in paths[:-1]:
  im=Image.open(p).convert('RGB').crop((420,0,1620,1040));im.thumbnail((720,624));frames.append(im)
 ...
 assert total==1500
 report[name]={'frames':gif.n_frames,'duration_ms':total,'inspection_lights':folder=='runtime-lit'}
 for page in range(3):
