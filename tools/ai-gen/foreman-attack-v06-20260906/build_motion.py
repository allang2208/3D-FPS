...
right=vk([(0,(-.20,-.12,-.65)),(.12,(-.34,.05,-.43)),(.24,(-.30,.20,.10)),(.35,(-.12,.18,.55)),(.42,(-.10,-.04,.53)),(.50,(-.17,-.48,.14)),(.59625,(-.12,-.72,-.22)),(.71,(.10,-.69,-.38)),(.91,(-.06,-.40,-.48)),(1.20,(-.22,-.14,-.62)),(1.5,(-.20,-.12,-.65))])
...
def pose(t,clip):
 ...
 for side,sign in [('L',1),('R',-1)]:
  ...
  if clip=='Attack':
   ...
   flex=curve(t-.025 if side=='R' else t-.04,[(0,.10),(.36,-.13),(.51,-.18),(.63,.20),(.83,.13),(1.5,.10)])
  ...
  matrix(fo,ankle,fd)
 # Keep the proven rigid section whip rig; no nonuniform hierarchy scaling.
 points=[]
 for i in range(33):
  u=i/32;ang=u*math.tau*1.12;coil=Vector((.30*math.sin(ang)*(1-.22*u),-.11*u-.08*math.sin(math.pi*u),-.42*(1-math.cos(ang))-.18*u));over=Vector((-.20*math.sin(u*math.pi),1.85*u,.70*math.sin(u*math.pi)));ext=Vector((.07*math.sin(u*math.tau),-2.70*u,-.65*u));off=coil
  if clip=='Attack':
   recoil=Vector((.48*math.sin(math.pi*u),-1.35*u,-.80*u))
   # Recovery propagates from grip to tip; the whole rope never closes at once.
   delayed=t-.14*u*max(0,min(1,(t-.69)/.22))
   off=curve(delayed,[(0,coil),(.41,over),(.59625,ext),(.69,ext+Vector((0,.04*u,-.05*u))),(.89,recoil),(1.36,coil),(1.5,coil)])
   if .69<t<1.5:
    envelope=math.sin(math.pi*(t-.69)/.81)**2
    off+=Vector((.18*math.sin(12*(t-.69)-5*u),.10*math.sin(10*(t-.69)-6*u),.10*math.sin(13*(t-.69)-5*u)))*u*envelope
   # Keep the first short section aligned to the fixed handle.
   blend=min(1,u/.12);blend=blend*blend*(3-2*blend)
   off=(grip_dir*(2.7*u)).lerp(off,blend)
  if clip=='Death':
   settle=curve(t,[(0,0),(.73,0),(1.17,1),(1.4,1)])
   on_ground=Vector((grip.x+.24*math.sin(ang),grip.y+.24*(1-math.cos(ang))+.07*u,.055))
   off=(grip+off).lerp(on_ground,settle)-grip
  points.append(grip+off)
 # Cubic transition removes the right-angle kink immediately after the handle.
 p0=points[0];p3=points[6];p1=p0+grip_dir*.16;p2=p3-(points[7]-points[5]).normalized()*.14
 for j in range(1,6):
  q=j/6;points[j]=(1-q)**3*p0+3*(1-q)**2*q*p1+3*(1-q)*q*q*p2+q**3*p3
 normal=Vector((0,1,0))
 for i in range(33):
  tangent=points[min(32,i+1)]-points[max(0,i-1)]
  if tangent.length<1e-6:tangent=Vector((0,0,-1))
  tangent.normalize();normal-=tangent*normal.dot(tangent)
  if normal.length<1e-5:normal=Vector((1,0,0))-tangent*tangent.x
  normal.normalize();z=normal.cross(tangent);m=Matrix((normal,tangent,z)).transposed().to_4x4();m.translation=points[i];a.pose.bones[f'whip.{i:02d}'].matrix=m
 bpy.context.view_layer.update()
...
bpy.ops.wm.save_as_mainfile(filepath=str(R/'foreman-attack-v06.blend'))
bpy.ops.object.select_all(action='DESELECT')
for o in [a,body,whip,bpy.data.objects['WhipHandle']]:o.select_set(True)
bpy.context.view_layer.objects.active=a
bpy.ops.export_scene.gltf(filepath=str(R/'foreman-attack-v06.glb'),export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',export_frame_range=False,export_force_sampling=True,export_frame_step=1,export_def_bones=True)
(R/'build-report.json').write_text(json.dumps(report,indent=2));print('FOREMAN_V06_BUILD',json.dumps(report))
