...
func run():
 ...
 if not OS.get_cmdline_user_args().has("--formal"):
  enemy.get_node("Model").free();var model=load("res://tools/ai-gen/foreman-attack-v06-20260906/foreman-attack-v06.glb").instantiate();model.name="Model";enemy.add_child(model)
