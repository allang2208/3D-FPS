...
R=Path('E:\\3d\\3-dfps\\tools\\ai-gen\\humanoid-detail-v02-20260906\\modern'); SOURCE=Path('E:\\3d\\3-dfps\\tools\\ai-gen\\modern-zombie-v01-20260906')
...
def alias(original,name,speed=1):
 ...
 return act
alias('idle_220f','Idle');alias('walk_limp_60f','Walk')
...
for points in foot_samples.values():
 ...
 for prev,nxt in zip(points,points[1:]):
  if prev.z<floor+.045 and nxt.z<floor+.045:
   ...
   if .08<speed<1.5:speeds.append(speed)
...
regions=['Skin','Shirt','Trousers','Hair','Leather','DriedBlood','Eyes']
regions += ['Tie']
face_regions=[]
...
for region in regions:body.data.materials.append(bpy.data.materials.new('ModernZombie_'+region))
...
for channel in ['albedo','roughness','normal']:
 im=bpy.data.images.new('ModernZombie_'+channel,width=2048,height=2048,alpha=False);im.colorspace_settings.name='sRGB' if channel=='albedo' else 'Non-Color';images[channel]=im
...
game=bpy.data.materials.new('ModernZombie_PBR');game.use_nodes=True;nt=game.node_tree;bs=nt.nodes.get('Principled BSDF');bs.inputs['Metallic'].default_value=0
...
bpy.ops.wm.save_as_mainfile(filepath=str(R/'modern-zombie-v02.blend'))
bpy.ops.export_scene.gltf(filepath=str(R/'modern-zombie-v02.glb'),export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',export_frame_range=False,export_force_sampling=True,export_frame_step=1,export_def_bones=False)
