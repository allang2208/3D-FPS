...
- zombie-raw.glb：生成网格；zombie-model.blend：规范化模型与渲染设置；zombie-rigged.blend：可编辑蒙皮与四段动作；zombie-preview.glb：正式导出源。
...
从 raw GLB 重建时先用 Blender 后台执行 inspect_mesh.py，生成 zombie-model.blend；再执行 rig_and_animate.py，最后用支持 Pillow 的 Python 执行 package_preview.py。已有规范化模型时可以跳过 inspect_mesh.py。用 Blender 执行 check_export.py，附 -- --walk 或 -- --death 查看相应动作。无需再次请求生成服务。工具版本为 Blender 5.1；安装位置按当前机器配置。
