...
bpy.ops.import_scene.gltf(filepath=str(R/'foreman-attack-v09.glb'))
...
for name,duration in [('Idle',1),('Walk',1.5),('Attack',1.5),('Death',1.4)]:
 ...
 if name=='Attack':
  samples=[]
  for t in [.42,.59625]:
   f=start+t*80;s.frame_set(int(f),subframe=f-int(f));bpy.context.view_layer.update();samples.append(a.pose.bones['hand.R'].matrix.translation.copy())
  drop=samples[0].z-samples[1].z;forward=abs(samples[1].y-samples[0].y)
  report[name]['wrist_downstroke_m']=drop;report[name]['wrist_forward_travel_m']=forward
  assert drop>.8 and forward<drop*.6,(drop,forward)
  planted=[feet[i][0] for i in range(len(feet)) if .51<=i/160<=.94]
  drift=max((p-planted[0]).xy.length for p in planted)
  report[name]['front_foot_planted_drift_m']=drift
  assert drift<.012,('attack front foot drift',drift)
 if name=='Walk':
  slip=[]
...
(R/'review-report.json').write_text(json.dumps(report,indent=2));print('FOREMAN_V09_REVIEW',json.dumps(report))
