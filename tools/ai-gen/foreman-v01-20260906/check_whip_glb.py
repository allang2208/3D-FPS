...
source=ROOT/(sys.argv[1] if len(sys.argv)>1 else 'foreman-whip-v02.glb')
...
for anim in doc['animations']:
 ...
 for t in np.linspace(0,end,round(end*160)+1):
  ...
  assert np.isfinite(out).all() and dist<3.5,(anim['name'],t,dist)
...
(ROOT/('arm-glb-validation.json' if source.stem=='foreman-arm-v03' else 'whip-glb-validation.json')).write_text(json.dumps(report,indent=2));print(json.dumps(report))
