...
bpy.ops.import_scene.gltf(filepath=str(ROOT/'foreman-raw.glb'))
...
report={'raw_min':list(lo),'raw_max':list(hi),'height_m':2.6,'meshes':[]}
for o in meshes:
    ...
    samples={}
    for h in (.1,.3,.6,.9,1.2,1.5,1.8,2.1,2.4):
...
for name,pos in [('front',(0,-5,1.05)),('threequarter',(3,-5,2.1)),('back',(0,5,1.05))]:
    camera.location=pos;camera.rotation_euler=(Vector((0,0,1.3))-camera.location).to_track_quat('-Z','Y').to_euler()
    scene.render.filepath=str(ROOT/f'model-{name}.png');bpy.ops.render.render(write_still=True)
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'foreman-model.blend'))
