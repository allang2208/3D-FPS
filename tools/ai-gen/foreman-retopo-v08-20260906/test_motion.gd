...
func run():
 ...
 if not OS.get_cmdline_user_args().has("--formal"):
  enemy.get_node("Model").free();var model=load("res://tools/ai-gen/foreman-retopo-v08-20260906/foreman-retopo-v08.glb").instantiate();model.name="Model";enemy.add_child(model)
