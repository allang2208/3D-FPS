extends SceneTree
const OUT="E:/3d/3-dfps/tools/ai-gen/modern-zombie-v01-20260906/rendered"
...
func render_all() -> void:
	...
	camera.size=2.5
	var versions=["A","B","C","D","E","quaternius","previous"]
	if ResourceLoader.exists("res://modern-zombie-v01.glb"):
		versions=["modern"]
	if "--ual" in OS.get_cmdline_user_args():versions=["ual"]
	for version in versions:
		var file="res://denys-"+version+"-motions.gltf"
		if version=="quaternius" or version=="previous":file="res://"+version+".glb"
		if version=="modern":file="res://modern-zombie-v01.glb"
		if version=="ual":file="res://ual-mannequin.glb"
		if not ResourceLoader.exists(file):continue
		var folder=OUT.path_join(version)
		DirAccess.make_dir_recursive_absolute(folder)
		var model=load(file).instantiate()
		model.scale=Vector3.ONE*(1.0 if version in ["previous","modern","ual"] else (1.25 if version=="quaternius" else .01))
		world.add_child(model)
		var ap: AnimationPlayer=model.find_child("AnimationPlayer",true,false)
		ap.callback_mode_process=AnimationMixer.ANIMATION_CALLBACK_MODE_PROCESS_MANUAL
		var idle="Idle" if version in ["quaternius","previous","modern"] else "idle_220f"
		...
		if version=="modern":clips=["Idle","Walk","Attack","AttackRight","Death","HitReact"]
