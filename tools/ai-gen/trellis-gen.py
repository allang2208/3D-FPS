...
def build_workflow(image_name, seed, faces, prefix):
    return {
        ...
        "11": {
            "class_type": "Trellis2TexSlatGenerator",
            "inputs": {
                "pipeline": ["10", 2],
                "image_cond": ["8", 0],
                "shape_slat": ["10", 0],
        ...
        "14": {
            ...
            "inputs": {"mesh": ["13", 0], "reorient_vertices": "90 degrees"},
        },
        "15": {
            "class_type": "Trellis2SimplifyTrimesh",
            "inputs": {"trimesh": ["14", 0], "target_face_num": faces, "method": "Meshlib"},
        },
        "16": {
        ...
        "17": {
            ...
            "inputs": {"trimesh": ["16", 0], "filename_prefix": prefix, "file_format": "glb"},
        },
    }
...
def main():
    ...
    while time.time() < deadline:
        ...
        if status.get("completed"):
            ...
            if not item:
                # history 对多 OUTPUT_NODE 的导出不保证出现；按 ExportMesh 的文件名规则直取
                print("[trellis] history has no GLB entry, using filename convention ...", flush=True)
                item = {"filename": f"{args.prefix}_00001_.glb", "subfolder": "", "type": "output"}
