...
bpy.ops.wm.open_mainfile(filepath=str(R.parent/'foreman-v01-20260906/foreman-arm-v03.blend'))
...
atk=vk([(0,(.07,0,.02,0)),(.17,(.02,-.10,.025,0)),(.38,(-.08,-.34,.015,.04)),(.51,(.13,-.02,.075,.17)),(.61,(.36,.27,.13,.24)),(.74,(.40,.29,.14,.24)),(1.02,(.19,.13,.07,.14)),(1.5,(.07,0,.02,0))])
right=vk([(0,(-.20,-.12,-.65)),(.12,(-.32,.04,-.46)),(.25,(-.36,.17,-.12)),(.36,(-.31,.12,.22)),(.44,(-.25,-.22,.15)),(.53,(-.18,-.55,-.10)),(.59625,(-.12,-.70,-.26)),(.71,(-.05,-.68,-.39)),(.92,(-.22,-.33,-.47)),(1.22,(-.23,-.14,-.61)),(1.5,(-.20,-.12,-.65))])
left=vk([(0,(.22,-.12,-.64)),(.21,(.33,-.23,-.44)),(.39,(.46,-.24,-.21)),(.57,(.33,.04,-.44)),(.76,(.25,.09,-.53)),(1.04,(.23,-.02,-.58)),(1.5,(.22,-.12,-.64))])
...
def pose(t,clip):
 ...
 for side,sign in [('L',1),('R',-1)]:
  ...
  if side=='R':grip=wrist+hdir*lengths[ha]*.65
 ...
 points=[]
 for i in range(33):
  u=i/32;ang=u*math.tau*2.15;coil=Vector((.24*math.sin(ang),.05*u,-.24*(1-math.cos(ang))-.10*u));over=Vector((-.20*math.sin(u*math.pi),1.85*u,.70*math.sin(u*math.pi)));ext=Vector((.07*math.sin(u*math.tau),-2.70*u,-.65*u));off=coil
  if clip=='Attack':
   off=curve(t,[(0,coil),(.41,over),(.59625,ext),(.69,ext+Vector((0,.04*u,-.05*u))),(1.09,coil*.98),(1.5,coil)])
  if clip=='Death':
...
bpy.ops.wm.save_as_mainfile(filepath=str(R/'foreman-motion-v04.blend'))
bpy.ops.object.select_all(action='DESELECT')
for o in [a,body,whip]:o.select_set(True)
bpy.context.view_layer.objects.active=a
bpy.ops.export_scene.gltf(filepath=str(R/'foreman-motion-v04.glb'),export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',export_frame_range=False,export_force_sampling=True,export_frame_step=1,export_def_bones=True)
(R/'build-report.json').write_text(json.dumps(report,indent=2));print('FOREMAN_V04_BUILD',json.dumps(report))
