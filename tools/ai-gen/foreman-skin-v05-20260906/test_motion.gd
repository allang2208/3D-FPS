...
func run():
 ...
 if not OS.get_cmdline_user_args().has("--formal"):
  enemy.get_node("Model").free();var model=load("res://tools/ai-gen/foreman-skin-v05-20260906/foreman-skin-v05.glb").instantiate();model.name="Model";enemy.add_child(model)
