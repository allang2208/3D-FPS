...
for clip,duration in specs.items():
    frames=[]
    paths=sorted((R/'rendered/wolf'/clip).glob('[0-9]*.png'))
    for p in paths[:-1]:
...
for view in ['left','right']:
    ...
    for j,(version,label) in enumerate([('source','原黑狼'),('wolf','黑色皮毛优化 · 原骨架与动作')]):
...
for j,view in enumerate(['left','right','front','side']):
    im=Image.open(R/'rendered/wolf'/(view+'.png')).convert('RGB').resize((800,582),Image.Resampling.LANCZOS)
    sheet.paste(im,(j%2*800,j//2*600))
sheet.save(R/'black-wolf-review.jpg',quality=95)
(R/'preview-report.json').write_text(json.dumps(report,indent=2));print('BLACK_WOLF_PREVIEWS_COMPLETE',json.dumps(report))
