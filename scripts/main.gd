...
func _build_enemies() -> void:
	...
	var miner: Node3D = load("res://scenes/enemies/miner_workwear_zombie.tscn").instantiate()
	miner.position = Vector3(-8, 0, -7)
	add_child(miner)
	miner.setup(_player, _on_enemy_killed)
	var runner: Node3D = load("res://scenes/enemies/runner_zombie.tscn").instantiate()
	runner.position = Vector3(7, 0, -10)
	add_child(runner)
	runner.setup(_player, _on_enemy_killed)
