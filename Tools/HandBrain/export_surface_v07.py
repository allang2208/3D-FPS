...
bpy.ops.wm.open_mainfile(filepath=str(r/'HandBrain_Refined_Baked.blend'))
...
for name in ['HandBrain_Body','HandBrain_AttackArm','HandBrain_CrownHands','HandBrain_OralTeeth']:
 ...
 for attr in list(obj.data.attributes):
  if attr.name in ['SurfaceRegion','SculptNail','SculptArea']:obj.data.attributes.remove(attr)
print('SURFACE_EXPORT_JOIN',flush=True)
...
print('SURFACE_EXPORT_FBX',flush=True)
...
bpy.ops.export_scene.fbx(filepath=str(r/'SK_HandBrain_Surface.fbx'),bake_anim=False,**kw)
...
bpy.ops.export_scene.fbx(filepath=str(r/'A_HandBrain_Howl_Surface.fbx'),bake_anim=True,**kw)
