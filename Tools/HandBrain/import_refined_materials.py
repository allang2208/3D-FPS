...
dest='/Game/Monsters/HandBrain/RefinedV03'
lib=unreal.EditorAssetLibrary;tools=unreal.AssetToolsHelpers.get_asset_tools();mel=unreal.MaterialEditingLibrary
mesh=unreal.load_asset('/Game/Monsters/HandBrain/SK_HandBrain');assert mesh
old=[s.material_interface.get_path_name() for s in mesh.get_editor_property('materials')]
report={'fab_material_used':False,'previous_materials':old,'materials':[],'mesh_changed':False,'animations_changed':False}
materials=[]
for index,row in enumerate(rows):
 ...
 mel.recompile_material(m);lib.save_loaded_asset(m);materials.append(m);report['materials'].append(m.get_path_name())
...
mesh.set_editor_property('materials',slots);lib.save_loaded_asset(mesh)
