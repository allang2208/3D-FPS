...
Uses exported user-added Fab ZombiSkinMaterial maps with original Hunyuan maps.
"""
...
OUT=ROOT/'realism_v05'; OUT.mkdir(exist_ok=True)
...
exec((ROOT/'realism_v05/adjust_jaw.py').read_text(),globals())
before={o.name:hashlib.sha256(b''.join(str(tuple(v.co)).encode() for v in o.data.vertices)).hexdigest() for o in meshes}
report={'fab_downloaded':True,'sources':'Existing Hunyuan maps plus user-added ZombiSkinMaterial height, AO, color luminance and roughness; local green/wound blend','materials':[]}
...
for obj in meshes:
 for slot,d in enumerate(defs[obj.name]):
  ...
  if oral:
   ...
   skin=mul(bc,(.38,.50,.27,1));blood=mul(bc,(.82,.23,.19,1))
   color=mix(skin,blood,wound)
   variation=mix((.70,.76,.64,1),(1.12,1.08,.94,1),broad);color=mul(color,variation)
   dry=math('ADD',.67,math('MULTIPLY',fine,.12));wet=math('ADD',.34,math('MULTIPLY',fine,.10))
   rough=math('ADD',math('MULTIPLY',dry,math('SUBTRACT',1,wound)),math('MULTIPLY',wet,wound));bump_dist=.0006
  if not oral and not teeth:
   # Rest-space box projection is baked to the existing UVs for animation.
   scale=node('ShaderNodeVectorMath');scale.operation='SCALE';scale.inputs[3].default_value=1.1
   link(tc.outputs['Object'],scale.inputs[0])
   def fab(semantic):
    t=node('ShaderNodeTexImage');t.image=bpy.data.images.load(str(ROOT/'material_v04/fab_source'/('T_ZombieSkinMaterial1_'+semantic+'.png')),check_existing=True)
    t.image.colorspace_settings.name='sRGB' if semantic=='basecolor' else 'Non-Color'
    t.projection='BOX';t.projection_blend=.3;link(scale.outputs[0],t.inputs['Vector']);return t.outputs['Color']
   fheight=fab('height');frough=fab('roughness');fao=fab('ambientocclusion')
   gray=node('ShaderNodeRGBToBW');link(fab('basecolor'),gray.inputs[0])
   tone=math('MINIMUM',1.18,math('ADD',.78,math('MULTIPLY',gray.outputs[0],2.5)))
   color=mul(color,tone);color=mul(color,math('ADD',.86,math('MULTIPLY',fao,.14)))
   rough=math('ADD',math('MULTIPLY',rough,.72),math('MULTIPLY',frough,.28))
  if obj.name=='HandBrain_Body' and slot==2:
   edge=node('ShaderNodeAttribute');edge.attribute_name='lip_band_t'
   mask=math('MULTIPLY',math('MAXIMUM',0,math('SUBTRACT',edge.outputs['Fac'],.55)),1.6)
   color=mix(color,(.075,.027,.022,1),mask)
   rough=math('SUBTRACT',rough,math('MULTIPLY',mask,.12))
  link(color,p.inputs['Base Color']);link(rough,p.inputs['Roughness'])
  p.inputs['Subsurface Weight'].default_value=.018 if not teeth else .01
  ...
  final_normal=bump.outputs[0]
  if not oral and not teeth:
   tissue=node('ShaderNodeBump');tissue.inputs['Strength'].default_value=.6;tissue.inputs['Distance'].default_value=.010
   link(fheight,tissue.inputs['Height']);link(final_normal,tissue.inputs['Normal']);final_normal=tissue.outputs[0]
  link(final_normal,p.inputs['Normal'])
...
report['material_pass_preserves_adjusted_geometry']=True
