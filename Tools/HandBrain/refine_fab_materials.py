...
OUT=ROOT/'material_v04'; OUT.mkdir(exist_ok=True)
...
for obj in meshes:
 for slot,d in enumerate(defs[obj.name]):
  ...
  if not oral and not teeth:
   ...
   scale=node('ShaderNodeVectorMath');scale.operation='SCALE';scale.inputs[3].default_value=3.0
   link(tc.outputs['Object'],scale.inputs[0])
   def fab(semantic):
    t=node('ShaderNodeTexImage');t.image=bpy.data.images.load(str(OUT/'fab_source'/('T_ZombieSkinMaterial1_'+semantic+'.png')),check_existing=True)
  ...
  if not oral and not teeth:
   tissue=node('ShaderNodeBump');tissue.inputs['Strength'].default_value=.6;tissue.inputs['Distance'].default_value=.004
