...
ROOT=Path('D:/FPS3D/FPSGAME');OUT=ROOT/'Saved/MeleeArmOpeningReview20260923/CameraLagGeometry';OUT.mkdir(parents=True,exist_ok=True)
...
for family in ('Standard',):
    base=ROOT/'SourceAssets/MeleeArmOpening20260923'/'LongGrip'
    ...
    for clip in ('Thrust','SprintOverhead'):
        ...
        for row in [min(rows,key=lambda r:abs(r['seconds']-t)) for t in ([.54,.58,.625] if clip=='Thrust' else [1.125])]:
            v=np.zeros((len(p),3))
            ...
            for lag_fps in (0,60,30):
                vfov=75
                def smooth(t):
                    t=max(0.,min(1.,t));return t*t*(3.-2.*t)
                start,end=(.4,.64) if clip=='Thrust' else (.97,1.22)
                t=row['seconds']
                lag=0. if lag_fps==0 else 100.*(smooth((t-start)/(end-start))-smooth((t-1./lag_fps-start)/(end-start)))
                camera.location=(0,-lag,0)
                camera.data.lens=24/(2*math.tan(math.radians(vfov*.5)))
                scene.render.filepath=str(OUT/f'Highland_{clip}_{row["seconds"]:.3f}_lag{lag_fps}.png');bpy.ops.render.render(write_still=True)
