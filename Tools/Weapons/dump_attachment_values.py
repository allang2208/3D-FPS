...
A('| 项 | 现状 |')
A('| --- | --- |')
A('| PKM 没有任何改造件 | `allowed: []`、`options: {}`；`Option()` 先检查 `allowed`，游戏里一件都选不了。 |')
...
A('| 项 | 调整前 | 调整后 |')
A('| --- | --- | --- |')
A('| `weapons[].base.spread_mult` | 该键不存在，所有枪静默取 1 | 七把长枪（M4A1 / AKM / QBZ-191 / ASH-12 / M16A2 / A762 / PKM）= `2`；M1911 与 DW715 不写键，保持参考值 1 |')
