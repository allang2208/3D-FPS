...
outer_swing='--outer-swing' in sys.argv
...
if outer_swing:ROOT=ROOT.parent/'OuterSwing/Previews'
source=ROOT/('SingleHandPickaxe' if pickaxe else 'DefaultAxe')
if drag_ground or natural_wrist or outer_swing:source=ROOT/state
...
for i,source_frame in enumerate(metadata['source_frames_1_based']):
    ...
    if outer_swing:label='Pickaxe | Outside lift, forward strike and outside recovery'
...
if outer_swing:gif=ROOT/f'InfectedMiner_OuterSwing_{state}.gif'
...
(ROOT/(f'preview_{state.lower()}.json' if drag_ground or natural_wrist or outer_swing else 'preview.json')).write_text(json.dumps(metadata,indent=2))
