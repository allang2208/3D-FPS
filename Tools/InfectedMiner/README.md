...
Current attack: [continuous exterior pickaxe path](../../Docs/InfectedMinerOuterSwing20260913.md). Run Blender `author_outer_swing.py`, then UE Python `import_outer_swing.py`. The original BP uses `OuterSwing20260913/A_Miner_OuterSwing_Attack`; Idle/Walk remain `NaturalWrist`, mesh/grip remain `DragGround`. Editable source and attack FBX are under `SourceAssets/InfectedMiner20260913/OuterSwing/Delivery`. Requested preview scripts use `--outer-swing`.
