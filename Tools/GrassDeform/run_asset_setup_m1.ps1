# Runs the M1 grass deform asset setup headless.
#
...
#
#   powershell -NoProfile -File Tools/AssetPipeline/mcp_call_codex.ps1 -PythonScript Tools/GrassDeform/setup_assets_m1.py
#
# Plan: Docs/WorldGeneration/grass-interaction-gpu-20260925.md section 4.
...
$script = Join-Path $PSScriptRoot 'setup_assets_m1.py'
$logDir = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) 'Saved\Logs'
$log = Join-Path $logDir 'GrassDeformM1.log'
...
if ($holders.Count -gt 0) {
    ...
    Write-Host '    -PythonScript Tools/GrassDeform/setup_assets_m1.py' -ForegroundColor White
    Write-Host ''
...
Write-Host "No engine process holds the project. Running headless asset setup..." -ForegroundColor Green
...
if ($code -ne 0) {
    Write-Host "Asset setup exited with code $code. See the output above and $log." -ForegroundColor Red
    exit $code
}

Write-Host 'Asset setup finished. Check the GRASS_DEFORM_M1_RESULT line above for the report.' -ForegroundColor Green
exit 0