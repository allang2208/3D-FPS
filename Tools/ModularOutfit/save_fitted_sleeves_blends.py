"""Save editable fitted-sleeve meshes; UE imports native bindings without FBX."""
import json
...
for entry in json.loads((ROOT/'manifest.json').read_text()):
    ...
    mesh = bpy.data.meshes.new(name+'_FittedSleevesV1')
    ...
    mat = bpy.data.materials.new('FieldSweater_Authoring')
    mat.diffuse_color = (.115, .135, .080, 1)
    mat.use_nodes = True
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/f'{name}_FittedSleevesV1.blend'))
    print('FITTED_SLEEVES_BLEND_SAVED', name, flush=True)
