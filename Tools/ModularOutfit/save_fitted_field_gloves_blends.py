"""Save editable fitted-glove meshes; UE imports native bindings without FBX."""
import json
...
for entry in json.loads((ROOT/'manifest.json').read_text()):
    ...
    mesh = bpy.data.meshes.new(name+'_FittedFieldGlovesV1')
    ...
    mat = bpy.data.materials.new('FieldLeather_Authoring')
    mat.diffuse_color = (.18, .085, .035, 1)
    mat.use_nodes = True
    ...
    for face, uv, normals in zip(mesh.polygons, data['uv'], data['normals']):
        ...
        for loop, (u, v), n in zip(face.loop_indices, uv, normals):
            layer.data[loop].uv = (u, 1-v)
            split_normals.append((n[0], -n[1], n[2]))
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/f'{name}_FittedFieldGlovesV1.blend'))
    print('FITTED_GLOVES_BLEND_SAVED', name, flush=True)
