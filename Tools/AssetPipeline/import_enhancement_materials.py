...
P=Path('D:/FPS3D/FPSGAME/SourceAssets/EnhancementMaterials5080_20260911/UE');D='/Game/Items/EnhancementMaterials';tools=unreal.AssetToolsHelpers.get_asset_tools();lib=unreal.MaterialEditingLibrary
...
Path('D:/FPS3D/FPSGAME/Saved/EnhancementMaterialsImport.json').write_text(json.dumps(report,indent=2));unreal.log('ENHANCEMENT_MATERIAL_IMPORT_PASS')
# Keep the requested purple mineral variant when rebuilding the base asset imports.
exec(compile(Path(__file__).with_name('apply_amethyst_stone.py').read_text(),str(Path(__file__).with_name('apply_amethyst_stone.py')),'exec'))
exec(compile(Path(__file__).with_name('apply_blue_magic_dust.py').read_text(),str(Path(__file__).with_name('apply_blue_magic_dust.py')),'exec'))
