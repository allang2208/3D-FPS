...
在 `DayNight_Lighting` 运行游戏，出生点附近按**关卡固定锚点**放置宝箱（检测地面及阻挡；场景已有宝箱时不重复创建）。走近至 2.4 米内显示 `E · 武器仓库`，按 E 同时打开仓库与装备背包页。
...
此前宝箱在 `FPSGAMEPlayerController` 的 3 秒定时器里以**玩家当前位置与视角**为基准找点（190cm 前方，依次尝试 6 个偏航角），玩家在 3 秒内走动或转头就会让落点每次不同。现在改为：

1. 锚点取关卡 `PlayerStart`（与 `FPSGAMEGameMode` 的出生规则同一数据源），并把锚点重新贴地；宝箱位置只由关卡与配置决定，不读玩家当前站位。
2. 关卡没有可用 `PlayerStart` 时才退回旧的“玩家当前位置”口径，来源写进日志 `anchor=playerStart|player`。
3. 落点校验沿用旧合同：竖直探针找地面（`ImpactNormal.Z ≥ 0.8`）＋ 宝箱胶囊盒 `ECC_Pawn` 阻挡检查（忽略玩家）；首选点被占用时按候选偏航依次兜底，顺序固定，因此结果仍可复现。

可调项在 `Content/ColdSteelData/warehouse_assets.json` 的 `spawn` 段（删掉该段即用代码默认值）：

| 字段 | 含义 |
| --- | --- |
| `offset` | 锚点局部偏移，cm，`x` 前 / `y` 右 / `z` 上；缺省 `190,0,0`＝原自动生成距离 |
| `location` | `{x,y,z}` 世界坐标，用于彻底钉死位置（`z` 会重新贴地）；填写后 `offset` 只作为兜底环半径 |
| `yaw` / `yawOffset` | 固定朝向（度）／相对锚点朝向的偏航 |
| `angles` | 首选点被占用时依次尝试的偏航角度，缺省 `0,60,-60,120,-120,180` |

启动日志会打印最终落点与来源：`WarehouseChest: fixed spawn <坐标> facing <朝向> (anchor=...)`；找不到合法落点时打印警告且不放置。本次改动未编译、未运行，等待用户自行验证。
