...
初始结构来自 `E:/3d/3-dfps/ui/status_bar.gd`。2026-09-12 的 [正式规则 v2](ui-cold-steel-design-system.md) 已替换旧外观：中性黑灰玻璃底 248/255 不透明度、BackgroundBlur 强度 9／半径 21、银灰细边、公共 Noto Sans SC 与 JetBrains Mono；标签在上、9px 轨道居中、当前值/上限在下。生命渐变 `#763B43 → #BD626D`，魔法渐变 `#36566E → #7194AC` 保留，轨道改用公共深灰 `Content`。生命不高于 25% 时数值和边线使用静态警示色；没有持续闪烁、伪造回血动画或滞后的数值。下方验收记录属于初始版本，新外观未测试，见 [本轮记录](status-cold-glass-20260912.md)。
