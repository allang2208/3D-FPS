...
后续接入更新：本文记录当时的发布状态。随后巧手任务已将选中的新版 M4 图标接入 `Skills/rifle_mastery_cold_steel.png`，巧手与闪避统一为同系列银灰六边形。当前配置及恢复说明见 [巧手交付](dexterous-hands-plan-20260913.md)，不再将下文的“仍为候选”理解为当前状态。
