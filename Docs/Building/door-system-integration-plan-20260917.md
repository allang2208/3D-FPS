...
## 9. 门框进深统一 40 cm 与双开门门扇换模型（2026-09-18 第二轮，已完成）

用户口径："统一门框的进深大小为 20 cm 体素格的整数倍，我希望设置为 **40 CM**；
把现在单开门的单门模型放到双开门中，单扇门进行替换。"

- **只跨出第 8 节那次回退的坑一次**：这次**没动任何开门代码**，也不重建几何——两次都用
  `copy_mesh_from_static_mesh → scale_mesh(逐轴) → copy_mesh_to_static_mesh`，并**逐槽拷回材质**
  （容器先用 `duplicate_asset` 复制源资产，避免 `copy_mesh_to_static_mesh` 只留一个空槽）。
- 单扇门：门**板**仍是包里的 `SM_Door`（未改）；门**框**换成 `Props/SingleDoor20260918/SM_SingleDoorFrame_D40`
  （24.84 × 113.99 × 212 → **40 × 114 × 212**，只缩进深一轴；环状网格用三角面碰撞），
  `ColdSteelDoor.cpp` 只改门框默认路径，铰链／开角 92°／占格 (2,6,11) 全部不变。
  复现：`SourceAssets/SingleDoor20260918/bake_single_door_frame_d40_20260918.py`
  （第三轮高度统一时改名为 `bake_single_door_d40_h220_20260918.py`，同时多烘一版门板）。
- 双开门：门框重建为 40 × 200 × 200（占格 **(2,10,10)**），门扇换成单扇门的门扇模型缩放版。
  细节、构建日志与实测清单见 [双开门案例第 8 节](double-door-200x200-20260918.md#8-第二轮门框进深统一-40-cm--门扇换成单开门门扇2026-09-18)。
- 顺带修掉一个编译阻塞：门／窗／双开门三个 .cpp 在匿名命名空间里定义同名常量（`DefaultLeafMesh`／
...
- 单扇门：门框 `SM_SingleDoorFrame_D40` 重新烘焙成 **40 × 114 × 220**（24.84 × 113.99 × 212，
  进深 2 格、高度 11 格）；门板换成 `SM_SingleDoorLeaf_H208` —— 包里 `SM_Door` 只按同一 Z 比例
  （220/212）缩高到 **207.55 cm**，`ColdSteelDoor.cpp` 只改门板默认路径，开门逻辑仍未动。
  **同步的依据**：包门框的洞口实测正好 90 × 200 ＝ 门板尺寸，同比例缩放后洞口仍被门板填满。
  复现：`SourceAssets/SingleDoor20260918/bake_single_door_d40_h220_20260918.py`（脚本已取代第二轮的
  `bake_single_door_frame_d40_20260918.py`）。
- 双开门：门框重建为 **40 × 200 × 220**（占格 **(2,10,11)**、洞口 184×204），门扇 **91.5 × 203**。
...
- 发布记录：提交 `f23265f`（`origin/main`），普通推送 `HEAD:main`；新增内容依赖
  `/Game/Props/SingleDoor20260918/SM_SingleDoorLeaf_D40`（旧 `SM_SingleDoorLeaf_H208` 已退役）。
...
## 12. 门宽统一 120 cm · 消除门与体素墙的透缝（2026-09-19 第五轮，已完成）

用户口径："门跟体素建造之间有缝隙，无法贴合。"

- **根因**：第二轮统一进深时刻意保留了包模型的宽度 **113.99**，而占格 Y 取整到 6 格＝**120 cm**——
  门框两侧各差 **3.005 cm**，20 cm 体素填不进 3 cm 的缝，门与墙之间必然透风。
  这正是 skill「占格必须整除」规则里"要人走的洞口必须整除"的漏网之鱼：
  高度／进深已整格，唯独宽度还挂着源模型名义值。双开门（200）与窗（100）都是整格，所以只有单扇门透缝。
- **修法**：只重烘网格——门框 `SM_SingleDoorFrame_D40` → **40 × 120 × 240**（三轴全整格，
  占格 (2,6,12) 不变且与实际外形完全一致）；门板 `SM_SingleDoorLeaf_D40` Y 随洞口同比例
  90 × 120/113.99 ≈ **94.74**（Z 仍 226.42），X 板厚 18.48 不缩（skill 第二轮结论 1）。
  复现：`bake_single_door_meshes_20260918.py`（新增 `WIDTH_CM = 120.0`，自检 14 项 PASS）。
- **C++ 零改动**：`AlignGeometry` 一切按网格包围盒实时摆放，铰链/门板/检测盒自动适配新宽度；
  只更新了 `ColdSteelDoor.h/.cpp` 两处尺寸注释。开门手感回归风险＝0（吸取第 8 节回退教训）。
- 编辑器进程占着资产，烘焙**在编辑器内远程通道跑**（`ue_python_exec.py`），落盘后**独立 commandlet
  进程读回**核实：frame (40,120,240)／leaf (18.48,94.74,226.42)／槽数 1/2／门框仍
  `CTF_USE_COMPLEX_AS_SIMPLE`／调色板三条占格 (2,6,12)，全部 OK
  （`SourceAssets/SingleDoor20260919/verify_door_grid_20260919.py` + 同名 `.log`）。
- 已放置过门的存档不受影响（占格与锚点公式都没变，旧门读档后按新网格重画）。
- **未实机实测**：靠墙放门看四边是否闭合、预览＝落地、按 E 开合、门框顶 240 与墙高配合由用户测试。

## 13. 门／窗框与体素互通：构件面成为焊接目标（2026-09-19，已编译）

用户报三件事：① 准星瞄在门框上不放吸附、想在门框上加体素／门旁续 1×1 m 墙都不行；
② 自动吸附（锥形辅助）又失效了；③ 窗的**正上方**完全建不了块。三个症状同一根因族：

- **构件的碰撞面从来不是体素焊接目标**。`UpdateTarget` 只认 `ResolveHit`（命中体素网格），
  门／窗／柱面一律落回"贴地寻优"分支；贴地探棒垂直打到门洞／窗玻璃上（`IsSurface` 法线过 45° 门槛
  或根本打空）→ 报"地面有陡坡、断崖或障碍"。①③ 的放置拒绝与 ② 的"辅助失效"都是它：
  锥形辅助射线同样只认体素面，视野里有门／窗／柱子挡在墙前面时，射线越近越优先命中构件 → 全部作废。
  09-18 门窗上量之前场景里没有这类件，所以"以前优化过的那版吸附"看起来像坏了——**没有回退，是被新场景暴露**。
- **构件不给体素提供支撑锚**：贴门框侧面／窗沿的格六面邻居里只有构件占格，`CanPlaceAt` 判"缺少与地基
  相连的接触面"。

修复（`VoxelBuildComponent.cpp`／`VoxelBuildWorld.{h,cpp}`／`VoxelBuildWorldPrefab.cpp`／`VoxelBuildGrounding.cpp`）：

1. `ResolvePrefabSurfaceCell()`：命中面属于已放置构件（沿挂载链上溯到 `AVoxelBuildPrefabActor`）且
   面内侧半厘米那格在世界占格表里 → 该格当焊接基准，与体素面同一条面偏移／向上生长／预览管线。
   摆动出来的门／窗扇打在占格外，自然拒绝（幽灵照常落回贴地分支）。
2. 中心射线与 `TrySnapAssist` 辅助射线**成对**接入构件面（同一口径，预览与提交不分叉）。
3. `PrefabSupportAt()`：格的六面邻居有构件占格 → 体素带锚。三处同改：`CanPlaceAt`（预览）、
   `ApplyChanges`（提交）、`RefreshSupportGraph`（读档；故 `RefreshPrefabOccupancy()` 挪到它之前）。
4. `ScenePlacementAllowed` 障碍探针豁免"相邻构件占格里的面"：窗顶那一格中心 ±8 cm 探针必打到
   自家窗框，以前直接"位置与场景障碍重叠"——这是③的第二道锁。
5. 拆除构件／脱落后 `ReanchorVoxelsAround()`：腾出格外壳一圈的体素重判锚定（只降不升），
   只锚在门上的墙会按既有流程失去地基→倒塌，不留隐形承重。
6. 顺手修的同族缺陷：`CanPlaceInVolume`／`CanCommit` 用**本地格**查世界占格表（自由体积错位）；
   `AimedPrefab` 只在构件模式刷新，切回体素后陈旧指针会被右键拆除优先消费；
   仰视门框侧面"向上叠一层"命中同一构件的竖直边柱永远红 → `PrefabColumnTop()` 落到占格顶之上。

验证：编辑器关闭时全量 `Result: Succeeded`（无新警告）；最后一批（`PrefabColumnTop`＋生长守卫）经
**Live Coding 编译成功**（编辑器重启占用期间），退出编辑器重开时会自动落进正式 DLL。
**未实机实测**：放门靠墙续墙、窗顶放块、幽灵吸附、拆门后悬空墙倒塌、存档往返，由用户测试。
