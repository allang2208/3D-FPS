...
2026-09-13：新增仅使用免费素材的程序河流、鹅卵石河滩和河石，沿用本地图与传送门，详见[河流初版](temperate-rivers.md)。该扩展会根据原种子在河道范围调整地形；旧的纯植被布局／高度记录不代表加入河流后的结果。
...
2026-09-13：入口显示从 game-dev 迁移的全屏加载页，切图使用 MoviePlayer；地图载入后继续覆盖画面并阻止游戏输入。出生区域碰撞就绪后先准备角色和视野，全部选用植被资源、附近 PCG 格子、树干碰撞、渲染数据和待编译 PSO 准备完成后才放行。加载页显示实际阶段、进度、等待时间，可按 Esc 或按钮取消／返回主场景。详见[后续卡顿修复](temperate-hills-transit-loading-20260913.md)。
...
2. 启动先提交出生点周围 3×3 格，异步碰撞就绪后生成加载阶段的玩家，移动和游戏输入继续锁定；初始视野圈准备完成后才放行。高精度地形半径 160 m，已加载格保留额外 64 m 缓冲；粗地形半径 384 m，超出再加 64 m 才卸载。最多两个后台网格任务，每帧最多提交一个网格、卸载一个格子。只后台计算数值和网格，组件操作在游戏线程执行，碰撞使用异步烹饪。前方地形未就绪时暂缓角色移动。谷地和出生点周围保留空地。
...
6. 加载页期间依次异步载入草、灌木、岩石、四种黑杨和雾体，同时等待派生资源构建；全部完成后再启用四层 PCG。`DA_TemperateHillsStreaming` 全部环境资源为软引用；成功加载的选用资源由 GameInstance 保留，往返门不重复卸载，退出游戏释放。地形与植被实例仍按距离卸载。树干碰撞仅在 96 m 内逐格建立，超过 128 m 逐格卸载。九个低雾候选点只激活 140 m 内最多三处，超过 180 m 卸载。地表仍混合草苔、草土和碎石，并读取现有天气管理器的地表湿度。
7. 本地图将 PCG 每帧时间目标设为 2 ms、同时生成组件上限设为 2、初始格子池设为 16；异步加载／流送注册／注销的时间目标分别为 2／2／1 ms，离开时恢复原值。正常游戏启动不再计算整片布局的审计哈希。上述时间是调度目标，不是实测帧耗时或加载速度承诺。
