...
这是单机、可行走的植被样地。地形当前在开始时全部建立，尚未实现大世界地形流送和远景 LOD；仅植被使用 PCG 按距离调度。世界存档只保存种子、版本与世界 ID，尚无砍树、采集、建筑、怪物导航和这些行为的持久化。日夜天气与事件栏已接入，见[丘陵天气接入说明](../WeatherTemperateHills-20260912.md)；画面效果与树木风动由用户测试。地图边缘是样地边界，尚无正式世界边界玩法。
...
此地图已可从主场景传送门或上方启动脚本进入，尚未接入主菜单。完整大世界、地形流送、性能及打包验收不属于本次已完成范围。
