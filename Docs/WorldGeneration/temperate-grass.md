...
2026-09-13，UE 5.8.2。接入现有温带丘陵与传送门，不重建地图或更换河流地表。当前版本按用户反馈改为偏黄的干草色，并增加不规则团聚和空隙。
...
- 底层的 `GrassSpacingCm=55` 作为平均候选密度参数：每个 110 cm 小区域随机产生 2–6 个候选，位置在区域内完全随机，不再每个 55 cm 格子固定放一株。基础概率 0.94 还会乘草甸、树冠、坡度与边缘权重，并非最终地表覆盖率。
- 点缀的密度参数为 180 cm，每个 360 cm 区域独立随机产生 2–6 个候选，基础概率 0.5。底草缩放 0.78–1.30，高草 0.65–1.30，同时包含每株随机量和所在草团的相关变化。
- 约 36 m 的草甸疏密叠加约 3.2 m 的草团噪声，生成密草团、稀疏连接带与自然小空隙。底层局部概率权重由原先接近恒定的 0.88–1 改为 0.28–1，高草向密草团集中。
- 使用约 25 m 尺度、每轴最大 6.5 m 偏移的场扭曲草甸与 40 m 物种区域的边缘。附近草仍共享少数主导草型，但边缘不沿矩形区域排布；同一世界种子可复现候选、位置、大小和朝向。
...
- 自有草材质复制原主材质并增加 PerInstanceFadeAmount 遮罩淡出。保留第一级逐叶风动，实例关闭第二、三级风动和演示角色的三个弯折等级；不新增角色追踪器。
- 保留原纹理明暗细节，将叶片基色和透光色同时重映射为麦秆黄、卡其及少量灰橄榄色。`MeadowDryness=0.92`、`MeadowColorVariation=0.16`，叠加每实例及世界位置的色调变化，避免统一绿色和统一黄色；基础亮度 0.85、饱和度 0.65、透光强度 0.55。
...
草色在 `/Game/WorldGeneration/TemperateHills/Grass/MI_Meadow_*` 材质实例调整：`MeadowDryness` 控制原草色与干草色之间的混合，`MeadowDryTint` 与 `MeadowOliveTint` 控制两类色调，`MeadowColorVariation` 控制每丛差异。原包材质和纹理保持原样，调整仅作用于丘陵的自有副本。
