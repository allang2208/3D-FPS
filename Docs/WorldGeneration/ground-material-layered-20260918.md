# 丘陵地表材质：分层、凹凸与细节（2026-09-18 基线）

> 2026-09-20 更新：当前实现见 [地表材质优化](ground-material-optimization-20260920.md)。本文保留首次实现与当时编译记录；其中共享尺度的高度步进、whiteout 法线、固定采样开销和步数默认值已被新版本替换。历史编译记录不代表新版本已完成实机验收。当前重建须在已打开的编辑器内执行，默认不运行本文的历史校验流程。
...
- 只出现在坡度缓于约 38°、**约 250 m 干燥斑块噪声**偏高、坡面中等、且离开道路 300 cm 以上的地面；出生点 16 m（1600 cm）内与河岸（`Bank > 0.30`）不生成；
