...
## 8. 第四轮强化：光晕层、随弹光源、更长默认段（2026-09-21 深夜，用户"继续强化"）

方向被认可后又加了三件：

1. **光晕层**：每条光段现在两层——核心层（原样）加一个**同材质、更宽更暗**的第二层（`HaloMesh` + `HaloMaterial`）。加色混合下"宽而暗"天然读成外发光，所以不需要新材质资产。`HaloWidth` 默认 2.8 倍宽、`HaloEmission` 默认 0.55；宽度 ≤1 倍或亮度 0 即整层隐藏。
2. **随弹光源**：两盏**不投影**的点光源跟着"离枪口最近的两条在飞光段"（也就是刚打出去那两发）移动，光带经过时照亮地面和墙面。固定两盏，开销与射速无关；流明单位（`LightLumens` 默认 220、`LightRadiusCM` 默认 220），不投影、不参与间接光与体积散射。**组件 tick 关闭前显式熄灭这两盏灯**——否则最后一盏会一直亮到下次开火。
3. **默认段长再抬一档**：步枪 600/1400 → **800/1800 cm**，手枪 300/1200 → **400/1300 cm**（30 fps 下 350 m/s = 1167 cm/帧，仍在覆盖范围内）。

新增 CVar：`fps.Tracer.HaloWidth`、`fps.Tracer.HaloEmission`、`fps.Tracer.LightLumens`、`fps.Tracer.LightRadiusCM`（与前三轮合计十个，全部实时生效）。
