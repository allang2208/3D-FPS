...
- 观感微调入口（2026-09-21 第二轮起改为实时 CVar，全部实时生效）：`fps.Tracer.LengthScale`（段长倍率，1 = 步枪 800/1800 cm、手枪 400/1300 cm）、`fps.Tracer.Every`（每 N 发画一条曳光，默认 1，第 1 发必画）、`fps.Tracer.LingerSeconds`（命中后停留淡出秒数，默认 0.12）、`fps.Tracer.PixelWidth`（屏幕像素宽，默认 4）、`fps.Tracer.Emission`（加色自发光，默认 2.6）、`fps.Tracer.Tint`（`R,G,B`，默认 `1.0,0.30,0.03`）、`fps.Tracer.HaloWidth`（光晕层宽度倍率，默认 2.8）、`fps.Tracer.HaloEmission`（光晕层自发光，默认 0.55）、`fps.Tracer.LightLumens`（随弹光源流明，默认 220，0 = 关）、`fps.Tracer.LightRadiusCM`（随弹光源半径，默认 220 cm）、`fps.Ballistics.SpeedScale`（弹速倍率，1 = 目录 `bullet_speed`）。背景与取舍见 `Docs/Weapons/ballistic-feel-options-20260921.md`。