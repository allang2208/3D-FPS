...
本轮替换 [采集初版](ProductionTools-20260913.md) 中“木材、矿物直接进背包”的规则。保留 `6 / 7 / 8` 工具切换、3.2 米使用距离、`F7` 返回武器及原有存档。

后续变更（2026-09-25）：**树木改为生命值口径**（斧头按伤害扣血、归零才倒），本文的“第三次命中后倒下”只适用于当时的命中数口径；当前口径见 [树木生命值与按伤害伐木](ProductionTreeHealth-20260925.md)。
