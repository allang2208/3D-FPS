...
void UVoxelBuildWidget::ShowState(const FString& Material,const FString& Brush,const FString& Message,bool bValid,int32 Count,bool bSnapEnabled)
{
    const FString SelectionText=FString::Printf(TEXT("%s · 已建造 %d 格\n%s\n%s"),*Material,Count,*Brush,bSnapEnabled?TEXT("体素吸附已开启 · F 自由放置"):TEXT("自由位置 · F 开启体素吸附"));
...
void UVoxelBuildWidget::RefreshLayout()
{
    ...
    const float Height=FMath::Min(View.Y<650?250.f:310.f,FMath::Max(100.f,View.Y-40.f))/Scale;
    ...
    Controls->SetText(FText::FromString(View.Y<650?TEXT("F 吸附/自由 · 1/2 材料\n滚轮形状 · R 旋转 · 中键取样\n左键放 / 右键拆 · Ctrl+Z 撤销\nV / Esc 退出"):
        TEXT("1 木材 / 2 石头\n滚轮切换单格、地板、墙面\nF 切换体素吸附 / 自由放置\n左键放置 / 右键拆除\nR 旋转墙面 · 中键取样\nCtrl+Z 撤销 · V / Esc 退出")));
}
