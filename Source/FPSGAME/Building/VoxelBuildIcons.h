 ...
 * one camera direction and one framing rule (camera-projected bounds fill the same fraction of the
...
public:
    ...
    /** Close the drawer without rendering remaining hidden jobs or retaining scene meshes. */
    void Suspend();
    bool HasFailed(const FString& Key) const;
    static FString ContentKey(const FVoxelBuildIconRequest& Request);
    static FString KeyForShape(FName MaterialId,int32 Shape);
    static FString KeyForPiece(FName PieceId);
private:
    struct FJob {FVoxelBuildIconRequest Request;double PreparedAt=0;};
    void EnsureStudio();
    bool Prepare(const FVoxelBuildIconRequest& Request);
    ...
    TArray<TWeakObjectPtr<class UTexture2D>> WarmingTextures;
    TMap<FString,int32> Attempts;
