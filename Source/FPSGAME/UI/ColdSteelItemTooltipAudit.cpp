...
void UColdSteelHUDWidget::RunItemTooltipAudit()
{
    ...
    GetWorld()->GetTimerManager().SetTimer(R->Timer,FTimerDelegate::CreateWeakLambda(this,[this,M,R,Check]()
    {
        ...
        case 10:{auto* Storage=WarehouseWidget->Board.Get();const auto G=Storage->GetCachedGeometry();const auto L=Storage->Layout(G);const FVector2D Pt=G.LocalToAbsolute(FVector2D(12+.5f*L.Cell,L.BagY+.5f*L.Cell)/Storage->Scale);FPointerEvent E(0,Pt,Pt-FVector2D(1,0),TSet<FKey>(),EKeys::Invalid,0,FModifierKeysState());Storage->NativeOnMouseMove(G,E);Check(ItemTooltip&&ItemTooltip->IsVisible()&&ItemTooltip->ItemId()==R->Gun,TEXT("native warehouse hover uses same item inspector"));break;}
