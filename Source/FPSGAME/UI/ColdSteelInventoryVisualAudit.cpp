...
void UColdSteelHUDWidget::RunInventoryVisualAudit()
{
    ...
    struct FRun{int32 Phase=0,Checks=0,Failures=0;FColdSteelProfile Fixture;FTimerHandle Timer;FString Gun,Potion;};auto R=MakeShared<FRun>();
    ...
    for(int32 N=0;N<6;++N){auto I=P->CreateItem(TEXT("ue_m4a1"));I.Place=N<2?1:0;I.Cell=N<2?6+N*3:N==5?36:(N-2)*5;
        ...
        if(N==5){Data->SetNumberField(TEXT("enhanceLevel"),10);auto Enchant=MakeShared<FJsonObject>();auto Prefix=MakeShared<FJsonObject>();Prefix->SetStringField(TEXT("name"),TEXT("沉重"));Enchant->SetObjectField(TEXT("prefix"),Prefix);Data->SetObjectField(TEXT("_enchantData"),Enchant);}
