...
TArray<FMetric> Metrics(const FColdSteelItem& Item,UColdSteelStatusModel* Model,UGunsmithSystem* G)
{
    ...
    else if(Item.Definition==TEXT("ue_rune_sword"))
    {
        const auto* E=Model?Model->GetGameInstance()->GetSubsystem<UColdSteelEnhancementSystem>():nullptr;
        const double Rate=FMath::Clamp((Model?double(Model->Derived(TEXT("aspd"))):1.)/FMath::Max(.1,E?E->Effect(Item,TEXT("attackIntervalMul"),1):1.)/(Model?1-Model->MasteryEffect(TEXT("swordMastery")).CooldownReduction:1.),.2,4.);
        Add(TEXT("damage"),TEXT("伤害"),ColdSteelWeaponStats::Damage(Item,Model,ColdSteelInventory::Number(Item,TEXT("melee_damage"),55)),TEXT(""),2);
        Add(TEXT("melee_interval"),TEXT("攻击间隔"),RuneSwordRhythm::AttackEnd/Rate,TEXT(" s"),2,true);
        Add(TEXT("melee_stamina"),TEXT("普通攻击体力消耗"),Model?Model->StaminaSettings().MeleeCost:FColdSteelStaminaTuning{}.MeleeCost,TEXT(""),2,true);
        Add(TEXT("melee_reach"),TEXT("最大攻击距离"),RuneSwordCombatTuning::ScaledReach(ColdSteelInventory::Number(Item,TEXT("melee_reach_cm"),180),RuneSwordThrustRhythm::ReachBonus)/100,TEXT(" m"),2);
        Add(TEXT("block_reduction"),TEXT("格挡伤害减免"),(1.f-RuneSwordGuardTuning::DamageTakenRatio)*100,TEXT("%"),2);
    }
    if(Model)
    {
...
void CompleteColdSteelTooltipSummary(const FColdSteelItem& Item,UColdSteelStatusModel* Model,UGunsmithSystem* G,FColdSteelTooltipContent& Out)
{
    ...
    Out.bWideIcon=Weapon||Item.Definition==TEXT("ue_rune_sword");
    ...
    if(Item.Definition==TEXT("ue_rune_sword"))Out.ValueScope=TEXT("按当前角色加成计算；伤害未计要害和目标护甲。最大距离含突刺。");
