...
TSharedRef<SWidget> UM4GunsmithWidget::BuildWorkbench()
{
    ...
    auto Header=SNew(SHorizontalBox)
        +SHorizontalBox::Slot().FillWidth(1)[SNew(SVerticalBox)+SVerticalBox::Slot().AutoHeight()[Label(TEXT("装备改造  /  装配工作台"),26)]+SVerticalBox::Slot().AutoHeight().Padding(0,5,0,0)[Label((W?W->Name:TEXT("武器"))+(W&&W->Ammo==TEXT("ammo_357")?TEXT("   /   .357 Magnum"):W&&W->Ammo==TEXT("ammo_45acp")?TEXT("   /   .45 ACP"):W&&W->Ammo==TEXT("ammo_58")?TEXT("   /   5.8 mm"):W&&W->Ammo==TEXT("ammo_762")?TEXT("   /   7.62 mm"):TEXT("   /   5.56 mm"))+TEXT("   /   选择配件预览，应用后保存"),14,ColdSteelUI::TextSecondary)]]
