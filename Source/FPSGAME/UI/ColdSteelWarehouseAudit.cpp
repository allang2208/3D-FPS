...
#include "ColdSteelWarehouseSpatialAudit.h"
...
void UColdSteelHUDWidget::RunWarehouseAudit()
{
    ...
    if(FParse::Param(FCommandLine::Get(),TEXT("WarehouseColdGlassAudit"))){RunWarehouseGlassAudit();return;}
    ...
    Check(M->TransferWarehouse(Gun.InstanceId,4,Capacity-ColdSteelWarehouse::CellsPerPage)&&!M->Equipped()&&M->FindItem(Gun.InstanceId)->Magazine==7,TEXT("equipped direct store despite full backpack preserves ammo"));
    ...
    auto Invalid=M->Snapshot();Invalid.Items[0].Place=4;Invalid.Items[0].Cell=Capacity;FString Reason;Check(!Validate(Invalid,Reason),TEXT("out of bounds warehouse save rejected"));
    ...
    Check(M->TransferWarehouse(Two.InstanceId,4,8)&&M->Items().Num()==2&&M->FindItem(One.InstanceId)->Count==Potion.StackMax&&M->FindItem(Two.InstanceId)->Count==4&&M->FindItem(Two.InstanceId)->Place==0,TEXT("explicit stack merge fills target and retains exact remainder at source"));
    ...
    auto Packed=Empty;for(int32 C=0;C<Capacity;++C){auto I=M->CreateItem(TEXT("hp_potion"),Potion.StackMax);I.Place=4;I.Cell=C;Packed.Items.Add(I);}Packed.Items.Add(A);M->CommitState(Packed);Before=Signature(M->Items());
    ...
    Check(!M->TransferWarehouse(Extra.InstanceId,4)&&Signature(M->Items())==Before,TEXT("automatic transfer rolls back partial merge when remainder has no space"));
    ...
    Check(M->CountWarehouseMaterial(IsPotion)==Capacity*Potion.StackMax,TEXT("predicate material count excludes backpack"));
    ...
    Check(M->ConsumeWarehouseMaterial(IsPotion,Capacity*Potion.StackMax+1)==Capacity*Potion.StackMax&&M->CountWarehouseMaterial(IsPotion)==0,TEXT("predicate consumption reports partial amount without underflow"));
    M->CommitState(Empty);M->WarehousePage=4;Check(M->AddWarehouseItem(SmallWeapon())&&Owner(M->Items(),4,4*ColdSteelWarehouse::CellsPerPage)>=0,TEXT("external reward uses current page free rectangle"));
    ...
    GetWorld()->GetTimerManager().SetTimer(R->Timer,[this,R,M,Check,Signature,WeakChest]() mutable {
        ...
        if(R->Phase==-1){
            ...
            if(Board&&WarehouseWidget->Board){
                auto* Storage=WarehouseWidget->Board.Get();const auto SG=Storage->GetCachedGeometry();const auto SL=Storage->Layout(SG);
                const FVector2D Point=SG.LocalToAbsolute(FVector2D(12+.5f*SL.Cell,SL.BagY+.5f*SL.Cell)/Storage->Scale);
                UDragDropOperation* Operation=nullptr;FPointerEvent Pointer(0,Point,Point,TSet<FKey>{EKeys::LeftMouseButton},EKeys::LeftMouseButton,0,FModifierKeysState());
                Storage->NativeOnMouseButtonDown(SG,Pointer);Storage->NativeOnDragDetected(SG,Pointer,Operation);auto* Drag=Cast<UColdSteelItemDrag>(Operation);
                if(Drag){const FString Id=Drag->ItemId;const auto Geometry=Board->GetCachedGeometry();const auto Layout=Board->Layout(Geometry);const FVector2D Screen=Geometry.LocalToAbsolute(FVector2D(14,Layout.BagY+2+2*Layout.Cell)/Board->Scale);
                    ...
                    Drag->Drop(P);Check(Storage->DraggedItem.IsEmpty(),TEXT("successful drag restores source presentation"));
                    const auto* I=M->FindItem(Id);if(I){Drag->SourcePlace=I->Place;Drag->SourceCell=I->Cell;Drag->SourceSnapshot=*I;}
                    FDragDropEvent ReturnEvent(Pointer,TSharedPtr<FDragDropOperation>());
                    Check(Storage->NativeOnDrop(SG,ReturnEvent,Drag)&&M->FindItem(Id)->Place==4,TEXT("native backpack drop restores original warehouse instance"));
            ...
            const auto W=WarehouseWidget->GetCachedGeometry(),B=InventoryPanel->GetCachedGeometry();Check(W.GetAbsolutePosition().X+W.GetAbsoluteSize().X<=B.GetAbsolutePosition().X&&W.GetAbsoluteSize().Equals(B.GetAbsoluteSize(),2),TEXT("equal drawers remain separated and inside viewport"));
