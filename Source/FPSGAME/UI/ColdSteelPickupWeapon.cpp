...
bool AColdSteelPickup::BuildWeapon(const FColdSteelItem& Item)
{
    if(Item.Definition!=TEXT("ue_m4a1")&&Item.Definition!=TEXT("ue_akm"))return false;
    // Reuse the assembly contract in a preview world; the character explicitly
    // skips gameplay initialization if spawned during another actor's BeginPlay.
    // Only the frozen weapon and its visible attachments enter the gameplay world.
    FPreviewScene Studio(FPreviewScene::ConstructionValues().SetEditor(false).SetCreatePhysicsScene(false).SetTransactional(false));
    ...
    auto* Rig=Studio.GetWorld()->SpawnActor<AFPSGAMECharacter>(FVector::ZeroVector,FRotator::ZeroRotator,Spawn);if(!Rig)return false;
    // End component play while the preview world still exists, including early
    // asset failures. FPreviewScene teardown otherwise leaves this to later GC.
    ON_SCOPE_EXIT { Rig->Destroy(); };
    Rig->SetActorTickEnabled(false);Rig->SetActorEnableCollision(false);Rig->bUseM4Infima=Item.Definition==TEXT("ue_m4a1");Rig->InitializeWeaponVisuals();
    ...
    FTransform Pose(Align);FBox Bounds(ForceInit);const auto& LOD=Render->LODRenderData[0];
