...
bool UColdSteelStatusModel::StageProductionDrops(FColdSteelProfile& State,const FProductionResource& Target,TArray<FString>& Ids)
{
    ...
    for(const auto& Definition:RewardDefinitions)
    {
        ...
        for(int32 N=0;N<Pieces;++N)
        {
            ...
            if(Target.Layer==0)Position+=Forward*(120+Index*95)+Side*Random.FRandRange(-28,28);
            ...
            Position.Z=World->Height(Position.X,Position.Y)+45;
            const float Yaw=Forward.Rotation().Yaw+(Target.Layer==0?-90:Random.FRandRange(-180,180));
