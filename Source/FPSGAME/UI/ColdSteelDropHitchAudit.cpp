...
void UColdSteelHUDWidget::RunDropHitchAudit()
{
    ...
    GetWorld()->GetTimerManager().SetTimer(R->Timer,FTimerDelegate::CreateWeakLambda(this,[this,M,R,Check](){
        ...
        switch(R->Phase++){
        case 0:SetInventoryTab(false);SetInventoryOpen(true);Scroll->ScrollToEnd();break;
        ...
        case 6:R->Outside=GetCachedGeometry().LocalToAbsolute(FVector2D(2,GetCachedGeometry().GetLocalSize().Y*.4));Click(R->Outside);Check(!bInventoryOpen&&!PC->IsMoveInputIgnored(),TEXT("outside click closes panel without leaking game input"));SetInventoryOpen(true);Scroll->ScrollToEnd();break;
        case 7:R->Start=G.LocalToAbsolute(FVector2D(12+L.Cell*2.5,L.BagY+L.Cell*.5)/Board->Scale);UE_LOG(LogTemp,Display,TEXT("DropHitchFixture: start=%s visible=%d scroll=%.1f"),*R->Start.ToString(),Scroll->GetCachedGeometry().IsUnderLocation(R->Start),Scroll->GetScrollOffset());App.ProcessMouseMoveEvent(Pointer(R->Start,R->Start,false));App.ProcessMouseButtonDownEvent(GEngine->GameViewport->GetWindow()->GetNativeWindow(),Pointer(R->Start,R->Start,true,EKeys::LeftMouseButton));break;
        ...
        case 15:Click(R->Outside);Check(!bInventoryOpen&&!PC->bShowMouseCursor,TEXT("outside click closes backpack through detached item popup"));SetInventoryOpen(true);Scroll->ScrollToEnd();break;
        case 16:R->Start=G.LocalToAbsolute(FVector2D(12+L.Cell*2.5,L.BagY+L.Cell*.5)/Board->Scale);App.ProcessMouseMoveEvent(Pointer(R->Start,R->Outside,false));App.ProcessMouseButtonDownEvent(GEngine->GameViewport->GetWindow()->GetNativeWindow(),Pointer(R->Start,R->Start,true,EKeys::LeftMouseButton));break;
