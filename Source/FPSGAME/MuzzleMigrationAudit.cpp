...
void AFPSGAMECharacter::RunMuzzleMigrationAudit()
{
    ...
    static const bool bAKM=FParse::Param(FCommandLine::Get(),TEXT("AKMMuzzleAudit"));
    static const FString WeaponId=bAKM?TEXT("ue_akm"):TEXT("ue_m4a1");
    if(bAKM){auto S=P->Snapshot();S.Items.Reset();S.Hotbar.Init(TEXT(""),4);S.HotbarDefinitions.Init(TEXT(""),4);auto W=P->CreateItem(WeaponId);W.Place=1;W.Cell=9;W.Magazine=17;S.Items.Add(W);auto A=P->CreateItem(TEXT("ammo_762"),100);A.Place=0;A.Cell=0;S.Items.Add(A);S.ActiveWeaponSlot=9;P->CommitState(S);ApplyColdSteelProfile(P);}
    const FString Dir=FPaths::ProjectSavedDir()/(bAKM?TEXT("AKMMuzzleAudit"):TEXT("MuzzleMigrationAudit"));IFileManager::Get().MakeDirectory(*Dir,true);
    ...
    UAudioMixerBlueprintLibrary::StartRecordingOutput(this,34.f);
    const TArray<FString> Variants={TEXT("true"),TEXT("brake"),TEXT("titanium_brake")};
    const float Recoils[]={75,85,70},Shakes[]={75,85,80},ADS[]={bAKM?.30f:.24f,bAKM?.33f:.264f,(bAKM?.30f:.24f)/.85f};
    for(int32 Index=0;Index<3;++Index)
    {
        const FString V=Variants[Index];const float T=bAKM?2.f+Index*8.f:Index*6.f;const float Recoil=Recoils[Index],Shake=Shakes[Index],AimTime=ADS[Index];
        Later(T+.1f,[this,P,G,PC,Check,V,Recoil,Shake,AimTime](){
            ...
            if(bAKM)Panel->ChooseOption(TEXT("optic"),V==TEXT("true")?TEXT("panoramic_red_dot"):V==TEXT("brake")?TEXT("prism_scope_2x"):TEXT("lpvo_1_6x"));
            ...
            const auto S=G->Calculate(WeaponId,G->Draft());
            ...
            if(bAKM){const FVector Axis=AKMViewmodel->GetSocketTransform(TEXT("WPN_root")).GetRotation().GetAxisY();Check(FVector::DotProduct(GetEffectiveMuzzleForward(),Axis)>.9999f,V+TEXT(" AKM muzzle follows barrel axis"));}
            else Check(Hidden,V+TEXT(" factory muzzle hidden without hiding barrel"));
        ...
        Later(T+1.6f,[this,Check,Dir,V](){
            const FVector Forward=GetEffectiveMuzzleForward();const FVector Original=bAKM?AKMViewmodel->GetSocketTransform(TEXT("WPN_root")).TransformPosition(AKMSoviet::Muzzle):AKMViewmodel->GetSocketLocation(TEXT("WPN_SOCKET_Muzzle"));
            ...
            // AKM source lengths 18.6 / 6.8 / 7.25 cm, rear mount Y=57.7 cm;
            // the Soviet barrel tip is Y=58.02467465 cm (not the M4 socket).
            const float ExpectedAKM=(V==TEXT("true")?18.6f:V==TEXT("brake")?6.8f:7.25f)+57.7f-58.02467465f;
            Check(Error<.15f&&(bAKM?FMath::IsNearlyEqual(Extension,ExpectedAKM,.05f):(Extension>1.f&&Extension<15.f)),V+TEXT(" bore axis and measured tip extension"));
        ...
        Later(T+2.12f,[this,Check,Input,V](){Input(EKeys::LeftMouseButton,IE_Released);Input(EKeys::RightMouseButton,IE_Released);
            if(bAKM)Check(FireSound&&SuppressedFireSound&&IsMuzzleSuppressed()==(V==TEXT("true")),V+TEXT(" AKM fire sound assets and suppressor routing available"));
            else Check(M4FireVoice&&M4FireVoice->Sound==(V==TEXT("true")?SuppressedFireSound:FireSound),V+TEXT(" real fire selects correct sound"));
        ...
        Later(T+(bAKM?7.f:5.6f),[this,P,G,Check,V](){Check(P->ReloadProfile()&&G->Installed(*P->Equipped()).FindRef(TEXT("muzzle"))==V&&MuzzleVariant==V,V+TEXT(" reload and saved instance preserve attachment"));});
    }
    Later(bAKM?26.2f:18.2f,[this,P,G,PC,Check](){
    ...
    Later(bAKM?30.f:22.f,[this,Counts,Lines,Dir](){UAudioMixerBlueprintLibrary::StopRecordingOutput(this,EAudioRecordingExportType::WavFile,TEXT("MuzzleAudio"),Dir+TEXT("/"));FFileHelper::SaveStringToFile(*Lines,*(Dir/TEXT("assertions.log")));UE_LOG(LogTemp,Display,TEXT("MUZZLE_MIGRATION_COMPLETE checks=%d failures=%d"),Counts->X,Counts->Y);});
    Later(bAKM?32.f:24.f,[Counts](){FPlatformMisc::RequestExitWithStatus(false,Counts->Y?1:0);});
}
