...
{if(Active.Get()!=Volley)return;Active.Reset();bQueuedRelease=false;if(auto* H=Hands())H->CancelSpellGesture(this);}
void UFPSIceSpikeComponent::Cancel()
{bQueuedGather=bQueuedRelease=false;if(Active.IsValid())Active->Destroy();Active.Reset();if(auto* H=Hands())H->CancelSpellGesture(this);}
