...
const FColdSteelSkillDefinition& UColdSteelStatusModel::MasteryDefinition(FName Id) const
{
    ...
    static const TMap<FName,FColdSteelSkillDefinition> AdditionalDefinitions=[] {
        TMap<FName,FColdSteelSkillDefinition> Out;for(FName Key:{FName(TEXT("swordMastery")),FName(TEXT("machineGunMastery")),FName(TEXT("shotgunMastery")),FName(TEXT("bowMastery")),FName(TEXT("heavyStrike"))})Out.Add(Key,ColdSteelSkills::LoadDefinition(Key));return Out;}();
