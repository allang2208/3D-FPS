...
struct FStormAudit
{
    ...
    void Tick()
    {
        ...
        if(Step==72)
        {
            ...
            Check(FMath::IsNearlyEqual(Coverage,.045f,.0001f)&&Density>0&&
                FMath::IsNearlyEqual(Density,FMath::Max(.0001f,Clouds->StormCloudDensity),.0001f),TEXT("overcast retains nonzero cloud extinction"));
