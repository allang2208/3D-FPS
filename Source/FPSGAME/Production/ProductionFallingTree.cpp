...
void AProductionFallingTree::InitializeFall(const FProductionResource& Resource,const FVector& Direction)
{
    ...
    Tree->SetSkeletalMesh(Cast<USkeletalMesh>(ProductionHarvestAssets::FallingMesh(ProductionHarvestAssets::TreeVariant(Resource.Mesh)).ResolveObject()));
    ...
    for(int32 Index=0;Index<Tree->GetNumMaterials();++Index)
        if(auto* Source=Tree->GetMaterial(Index))
        {
