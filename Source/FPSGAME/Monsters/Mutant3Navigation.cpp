...
bool AMonsterAIController::NavigateFeralTo(FVector Destination, float Acceptance, APawn* VisibleTarget)
{
    ...
    const bool Blocked = Moving && Mutant->GetVelocity().Size2D() < 25.f;
    ...
    const FVector Feet = Mutant->GetNavAgentLocation();
    ...
    const FVector CenterOffset = Mutant->GetActorLocation()-Feet;
    FNavLocation Goal;
    ...
    if ((!Blocked || !VisibleTarget) && Projected)
    {
        ...
        if (VisibleTarget && !Mutant->CanClawFrom(VisibleTarget,
            Goal.Location+CenterOffset, Mutant->GetClawStartDistance())) Reach = 5.f;
        ...
        if (Result == EPathFollowingRequestResult::AlreadyAtGoal &&
            (!VisibleTarget || Mutant->CanClawFrom(VisibleTarget,
                Mutant->GetActorLocation(), Mutant->GetClawStartDistance())))
        {
            bNavigationFailed = false;
            return false;
        }
    ...
    if (IsValid(VisibleTarget))
    {
        ...
        if (Away.IsNearlyZero()) Away = -Mutant->GetActorForwardVector();
        const float RingRadius = FMath::Max(65.f, Mutant->GetClawStartDistance()-40.f);
        const auto* Capsule = Mutant->GetCapsuleComponent();
        FCollisionQueryParams Params(SCENE_QUERY_STAT(Mutant3Approach), false, Mutant);
        ...
        for (float Angle : {0.f, 45.f, -45.f, 90.f, -90.f, 135.f, -135.f, 180.f})
        {
            ...
            if (!Nav->ProjectPointToNavigation(Candidate, Point, FVector(65,65,60), Data) ||
                !Mutant->CanClawFrom(VisibleTarget, Point.Location+CenterOffset, Mutant->GetClawStartDistance()-10.f)) continue;
            ...
            FHitResult Hit;
            if (GetWorld()->SweepSingleByChannel(Hit, Mutant->GetActorLocation(),
                Mutant->GetActorLocation()+Step, FQuat::Identity, Capsule->GetCollisionObjectType(), Shape, Params, Response)) continue;
