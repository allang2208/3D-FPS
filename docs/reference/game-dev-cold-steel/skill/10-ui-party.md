...
- **时代科技徽章交付边界**：以已选建筑为主体、既有冷钢六边形为母版；检查建筑地台、旗杆和塔楼是否压住外框，必要时缩小内部主体而非裁掉构件。请求透明和1024尺寸不代表实际输出满足要求；交付前读取真实色彩模式/尺寸，RGB中的黑底或棋盘格不能标为RGBA。保留原图、编辑祖先和提示词，透明成品准备好后才接入 `iconPath`。时代插档后同步检查徽章等级标记，不能把旧三晶图默认解释成IV级；未处理事项写TODO，不伪报完成。

### Loading可见性与主题样式合同

- DOM从行内样式迁到`loading-*`类后，须同时保留主题CSS的全屏fixed/inset、flex居中、尺寸、背景center/cover、进度轨道与文字样式；只有DOM和进度更新代码仍可能看不到界面。
- 载入遮罩的层级须与现有主菜单/模态层级核对；优先消费统一层级变量，避免低于仍显示的菜单，也不随意增加整套重复覆盖样式。先分清“无插画”“遮罩被盖住”“入口没调用”，再限定修改范围。
- Loading插画由DOM背景加载，不属于Phaser世界纹理；接入图片不修改纹理驻留、地图渲染或战斗生命周期。已有完整样式的发布基线保留自身实现，不把另一工作区的整段主题CSS覆盖过去。

### 新游戏静态序章与入口选择合同（2026-09-03）

- 主界面“开始游戏”在读取游戏尚未开放时直接进入新游戏，避免再弹出只有一个可用选项的确认层；存档系统开放后才恢复“新游戏/读取游戏”选择层。“新游戏”是玩家显式重开入口，必须用 `force: true` 播放序章，不能被历史观看标记短路；当前固定顺序为“静态序章完成或SKIP → 小鼠大王GAL式剧情引导完成或跳过 → 唯一的`Game.start()`”，不得提前启动世界再覆盖画面。
- 静态分镜使用两个全屏 `img` 双缓冲交叉切换，镜头轻推、闪电、雨线、能量与最终白场只写在主题CSS；雨线必须用远近独立周期、不同长度/速度/透明度的连续渐变雨丝在屏外无缝续接，禁止回退为短周期重复斜纹或整层同步回卷。不把对白框、字幕或噪点颗粒叠回已确认的纯画面叙事。正式帧放在 `assets/scenes/opening/`，未选稿、重复预览和中间缓存不进入Git。
- 左键仅在序章遮罩内推进一幕，SKIP按钮自行拦截，避免一次点击同时推进和结束；自动计时、左键加速、图片加载失败兜底共用同一帧令牌，旧图片回调不得覆盖新幕。最后一幕先全白再完成，SKIP可快速淡出。
- 序章打开时把 `#gameContainer` 设为 `inert`，限制Tab到SKIP并阻止指针穿透；结束时恢复进入前的 `inert` 状态、清除定时器/音效/预加载器并移除遮罩。入口选择层同样冻结底层菜单并负责焦点返回，Esc、遮罩空白点击和按钮禁用态均须可预测。
- 序章后的剧情引导使用独立`OpeningDialogue`全屏层，背景可复用已确认的主神空间分镜，立绘复用小鼠大王正式`portrait`；小鼠大王称号与现有形象保持不变，世界观解释统一为“原本是普通仓鼠，经历轮回与修炼后化形成为小鼠大王”。它不调用世界内`NPCDialogue`，不提前生成玩家/NPC，也不复制任务或存档状态。GAL对白只负责世界观、身份与下一项剧情目标，不讲WASD、鼠标或战斗按键；基础操作必须在玩家进入可控制世界后由常驻任务条结合真实目标讲解。逐字阶段单击只补全文，再次单击才前进；按钮、画面单击、Enter/Space/右方向键、Esc跳过与Tab焦点循环必须走同一一次性生命周期。引导结束后才释放底层`inert`并进入游戏启动链。
- **跳过全部教程分流（2026-09-05）**：GAL右上角保留“跳过剧情”（只结束当前对话并正常开始教程），另设“跳过所有教程”二次选择。两条跳过路线都发放当前商店一枚F钥匙的购买价200金币：自由探索路线的大地图仍按正式进度解锁；直接首城路线把首城状态推进到`awaiting_king`并声明`skipAuthorized`，只把首次教学地牢视作首城资格，不连带放开其他系列/等级门槛，仍需与小鼠大王交谈和手动确认正式候选，不登记地牢胜利、不发经验/掉落。两条路线都通过QuestStore归档首次探索与首城建设教程，不能只隐藏UI留下任务后台继续触发。

### 市政厅治理详情与社会政策草稿

- 通用建筑详情中的市政厅使用独立子视图，入口按建筑身份分流，普通建筑升级卡不受影响；冷钢颜色与20/16/14/12字阶仍只由主题CSS提供。
- 位面总览区分本地仓储与全局金币，压缩仓储使用真实计容接口；岗位管理调用人口系统的增减/补满接口，保留人口上限、运输交接和建筑生命周期，禁止只改显示数字或直接写岗位字段。
- 政策大面板经懒加载进入独立BasePanel模态，避免Input进入建筑启动依赖链。图中节点可查看前置、时代、成本、状态和收益草案；未实装的收益持续标注“规划未生效”，不可伪装成已获得Buff。
- 草稿与已保存规划分离：模拟采纳检查前置/时代/预算；加入目标按拓扑补齐前置；调整顺序不能越过依赖，退款/移除处理后续依赖；保存只写基地纯数据，取消丢弃未保存改动。
- 模态接线同时覆盖UIState、鼠标世界边界、RTS快捷键/边缘平移及浏览器/Electron Esc；限制焦点遍历，动作前重查原建筑与位面。切场/建筑失效清理模态，关闭清理刷新定时器、焦点监听与图片。
- 总览数值与岗位行增量更新，保留搜索、筛选和滚动；世界照常运行。数据正确、源码接线与实机UI验收分开记录，详细已完成/待办和发布状态只写交付文档、CHANGELOG/TODO。

### 战略出征与大地图冷钢交互

- 指挥建筑先开原详情，再由“出征”按钮进入独立准备模态。取消返回仍有效的原建筑详情及入口焦点；确认调用原战略出征入口，不直接加载目标城或复制兵力。名册资格统一遵循第06卷“出征名册”。
- 普通控件消费冷钢主题：浅银主按钮仅用于确认出征，刷新/编入/取消为深灰次按钮，清空为弱按钮；筛选选中用深底银线，HP/危险才用语义色。字体20/16/14/12，数字用主题等宽字体，不在业务层复制色板。
- 名册每秒按稳定ID增量刷新，保留仍有效的选择；只在面板打开期间计时，关闭释放卡片和计时器。确认前重读名单；世界时间照常流逝，已打开的模态不免除建筑损毁、单位离场或玩家死亡的校验。
- 桌面名册/说明分区滚动，窄屏中段单列，标题与操作固定；焦点遍历包含可见折叠控件。打开/关闭清理按住输入，沿用BasePanel、模态挂载和状态键，阻止Esc/射击/RTS快捷键穿透。
- 大地图优先展示地图与明确行动反馈；删除用户否定的位面介绍和敌城全量列表时，同时移除入口、展开状态和事件绑定，不保留空按钮。过时复制面板脚本和截图不作为新UI效果证据。

### 地牢独立探索台与固定通道布局

- 所有主题复用`DungeonExplorationConsole`与`mapLayout: exploration`，只按配置切换环境图，不复制UI、生成器或可达判定。主题覆盖与当前图片数量见`docs/dungeon-exploration-console-v9.md`，阶段进度不写入本节。
- 冷钢材质消费主菜单/背包`--bp-ui-shell/control/surface-hover`及10px外壳圆角，字体20/16/14/12、数字Consolas；安全撤离用成功绿，强制放弃用危险语义。只维护既有`dungeon-exploration-v9/v10/v11`块，不重复叠加整套样式。
- 原生命/魔法/等级、经济面板与时钟使用正常模式同一DOM、尺寸、位置、数据和交互；`game-style.css`只为探索态豁免`.top-right-hud`的地图隐藏规则。探索层低于原`#uiLayer`，旧布局保留原安全区，不为菜单预留不对称空白。
- 背景与台面共用上下两行Grid，默认40vh/60vh、台面区域保留16px底边距；图片`object-fit: cover; object-position: center`等比铺满，94%不透明度，不拉伸、不读取Canvas缓存高度拼接。拖动、方向键12px微调、Home/End和双击还原共同调整首行，通常保留至少360px台面；矮窗允许滚动，取色只在创建时读取。
- `mapExplorationBackgroundVariants`独立于旧地标`mapBackgroundVariants`：路线生成后、创建探索台前每局等概率抽一次并缓存，往返房间/调整布局不重抽，离场清理；未配置时回退`mapBackground`。仅使用正式同主题纯场景图，禁止含内置黑带或UI的底图；data/public配置同步，背景不参与拓扑。
- 左右共用12～24px边距，队伍与档案等宽且宽屏上限288px。底栏同一网格：左绿撤离、中央浅银进入、右侧放弃；两侧等宽等高且上限260px，按钮最小56px、操作带84px（矮窗72px），窄窗主按钮单独一行。收益跨整台，默认真实规则摘要、可展开原五项明细，明确非已获得。
- 按生成器全图row固定196×112网格、缺行留空，横竖直连优先、实际遮挡才避让，双向边去重。探索态保持连续全图，不按窗口分页或逐列重排；legacy/split/landmark分支、正式母图及模板保留。
- DOM节点、Canvas、命中及拖动共用内层实测视窗；常规节点48px、缩放10%～160%，低于60%隐藏细字。完整路线拟合全图；手动“当前位置”以80%～100%围绕当前位置及真实邻居，战斗返回保留缩放。窗口/台面/收益调整只补偿观察中心并钳制，不重置视角。
- 节点、编号与名称由同一`overflow: clip`视窗逐步裁切，禁止靠边整块隐藏。图标与视窗相交时才进入Tab顺序，完全离开的可走图标计入文字提示并可从下拉定位。探索态按真实房间钳制：屏幕余量水平60px/垂直56px随节点缩放且不超过各轴视窗1/4；触边按实际位置续拖，反向立即响应。旧绕线范围与80世界单位余量仅留给旧模式/完整路线拟合。
- 队员卡只读玩家和`PartySystem`实体data，保留原容量、64px头像、最小96px卡、数字下方血魔长条；单击/Shift选择、空位招募走原入口，空间不足时滚动，低于1100px改横向卡列。订阅成员变化并沿渲染节拍每250ms补刷，不常驻定时重建；销毁取消订阅。`RecruitUI.isOpen`阻断探索操作，招募模态圈定焦点、关闭恢复入口，释放事件仍传递清理输入状态。
- 右边缘16px鼠标悬停展开正常模式原`.side-menu`，不新增菜单按钮或复制栏目；栏目及文字周围保持展开，离开180ms收起。拖动不触发，尊重`.hidden`；悬停不抢焦点，仅焦点已在栏目时接管Tab，否则保留原Tab背包快捷键。点击外部/栏目、Escape、失焦及离窗收起；隐藏/销毁清理延时、监听、临时属性与指针捕获。
- `setVisible(false)`一起隐藏stage背景与台面、释放拖动并收起栏目；返回地图恢复，离场销毁。原组队栏仅在探索地图态隐藏，战斗/事件/观察时恢复。房间先选后进入，最终入口重查真实邻接与状态；不泄露迷雾、不虚构风险收益、不改变奖励和存档。
- 安全撤离仅起点可用，放弃保留清空背包的二次确认；共用`_returnToMainWithRetry`。通关/退出面板存活到切场成功，抛错、false或未到主城保留重试；重试只切场、不重复结算或清包。玩家缺失不先结算，加载中/观察中/非地牢地图不启动退出。决策层须覆盖探索台并正确处理`[hidden]`。
- 首次F级新手试炼在路线UI建立后必须先用同一冷钢决策模态讲清撤离：入口“安全撤离”保留原背包和本局已获物品，入口外“强制放弃”及死亡清空背包，已装备物品不属于背包；放弃确认必须列出当前实际会损失的格数、总件数与物品摘要。四个教学房在首次进入前各显示一次目标、风险与完成条件，并同时保留“先查看路线”和“确认进入”；取消不标记已读、不推进节点，确认后才回到原`_enterNode()`入口。战斗房按“普通攻击→闪避→奔跑蓄势→冲刺攻击”读取已被战斗系统接受的动作状态；事件房固定为无惩罚选择训练，首领房每局首次进入补满生命、魔法与体力。节点档案与模态共读节点`originalType/tutorialStep`，完成节点转为`empty`后也不能丢失教学身份；首次成功后停止重复弹教学，但固定单线仍可重复挑战。
- 归档保留正式图、获准概念的实际编辑祖先、提示词和来源；源图与成品字节相同时只留正式PNG，来源记录指向正式文件。废案、重复CSS片段和整文件before备份不入Git；公共文件只迁移本任务代码块，不能整文件覆盖并行改动。实现和回退入口见探索台说明。
...
- **重要信息提示栏标准工作流（2026-09-02）**：新增后台完成、科技解锁、奖励信件、特殊天气、工程/探险/军团结算或其他可回看通知前，必须先读`docs/important-information-notification-workflow.md`。先确定权威状态所有者、稳定ID、存档/过期、合并、点击已读和图标真源，再按“业务先提交→顶部FIFO自然结束→右下重要信息坞→真实详情页”接入；缺图先盘点复用，确实缺失才转`tools/ai-gen/WORKFLOW.md`生成候选。
...
- **重要信息坞合同（2026-09-01；2026-09-02扩展）**：`research-report-dock.js`统一承载科技简报、小鼠大王正式奖励信件、高影响位面天气、后台工程报告、探险回报与军团结算的右下非阻断表现，各类状态仍由`TechnologySystem`、`MailStore`、天气运行事件和`StrategicJournal`分别维护。业务必须先提交，再等顶部FIFO自然播完才允许展开；地牢战斗/Boss、切场、地图模式或近身交战只累计统一未读入口。科技点击成功后才由科技系统标已读；信件仅在主神空间靠近小鼠大王时打开，地牢中途暂存不得伪装成正式信件；天气只确认本条运行期预警，不暂停、不切位面并随结束/重置/读档清理；工程/探险/军团报告点击定位世界面板对应战略事件。报告不得复制信箱附件或重写待领取数，顶部队列被清理或渲染失败时不得影响已提交业务。完整新增类型、合并去重、图标与验收流程见`docs/important-information-notification-workflow.md`。
- **特殊天气图标合同（2026-09-01）**：普通小雨/中雨/大雨/暴风雨继续复用`assets/ui/event-icons/rain-*.png`；沙尘暴、干旱、死寂雾潮、矿层震荡、毒气蔓延和沉眠者归来使用`weather-event-icons.js`的六项唯一映射。右下重要信息坞和顶部时间进度栏必须消费事件模型同一`iconPath`，加载失败才回退原emoji；不得为两处UI维护第二套图片或用通用气象塔科技图遮蔽具体天气语义。正式运行图统一256×256 RGBA、框外透明，1024源图、提示词与预览归档在`tools/ai-gen/_weather_event_icons_20260901/`。
...
- **领奖与返回补充（2026-08-30）**：奖励面板使用20px标题、16px卡名、14px正文、12px状态和主题数字字体，固定奖励摘要与三选一卡分开，复用正式物品图标及2:3冷钢卡框。`game-style.css`不再保留旧金色/漂浮卡样式，布局也归主题真源。领取成功后整组禁用，选中卡仍保持明亮；2秒自动继续与显式继续按钮共用完成回调，清理不触发通关。奖励层循环Tab并拦截Esc，`Input`同时隔离浏览器按键和EXE转发Esc，禁止在结算层下方打开暂停菜单冻结计时。路线UI恢复异常仍显示通关面板，返回加载期间保留面板和禁用状态，失败沿原切场入口重试、不重复发奖。窄窗/矮窗允许面板内滚动，未运行视觉验证。
...
- **DOM图标加载边界**：科技树 `iconPath` 与建筑升级 `iconImage` 由DOM图片直接读取相对路径，不需要登记到 Phaser `BootScene` 纹理预载。新增“建筑解锁＋标准化”两枚科技图和四枚本栋升级图时，仍须在64px科技卡、48px详情/升级卡下逐枚复核语义差异、完整外框与失败emoji回退；不能用额外预载掩盖错误路径。
...
- **科技树经济页连线合同（2026-09-02）**：经济页普通前置必须保持从左向右，依赖住房等级的产业节点放在对应住房门槛右侧，禁止用同列长竖线表达时代关系。一个前置同时通往三个及以上右侧目标时，SVG只绘制一根粗总线并在各目标行分叉，不能让多条彩线重复覆盖同一主干；同一列若有多个来源同时形成总线，必须按来源lane分配不同的列间X轨道，不得完全叠线。多前置目标在卡片左侧使用独立输入端口与AND/OR汇流框，再以单箭头进入卡片。所有单前置线直接以箭头标明解锁方向，跨越两列以上的独立长线显示来源标签。选中目标时，完整前置路径必须用SVG最末层的独立流动亮色虚线覆盖，不能依赖修改底层状态线；共享总线只高亮从来源到实际选中分轨的有效区段，入口、分叉、端口和汇流点用发光接头补齐，避免虚线在子路径或折点处出现视觉断裂；无关线路、端口和汇流框同步弱化。当前页无法绘制的跨分支前置必须在目标卡片上显示具体来源科技，位面未揭示名称仍须遮蔽。走线必须留在列间/行间通道，静态几何不得穿过非端点科技卡；图例同步解释箭头、汇流框、总线、当前选中路线和完成/未完成线型。
- **科技树军事页黑火药主干合同（2026-09-02）**：军事建筑卡按“I级 → II级 → 黑火药 → 近代III级 → 现代IV级”由左向右排列；黑火药位于column4并作为近代军事共同起点，军事页必须将它通往六条工业建筑路线和教堂近代升级的七条出线合并成一根粗总线再分叉。靶场二级“长弓整编”位于黑火药左侧，黑火药自身仍在“黑火药→靶场”行中表达发现路线与近代主干身份，图例明确说明粗线含义。现代节点只从本线近代节点继续，禁止用重复直连黑火药的跨屏长线制造视觉噪声；蒸汽动力等跨页AND条件仍在卡片状态与详情前置清单中完整展示。
...
- **右上经济面板（2026-08-22，2026-08-23 军事人口/时钟等高，2026-08-26 可展开详情/卷轴动效）**：金币、能源、食物和人口概览所在栏统一称为“经济面板”；与 `.game-time` 同挂在 `.top-right-hud` 弹性容器，经济面板在前、时间在后，折叠态必须读取 `--top-right-instrument-height` 保持74px等高，禁止按时间文本宽度硬编码 `right` 偏移；容器与顶部状态栏共享 `top:12px` 基线并使用 `align-items:flex-start` 顶边对齐。金币/能源/食物保持首行三列，第二行对称显示“兵力 已出兵/房屋容量”和“人口 实际居民总数”，中间必须有分隔；军事人口读取 `MilitaryPopulationSystem`，工作人口经 `EconomyHudSystem` 只读桥读取 `PopulationEconomySystem.getPopulationSnapshot()`，军事容量仍取房屋容量，经济岗位取实际居民，彼此不占用。面板底部使用原生按钮和向下箭头展开/收起，维护 `aria-controls/aria-expanded/aria-label`；经济面板与顶部天气/时间进度栏的箭头都必须在各自小按钮内水平、垂直居中，并以按钮中心为旋转轴。交互沿用顶部天气/时间进度栏在缩略外壳上展开的语言，但经济面板宽度必须始终固定250px，禁止横向放大。详情通过网格行从缩略面板底部向下展开，配合轻微下移复位和淡入形成卷轴下放效果，边框/阴影同步加强，时钟不得跳位；收起时向上卷回并恢复74px缩略版，禁止另开浮层或瞬时切换 `display`。详情仓容必须读取 `EnergyManager` 的当前位面活动仓库：总占用按共享物理容量计算，且只有“总占用”容量条按填充进度依次显露绿→青→黄→红危险度色带；能源、食物、兵力和工作人口容量条继续保留各自资源语义色，不得一并改成危险度色带。能源/食物的“距满仓剩余量”分别按每座仓库的能源/食物压缩倍率折算，禁止把两种资源误当成独立仓库；金币仍是背包单格无限堆叠，只能明确显示“无上限”，禁止伪造仓容百分比；兵力显示军事容量，居民/住房与岗位已用/实际人口分别显示。净收支统一读取 `EconomyFlowSystem` 的收入减消耗，并标注周期均摊/近60秒实收支，详见下方合同；不得在 UI 内重复实现生产公式。金币读取 `GoldManager.getGold()`，能源和食物读取 `EnergyManager.getEnergy()/getFood()`；沿用 HUD 刷新入口，动态进度只写内联 `width`，其余视觉样式归 `panel-theme-backpack.css`。已初始化概览数字发生变化时只重播对应数字的单次辉光动画，首次载入不触发；保留资源语义色、冷钢主题变量，并尊重 `prefers-reduced-motion`。
...
- **时钟包裹与经济概览大数缩写修订（2026-08-27）**：经济面板保持 `250px` 固定；时钟新增独立 `--game-time-instrument-width:286px` 并由 `.game-time` 专用读取，补偿后时钟整体向右延伸，且 `.top-right-hud` 位置使用 `right: calc(max(64px, 100px - (var(--game-time-instrument-width) - var(--top-right-instrument-width))))`，避免超出左侧面板基线。概览大数按阈值缩写：`>=10000` 为小写 `k`（10000=10k），`>=1000000` 为大写 `M`；鼠标悬停浮窗显示完整数值（资源 10,000；兵力 12,500 / 20,000；实际居民 12,500）；展开详情与 `economyGoldCapacityText`、`_updateEconomyCapacityMeter` 继续使用完整原始数值。
- **经济缩略高度硬边界（2026-08-26；2026-08-31展开修正）**：经济面板不能只依靠详情的`0fr`推断折叠高度；外壳折叠态必须同时锁定`min-height/max-height:74px`，避免详情内边距或分隔线留下像素级增高。展开态按内容向下扩展，`max-height:calc(100dvh - 44px)`（`100vh`回退）为顶部、底部箭头与边距留空间，禁止恢复固定460px上限。详情容器保留`min-height:0`，横向隐藏、纵向`auto`并限制滚动外溢；矮窗口必须能滚到末行。与详情网格行共同过渡，经济宽250px、时钟286×74px及原顶边对齐不变；底部箭头属于外接控制柄，不反向撑高外壳。

### 经济建筑双按钮与科技门禁

- 手动升级有科技门禁不代表“持续升级”自动受控。共享升级卡从经济项目状态读取`technologyLockReason`，未解锁时保留禁用按钮并提示原因；业务入口在占用持续名额前使用同一状态。
- 活动持续目标的按钮始终可停止，不能因科技锁定、资源不足或当前读条而禁用。使用既有科技状态轮询刷新解锁态，不为单栋建筑另建事件系统；停止只清意图，不取消已付款工程。
- 地热等内部周期入账建筑采用稳定发电效率条，周期只作数值说明，升级工程才显示完成进度；岗位条的满员100%不等于生产人效100%。

### 人口、幸福度与净收支UI合同（2026-08-31）

- 经济概览显示实际居民总数，岗位已用/空闲与居民/住房分别放详情；住房容量变化不是人口增减。人数新增绿色、减少红色单次800ms反馈，初次显示、切场、读档与岗位变化不闪；`viewRevision`仅重建比较基线、不入存档。尊重减少动态效果，大数缩写但悬停/无障碍保留全数。
- 市政厅与房屋复用`population-growth-view.js`快照组件，不在面板重算人口周期。增长正向、饥荒反向，正常/缺粮/流失分别复用人口/警告/危险主题；满房继续展示减速进度。幸福度分值、目标、六项贡献是点数，增长修正才是百分比。
- 幸福度图标真源在`assets/ui/happiness/`：`happiness/food/housing/housing-quality/entertainment/commerce/safety.png`依次对应总幸福度与六因素。128px透明副本按32px显示，标签和值用独立节点刷新，不能替换整个容器丢图。选定1254px源图、提示词与限定导出脚本保留在`tools/ai-gen/happiness-icons-cold-steel-20260831/`；功能本地接入、素材发布、实机验收分别记录。
- 金币/能源/食物显示“每秒净收支”及收入、消耗拆分；只读`EconomyFlowSystem.getSnapshot()`，不再用余额近5秒采样。周期预算与近60游戏秒实收支必须明确标注，悬停说明来源/观察时长，正绿负红；未知来源不可用要明示。新增消费沿资源入口或注册来源接入，UI不复制经营公式。
- 以上为本地实现合同，源码发布状态见 [交付记录](../docs/population-economy-publication-2026-08-31.md)，不因文档或图标入库声称核心功能已发布。
...
- 自动触发的新手指引同样登记为任务定义并写入`QuestStore`；未触发前用定义字段隐藏，触发、完成与旧档恢复通过业务事件对齐状态，不能只依赖限时通知或另存一份布尔值。无任务场景会话的指引不得占用或清空正在执行的裂隙会话。
- **首次位面探索引导（2026-09-05）**：序章进入游戏后自动激活`first_city_founding`，其目标顺序固定为“小鼠大王赠送F级钥匙→中央祭坛打开钥匙出征→成功通关`abandonedMineBeginner`→返回小鼠大王开启首城选址→从合法候选确认首城”。正常教程路线进入主神空间后，常驻目标条明确“WASD移动→靠近小鼠大王→鼠标左键点击交互”，顶部通知只宣布主线开启，不复制完整操作句；小鼠大王在首次钥匙领取完成前暂停游走，领取后恢复既有移动。跳过全部教程路线不得触发。新手钥匙是明确的背包教学奖励：只在玩家点击领取后调用`EquipManager.addToInventory()`直接入包，背包满则保留领取资格，不转仓库/信箱；首次成功前若背包与仓库都没有可用F钥匙，可免费补领一枚，避免失败软锁，但同一时刻不重复赠送。阶段对白、任务`activeHints`与常驻目标条都从QuestStore目标和背包/地牢/位面业务状态动态推导。首次地牢的基础战斗房允许常驻目标条临时切换为4步操作教学，并且必须读取玩家实际进入的普通攻击、闪避、冲刺就绪与冲刺攻击状态推进，不能只监听原始按键；完成后恢复主线提示，不修改战斗或清场门槛。世界目标圈必须在中立NPC本帧位置同步后绘制，以当前帧真实Alpha范围、分层贴图与完整点击区的并集计算包围圆，不能再用脚底碰撞半径；圆圈和箭头统一为带深琥珀描边的金色，以适配白色大理石背景。常驻目标条必须把高频数字刷新与无障碍阶段播报分离，只在阶段变化时播报；前两步允许把暂时无关的HUD降到低透明度，但悬停/聚焦必须恢复。常驻目标条读取`_minimapLayout()`发布的实际占位并锚在小地图右侧12px，禁止占用左下方RTS兵线/指令栏堆栈；它属于小型HUD，外壳必须直接使用`--bp-ui-shell`、主题边线/圆角/阴影，不能自建半透明渐隐板或金色侧条，文字按“11px系列标签+12px等宽步骤、16px任务标题、14px正文、12px当前目标”四档组织，普通信息只用银白/冷灰。大地图只以`WorldProgressionSystem`的首次地牢成功/既有位面进度为解锁真源，失败、放弃、仅打开祭坛或仅看完对白都不能解锁。首次成功后先进入`awaiting_king`，交谈后再进入`selecting`；位面面板展示获准的地貌生成模板，玩家必须在航图上点击一个非山脉、非山口、未占用且满足城距的地格，所选地格的地貌决定生成模板，正式城市坐标以该地格为准。首城授予只旁路一次目标模板的地牢/科技/资源门槛，不伪造该系列地牢完成，也不解锁其他位面。传送门建筑与普通建造入口不得绕过选址。确认后免费建立市政厅和首座传送门，并保留由玩家主动进入首城的按钮。
- **正式位面目录可见性（2026-09-05）**：配置中的scene模板、仅满足配置前置但尚未正式发现的位面、以及交互开发工具的临时直连现场，都不能出现在正式大地图左侧目录、数量统计、状态提示或操作按钮中。常驻目录只读取已登记航图发现或曾真实建城的位面；首城选址是唯一允许展示待建候选的例外，并继续只消费`getFirstFoundingCandidates()`。正式传送门目的地同样不得请求`includeDevTests`，构造列表也只能列正式已知位面；测试现场的直达入口只能放在交互开发工具位面页，不能借正式世界面板或主神空间传送门取得入口。
- **首城地貌模板与玩家选址（2026-09-05更新）**：`getFirstFoundingCandidates()`只列配置显式声明`firstFoundingCandidate:true`且尚未承载正式位面的地貌模板；当前沙漠、雪原、林地、地牢遗迹与矿洞模板均可承接唯一一次首城授予。模板列表只负责地图聚焦与生成类型；进入选址阶段后，应在战略城市/营地完成初始化后按本局地图种子稳定抽取一处合法且周边开阔的建议点，并按`firstFounding.initialVisionRadius`揭开初始视野。建议点只帮助新手定位，玩家仍可自由改选；确认必须消费玩家最后点击的合法地格，并由`planeSceneId`反查模板。首城资格不等于目标模板对应地牢已经通关。主神空间传送门与位面建筑配置不得保留会被误作正式目录真源的静态`entries`/`destinations`名单，玩家可见目的地统一从`WorldProgressionSystem`的正式进度产生。
- **首城建设引导（2026-09-05更新）**：玩家首次实际进入已授予的首座位面时自动激活`first_plane_settlement`，不在选址面板或主神空间提前开始。固定七步为“首座免费仓库→攻击能源矿脉并实际入库300能源→首座免费房屋与首名居民→麦田风车建成且至少1名农夫上岗→研究所→至少1名研究员并选择研究目标→研究所产出首批真实科研进度”。补入房屋和岗位是硬前置：市政厅本身不提供居民，风车/研究所只建不安排人口不会产粮或科研。采矿目标只要求300能源，用于确认玩家掌握真实入库；仓库冷启动3000能源保持不变，采矿首次达到300时额外存入1200能源建设补给且不计入周期生产统计，使教学顺序下的4500能源刚好覆盖1500风车与3000研究所。只统计`EnergyNode`攻击后由`EnergyManager.depositEnergy()`返回的实际入库量，仓库赠送、余额变化和地面掉落不冒充采矿。首名居民完成风车教学后若没有空闲人口，研究阶段必须先高亮风车“-1”，再高亮研究所“+1”，明确教学岗位调任而不是要求被动等待人口增长。建筑落地、岗位变更、场景进入与科研进度分别发布业务事件，QuestStore只记目标进度；任务提示可从真实存活建筑和科研状态恢复，离开首城时改为引导返回，不创建任务场景会话。按当前1名研究员约0.1点/秒的起步速度，完成60点早期科技约需10分钟，强制等待不属于新手教学，因此最后一步只在真实科研产出后毕业，科技继续由原系统推进。毕业提示提供“扩建住房、继续采矿、规划下一科技”三个自由目标；常驻条继续位于小地图右侧，并可用金色世界圈跟随市政厅、矿脉、风车和研究所；建筑卡、岗位按钮和科技树入口使用同语义金色焦点，但不自动点击、自动分配人口或伪造科研进度。
- 建筑卡教学焦点必须落在整张`.we-thumb`真实点击卡片上；建筑网格存在滚动裁切，因此卡片焦点使用卡片内部的金色双层轮廓，外部辉光只作辅助，禁止用会被滚动容器截断的外扩`outline`表达完整选区。其他不处于滚动网格内的教程按钮仍可沿用通用外圈。
- 仓库完成进入采矿阶段后，指引每次刷新都从当前首城存活、未枯竭的能源矿脉中按玩家距离选最近目标；目标圈标记矿脉，玩家头顶另绘半透明金色方向箭头并逐帧跟随玩家旋转指向该矿，即使矿脉在屏幕外也保留。矿脉耗尽或玩家移动导致最近目标变化时允许自动切换，但箭头只导航，不自动寻路、攻击或入库。
- **新手文案口径（2026-09-05）**：玩家首次接触时使用“F级钥匙（时空锚点）”，后续统一简称“F级钥匙”；采矿目标统一写“实际入库300能源”，不得称为300能源矿石；科研毕业条件写成“研究进度开始增长”，不向玩家暴露“真实进度”等实现术语。新手地牢固定安全女神像统一称“事件选择训练”，不误写成随机事件。入口集中解释撤离与背包损失，后续房间弹窗只说明本房目的和完成条件。首城科研首次产出后由小鼠大王通讯完成故事收束，再给出自由扩建方向。
...
- **地牢布局规格（2026-08-29）**：`scene7` 地牢战斗读取双份 `game-config.json#minimap.dungeon`，与位面统一为 `252×150 @ (10,60)`；绘制、战争迷雾、点击和拖动一律继续消费 `_minimapLayout()`，禁止各自判断地牢并复制尺寸。静态缓存键除 zoom、世界尺寸、墙数量外还必须包含最终宽高与锚点，确保场景切换后的规格变化触发重绘。
...
- **静态层墙壁和边界线绘制都要框内裁剪**（与动态层 `inBox` 同口径）：墙可带负坐标/越界
  坐标；矩形先求交集，线段先裁到地图框，禁止把范围外内容提交到 Graphics。
...
- `mailbox-panel.js`复用BasePanel、UIState与右栏挂载层，样式只在`panel-theme-backpack.css`的`mouse-king-mailbox`块维护；20/16/14/12字阶，信件与附件分别20项分页，未领数量不等于未读数量。
- 领取只扣除实际入库数量，剩余保留原信；批量领取关闭后停止后续批次。背包/仓库整理遵循原访问条件，返回保留信件/筛选/滚动位置并重算容量；NPC失效、切场和读档取消往返，不能由旧回调重开面板。
- 浏览器键盘与EXE转发Esc都由Input信箱门禁接管，保留原生按钮和Tab，不穿透到角色动作；邮件附件tooltip复用原物品数据，不重新滚装备。实际布局与旧档迁移尚待用户验收。
...
- **冷钢探索台v9（历史接入；当前布线和尺寸以上文合同为准）**：用户选定`02-dark-exploration-hybrid-v9.png`后，默认`mapLayout: exploration`。上方约30%为纯环境，中央下方整合路线、固定档案、真实奖励预览、特工状态及两种撤离；保留左274/右104安全区并各加12px间隙，原生命/魔法/等级、组队栏与右侧入口尺寸不动。路线采用`dungeon-exploration-console.js`内独立Canvas画连接、原生button画房间，CSS实测路线窗口统一驱动绘制/拖动/聚焦；旧gameCanvas继续画背景，Phaser隐藏合同不变。`dungeon-expedition-layout.js`只读真实节点/边；现已改用上文固定row网格，交会不创建玩法连接点。四条以上靠分区/平移/相邻房间选择框查看，不把所有房间无限缩小塞满。
  房间先选中、档案固定展示，中央大按钮再次通过`isNodeClickable()`进入`_enterNode()`；未揭示房间不泄漏类型/奖励，未知事件不标低风险，风险只作房间类型提示而非新增数值评级。收益继续取ChestRoomSystem、奖励规则及祭品权重；入侵原标签挂到新台标题区，不改判定/追击/击败后移除。安全撤离起点可用，其它位置禁用并解释；强制放弃沿用清空全部背包的确认流程。返回地图重置选择为当前位置，观察/战斗/事件隐藏台，shutdown清理DOM/ResizeObserver/指针捕获。Tab/Enter/空格/方向键在台内不穿透游戏，鼠标松开继续交给全局清理。恐怖三档用新独立古堡和门厅资产，其它地牢保留各自背景；不调用识图、模板或旧母图抽选。详见`docs/dungeon-exploration-console-v9.md`。未运行测试或运行时验证，由用户测试。
- **独立路线恢复（2026-08-30，历史过渡）**：此前曾按用户要求恢复`legacy`上40%/下60%，保留玩法与并行修改后提供三张候选；随后选择第二版并细化为v9，现已由上述探索台接替。旧分支与背景不删除。见`docs/dungeon-console-restore-and-concepts.md`。
- **远征组合界面（2026-08-30，已停用的尝试）**：用户选择上方环境＋下方独立路线，`DungeonConfig`默认`mapLayout: split`。中央上36%只展示环境，下64%承载路线；继续预留左274/右104安全区，保留既有地标血魔条、组队和右侧入口尺寸。`dungeon-expedition-layout.js`按完整逻辑图稳定分列/减少交叉，图片不限制随机路线；每区按窗口显示1～4核心列并保留接头，聚焦不低于0.95，密集处拖动查看。节点、命中、档案、回头定位和入侵者共用展示坐标；切区/悬停不重排，窗口变化清理旧指针并重聚焦。背景本局固定，旧图/通道模板保留。以下贴图同矩阵、按图片容量分区规则只适用于显式旧模式，不能重新施加到组合版。详见`docs/dungeon-expedition-split-ui.md`；本轮未运行测试或运行时验证，由用户测试。
...
- **矿洞静态通道模板（2026-08-30）**：初/中/高级矿洞复用地标HUD与徽记，不改状态栏/组队栏/右栏尺寸位置。`src/config/mine-route-landmarks.js`只登记完整平台`lanes`与楼梯`connectors`；基础选图间距76，正式展示至少96源像素槽位，并按实际窗口反投影边界、徽记/标签边距与屏幕点击缓冲筛选。每区最多3列、可见容量58%核心预算；单列超量也按节点分区，核心归属按ID；剩余容量只容纳真实逻辑邻接点。显示、悬停、点击与档案入口共用可见节点集合，切区/缩放清空旧指针与卡片，尺寸进入缓存键。布线统一使用下述公共线束，不新增玩法连接；可前往线最后绘制。极窄裁切无平台时只允许背景和路线同步聚焦平台，不能移动HUD。`weighted-terrain`选图在本局稳定，恐怖地牢保留原选图及节点结构带投影。详见`docs/dungeon-mine-landmark-ui-2026-08-30.md`。
- **地标路线简洁显示（2026-08-30修订）**：默认只常显当前位置真实邻接线，悬停可见节点补充其关联线；“全部连线”可展开当前区段，浏览不含当前位置的区段保留低对比全线。可前往状态必须优先于已访问状态，回头路也使用连续亮线和目标方向箭头；当前路线最后画、悬停目标置顶。远端线降对比并取消常驻接头/阻断叉，悬停检查时再显示阻断。节点投影与公共轨道独立于边显隐缓存，悬停/全线切换不得让节点或线路跳位。显示接头优先当前位置及可走邻居，区段索引标注可走数量。悬停命中允许只读检查不可达节点，实际点击与档案进入仍必须经过`isNodeClickable()`；不得借简化路线改动图、迷雾、HUD或玩法。
- **公共线束生成（2026-08-30，取代三轨叠画）**：按本区可见节点的完整真实边集生成固定几何，再控制显隐。矿洞平台沿已登记道路中心线，楼梯沿已登记两端连线，不再整体旁移到路外；节点后画遮住线头；同坐标站点合并，按长度选固定通道。禁止拥挤加价改道、整边独立法线偏移或逐节点圆弧绕行。其它地标按列对共用横竖主干，同列没有侧向空间时直连。`src/ui/dungeon-route-bundles.js`按共线重合区间拆分、归并同成员连续段；共享段只画一次，不限制3/4/6条，正反方向同几何，不新增玩法连接。中性主干旁用`×N`表示当前显示且实际共段的独立边数，混合状态补充可走数；同成员最多一标、每屏最多6标，避让节点/文字，空间不足隐藏。悬停可走目标时突出完整路径并压暗其它线，方向箭头置于目标端。保留节点投影、原背景、可达/迷雾/点击与HUD；必须由用户复测贴合、避让、回头路、显隐切换和窄窗，不宣称全图无交叉。
- **路线与背景同源（2026-08-30）**：当前矿洞读取人工通道模板，不是运行时识图。后续先定道路结构/楼层/分叉和容量，再以相同坐标制作背景；每张图成对交付道路中心线、路面范围、完整楼梯折线、节点槽位和禁行数据，不能靠配色或路外偏移掩盖结构不匹配。旧图逐张校准，先验收一张样板再扩展；不声称本轮已完成所有图的校准或新生成管线。详见`docs/dungeon-route-structure-workflow.md`。
- **路线相近冷色区分（2026-08-30）**：地标连接身份色统一登记在主题`--bp-route-color-1`至`8`，按完整真实边集稳定分配，岔口优先异色；不能随当前节点、悬停、切区或全线开关重新洗色。独立出口和悬停聚焦的完整路径与箭头、目标外框/呼吸环和“可前往”标签同色；多路线共享主干使用主题中性冷钢色及数量标记，房间内部类型图标/底色保留原语义。不可走与已走段降低亮度，全不可走且未完成的段为虚线，悬停检查保留目标端锈红阻断叉；颜色不能替代可达判定或承诺消除几何交叉。只读主题色须缓存，不每帧读取DOM样式。
