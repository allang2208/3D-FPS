// ============================================================
// 世界地图：本局种子六边形地貌 + 真实位面状态，观察/支援仍走原切场协议。
// 军团沿六边格行军；地块不常驻战场，遭遇由独立战略系统物化。
// ============================================================
...
import { strategicTerrain, strategicStepDays, strategicStepMs, formatStrategicDuration, formatStrategicTravelTime, strategicClockHint, strategicNow } from '../world/strategic-march.js';
...
export const WorldSwitchPanel = {
    ...
    _requestedFirstFoundingCellId: null,
    ...
    _getPanel() {
        if (!this._panel) {
            ...
            EventBus.on('world-map:changed', () => {
                ...
                this._requestedFirstFoundingCellId = null;
            ...
            this._panel.onOpen = () => {
                ...
                const requestedFirstFoundingCell = strategicCell(this._requestedFirstFoundingCellId);
                ...
                const openingFocus = eventCell
                    ...
                    : requestedFirstFoundingCell
                        ? { sceneId: requestedFirstFoundingCell.planeSceneId, cellId: requestedFirstFoundingCell.id }
    ...
    openFirstFoundingSelection() {
        ...
        this._requestedFirstFoundingCellId = recommendation?.cellId || null;
        const opened = this.open(recommendation?.sceneId || candidates[0].sceneId);
        if (!opened) this._requestedFirstFoundingCellId = null;
        if (opened && this._panel?.isOpen && recommendation?.cell) this._selectCell(recommendation.cell, true, 'worlds');
        return opened;
    },
    openAtEvent(id) {
        if (!this._allowOpen()) return false;
    ...
    _openingFocus(requestedWorldId) {
        ...
        const visibleIds = new Set(visibleWorlds({
            includeFirstFoundingCandidates: this._firstFoundingSelection,
        }).map((world) => world.id));
        const planeIds = WORLD_MAP_PLANES
            .map((plane) => plane.sceneId)
            .filter((sceneId) => visibleIds.has(sceneId));
        // 明确的“定位某位面”入口继续服务其目标，普通 O 键不跟随旧追踪目标。
        if (planeIds.includes(requestedWorldId)) return { sceneId: requestedWorldId };
        ...
        if (armyCell) return { sceneId: armyCell.planeSceneId, cellId: armyCell.id };
        const home = Game._observerMode ? Game._observerHomeScene : SceneManager.currentScene;
        if (!inDungeon && !SceneManager.isQuestInstance(home) && planeIds.includes(home)) return { sceneId: home };

        // 地牢/主神空间没有独立战略地格，回退至已接通基地中建设最好的一个。
        // 读取真实快照（观察中的前台位面取即时副本），不改变资源、追踪或传送资格。
        const ranked = planeIds.filter((id) => WorldProgressionSystem.isPortalConstructed(id)
            && !WorldProgressionSystem.getPortalState(id).destroyed).map((id) => {
            const snapshot = isWorldLive(id) ? captureWorld(id) : getWorldSnapshot(id);
            const buildings = (snapshot?.structures || []).filter((building) => building.hp > 0
                && ['producer', 'hut'].includes(building.kind));
            const development = buildings.reduce((total, building) => total
                + Math.max(1, Number(building.economyLevel) || 1), 0);
            const population = Math.max(0, Number(snapshot?.populationEconomy?.total) || 0);
            return { id, development, population, count: buildings.length };
        });
        ranked.sort((a, b) => b.development - a.development || b.population - a.population
            || b.count - a.count || planeIds.indexOf(a.id) - planeIds.indexOf(b.id));
        return { sceneId: ranked[0]?.id
            || planeIds.find((id) => WorldProgressionSystem.getWorldMapDiscovery(id))
            || planeIds.find((id) => WorldProgressionSystem.isWorldEligible(id)) || planeIds[0] };
    },
    ...
    _selectWorld(id, focus = false) {
        ...
        this._selectedCellId = foundingCell?.planeSceneId === id
            ? foundingCell.id : WorldProgressionSystem.getWorldMapDiscovery(id)?.cellId || null;
    ...
    _worldName(id) {
        ...
        if (window.WorldProgressionSystem?.getWorldDisplayName) {
            const displayName = window.WorldProgressionSystem.getWorldDisplayName(id);
    ...
    _worldStatus(sceneId, current) {
        ...
        if (portal.destroyed) return '<b class="wm-tone-danger">传送门已摧毁，等待重建</b>';
        const protection = WorldProgressionSystem.getPortalProtection(sceneId);
        const protectionText = protection.active
            ? `<b class="wm-tone-success">新生保护 ${protection.remainingDays.toFixed(1)} 天</b> · `
            : '';
        ...
        const invasionText = invasion?.targetWorld === sceneId
            ? `<b class="wm-tone-warning">入侵第 ${invasion.waveIndex}/${invasion.waveCount} 波</b> · `
            : '';
        ...
        if (preview) {
            const parts = [];
            if (preview.defeated) parts.push('<b class="wm-tone-danger">⚠ 预估已失守</b>');
            ...
            if (preview.structuresLost > 0) parts.push(`<span class="wm-tone-warning">预估损失建筑 ${preview.structuresLost}</span>`);
    ...
    _constructionBlockReason(sceneId) {
        ...
        if (Strategy.active) {
            ...
            const entry = WorldProgressionSystem.getWorldMapDiscovery(sceneId);
            return Strategy.inMap && !Strategy._busy && !SceneManager.isLoading && Strategy.state.army.cellId === entry?.cellId
                ? '' : '请率军抵达此信标格，结束战斗后接通';
        }
    ...
    _worldExpeditionHtml(world) {
        ...
        if (this._firstFoundingSelection) {
            ...
            const recommendation = WorldProgressionSystem.ensureFirstFoundingRecommendation();
            const selectedHere = site.sceneId === world.id;
    ...
    _render() {
        ...
        const states = candidates.map((world) => {
            ...
            const entryCell = discovery?.cell || (selectedFoundingCell?.planeSceneId === world.id ? selectedFoundingCell : null);
            ...
            const badge = this._firstFoundingSelection
                ? selectedFoundingCell?.planeSceneId === world.id
        ...
        this._setSectionHtml('wmDestinations', states.map((world) => `
            <button type="button" class="wm-destination${world.isCurrent ? ' is-current' : ''}${world.connected ? '' : ' is-offline'}"
                ...
                <span class="wm-destination-name">${world.icon} ${escapeHtml(this._firstFoundingSelection ? `${WorldProgressionSystem.getWorldTerrainLabel(world.id)}模板` : this._worldName(world.id))}</span>
                <small class="wm-destination-state">${escapeHtml(world.badge)}</small>
            </button>`).join(''));
        let status;
        if (this._firstFoundingSelection) {
            status = !selectedFoundingCell || selectedFoundingCell.planeSceneId !== selected.id
        ...
        this._setSectionHtml('wmDetails', `
            <h2 class="wm-detail-title">${escapeHtml(this._firstFoundingSelection ? `${WorldProgressionSystem.getWorldTerrainLabel(selected.id)}建城模板` : this._worldName(selected.id))}</h2>
            ${selected.entryCell ? `<p class="wm-hint">${this._firstFoundingSelection ? '玩家所选地格' : selected.destroyed ? '旧入口地块' : selected.connected ? '接通地块' : '目标地块'}：${selected.entryCell.q}, ${selected.entryCell.r}</p>` : ''}
