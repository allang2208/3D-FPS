...
const NPCDialogue = {
    ...
    _getTutorialGreeting(npc) {
        ...
        if (npc.id === 'npc_altar') {
            if (stage === 'receive_key') return '祭坛没有检测到可用的 F 级钥匙。先去找小鼠大王领取新手钥匙。';
    ...
    _updateDialogueButtons(npc) {
        ...
        if (npcType === 'altar') {
            ...
            const keyButton = ['receive_key', 'replace_key'].includes(tutorialStage)
                ? `<button type="button" class="npc-option-btn npc-option-btn--primary" id="npcOptionStarterKey" onclick="NPCDialogue.claimStarterDungeonKey()">${tutorialStage === 'replace_key' ? '免费补领 F 级钥匙' : '领取 F 级新手钥匙'}</button>`
                : '';
