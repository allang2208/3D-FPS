...
export const EquipTooltipManager = {
    ...
    // ===== 防具/首饰：防御成长 + 属性加成 + 套装信息 =====
        if (fullItem && (fullItem.category === 'armor' || fullItem.category === 'accessory')) {
            const el = (item.enhanceLevel || fullItem.enhanceLevel || 0);
            const setNames = { light: '疾风（轻甲）', robe: '秘法（法袍）', heavy: '壁垒（重甲）', flowing: '流云（稀有轻甲）', eclipse: '蚀月（稀有法袍）', zhenyue: '镇岳（稀有重甲）', stellar: '星穹（史诗轻甲）', lunar: '苍月（史诗法袍）', tiangang: '天罡（史诗重甲）', oracle: '神域（神话重甲）', oracle_robe: '神谕（神话法袍）', holy: '圣辉（神话轻甲）' };
            const setBonuses = {
                light: '三件齐穿：+10% 移动速度',
                ...
                oracle: '三件齐穿：自动格挡60%概率减少90%伤害（最后乘法结算）、-12%移动速度',
                oracle_robe: '三件齐穿：技能冷却-28%、魔法伤害+35%',
                holy: '三件齐穿：暴击率+20%、物理攻击+15%、移动速度+10%',
            };
