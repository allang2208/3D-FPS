...
const COLLISION_CONTENT_OWNER = 'collision-editor';
const PREVIEW_ALPHA = 0.42;
const TEST_SPAWN_MIN_OFFSET = 120;
const TEST_SPAWN_MAX_OFFSET = 220;
...
export const CollisionEditor = {
    ...
    _previewLabel: null, // 冻结预览标识，避免与正式测试单位混淆
    ...
    _testBtnEl: null,
    _testSpawnSeq: 0,
    _testSpawnBusy: false,
    _visualLoadBusy: false,
    _visualLoadError: '',
    _selectionVersion: 0,
    _visualReadyPromise: Promise.resolve(true),
    ...
    close() {
        ...
        this._visualReadyPromise = Promise.resolve(false);
        this._testSpawnBusy = false;
        this._visualLoadBusy = false;
        this._visualLoadError = '';
        if (window.Game) window.Game._collisionEditMode = false;
        this._removePreview();
        RuntimeAssetManager.releaseInteractiveContent(COLLISION_CONTENT_OWNER);
    ...
    _buildPanel() {
        ...
        this._infoEl = info;
        const retryVisualBtn = document.createElement('button');
        retryVisualBtn.className = 'ce-btn';
        retryVisualBtn.textContent = '重新载入资源';
        retryVisualBtn.hidden = true;
        retryVisualBtn.addEventListener('click', () => this._retrySelectedContent());
        panel.appendChild(retryVisualBtn);
        this._retryVisualBtn = retryVisualBtn;
        ...
        testBtn.title = '保留半透明纸面预览，在其附近安全位置生成一只正式测试单位';
    ...
    select(kind, key) {
        ...
        this._visualLoadBusy = false;
        this._visualLoadError = '';
        this._visualReadyPromise = Promise.resolve(true);
        this._motionDebugSourceId = null;
        clearMotionMeleeDebugTraces();
        clearBasicMeleeDebugTraces();
        this._gateState = 'open';
        ...
        } else if (this._isCombatEntityKind()) {
            this._visualReadyPromise = this._prepareSelectedContent(kind, key, selectionVersion);
        } else {
            this._spawnPreview();
    ...
    _spawnPreview() {
        ...
        if (this._isCombatEntityKind()) {
            ...
            RuntimeAssetManager.attachInteractiveContentToEntity(COLLISION_CONTENT_OWNER, e);
        }
    ...
    _setVisualStatus(message, error = false) {
        if (!this._infoEl) return;
        const status = document.createElement('div');
        status.style.color = error ? '#ff9b8d' : '#ffd18a';
        status.textContent = message;
        this._infoEl.replaceChildren(status);
    },

    _retrySelectedContent() {
        if (!this.active || !this._isCombatEntityKind() || this._visualLoadBusy) return;
        this._removePreview();
        this._edit = null;
        this._gfx?.clear();
        this._visualReadyPromise = this._prepareSelectedContent(this._kind, this._key, ++this._selectionVersion);
    },

    /** 先载入并锁定完整资源，再创建纸面预览；绝不让占位贴图冒充测试结果。 */
    async _prepareSelectedContent(kind, key, selectionVersion) {
        this._visualLoadBusy = true;
        this._visualLoadError = '';
        this._syncTestButton();
        const label = kind === 'friendly' ? '友军' : '怪物';
        this._setVisualStatus(`⏳ 正在载入${label}完整贴图与动画：${key}`);
        const friendlyEntry = kind === 'friendly' ? FRIENDLY_UNIT_MAP[key] : null;
        let stage = '资源加载';
        try {
            await RuntimeAssetManager.ensureInteractiveContent(
                COLLISION_CONTENT_OWNER,
                kind === 'enemy'
                    ? { kind, type: key, config: enemyConfigData[key] }
                    : { kind, id: friendlyEntry?.config?.id },
                {
                    required: true,
                    onProgress: (ratio) => {
                        if (!this.active || selectionVersion !== this._selectionVersion) return;
                        const percent = Math.round(Math.max(0, Math.min(1, ratio || 0)) * 100);
                        this._setVisualStatus(`⏳ 正在载入${label}完整贴图与动画：${key}（${percent}%）`);
                    },
                }
            );
            if (!this.active || selectionVersion !== this._selectionVersion) return false;
            stage = '实体创建';
            this._spawnPreview();
            if (!this._entity) throw new Error(`无法创建${label}预览实体`);
            stage = '预览初始化';
            ...
            if (!this.active || selectionVersion !== this._selectionVersion) return false;
            this._visualLoadBusy = false;
            this._visualLoadError = error?.message || String(error);
            this._syncTestButton();
            this._setVisualStatus(`⚠️ ${label}${stage}失败：${this._visualLoadError}`, true);
            this._toast(`⚠️ ${key} ${stage}失败：${this._visualLoadError}`);
            console.warn(`[CollisionEditor] ${label}${stage}失败:`, key, error);
            return false;
        }
    },
    ...
    /** 保留冻结纸面预览，在附近安全点另生成一只正式测试单位。 */
    async _spawnTestEntity() {
        const Game = window.Game;
        if (!Game || !this._entity || !this._isCombatEntityKind() || this._testSpawnBusy) return;
        const selectionVersion = this._selectionVersion;
        const selectedKind = this._kind;
        const selectedKey = this._key;
        // 正式单位不再与冻结预览完全重叠；纵向偏移保持仍在玩家附近，
        // 位面测试怪出生后可立即进入对玩家/仓鼠的本地交战范围。
        const footprintOffset = (Number(this._edit?.radius) || 20) * 2 + 40;
        const spawnOffset = Math.max(TEST_SPAWN_MIN_OFFSET,
            Math.min(TEST_SPAWN_MAX_OFFSET, footprintOffset));
        const spawnX = this._entity.x;
        const spawnY = this._entity.y + spawnOffset;
        this._testSpawnBusy = true;
        this._syncTestButton();
        if (this._visualLoadBusy) this._toast(`⏳ 正在载入完整贴图与动画：${selectedKey}`);

        let visualReady = await this._visualReadyPromise;
        if (!this.active || selectionVersion !== this._selectionVersion) {
            this._testSpawnBusy = false;
            this._syncTestButton();
            return;
        }
        if (!visualReady) {
            this._testSpawnBusy = false;
            this._syncTestButton();
            this._toast(`⚠️ ${selectedKey} 的完整贴图或动画未载入，已取消生成`);
            return;
        }
        const id = `collision_test_${this._kind}_${this._key}_${Date.now()}_${++this._testSpawnSeq}`;
        const e = selectedKind === 'enemy'
            ? this._createEnemy(spawnX, spawnY)
            : this._createFriendly(spawnX, spawnY, id);
        if (!e) {
            this._testSpawnBusy = false;
            this._syncTestButton();
        ...
        // 正式测试体必须参与主循环。正常工厂会返回全新实体；这里仍显式清理
        // 纸面预览字段，防止未来某个工厂复用实例或构造覆盖时把 _editorFrozen
        // 带到正式体，导致 game.js 整帧跳过怪物/友军 update。
        if (e._editorFrozen) this._applyEditorFreeze(e, false);
        delete e._editorFrozen;
        delete e._collisionPreview;
        delete e._editorFreezeBackup;
        e.active = true;
        // 预览点在建筑/墙体密集的位面中可能不可行走；正式测试单位必须
        // 按自身 footprint 校正到附近安全点，否则出生在阻挡体内会表现为 AI 不移动。
        const testRadius = Math.max(4, Number(e.groundRadius ?? e.collisionRadius ?? e.collider?.radius) || 20);
        if (WallSystem?.findSafeSpawn && !WallSystem.canMoveTo?.(e.x, e.y, testRadius)) {
            const safe = WallSystem.findSafeSpawn(e.x, e.y, testRadius, 16);
            e.x = safe.x;
            e.y = safe.y;
        ...
        if (selectedKind === 'enemy'
            && ['scene8', 'scene9', 'scene10', 'scene11', 'scene12'].includes(window.SceneManager?.currentScene)) {
            // 位面中的正式测试怪复用防守怪索敌优先级，让近处仓鼠士兵/正式队友可成为
            // 攻击目标；不设置 _defenseMonster，避免进入波次数量、金币和入侵清理协议。
            e._preferDefenseTargets = true;
            e._engageHostileRange = 320;
            e._alertRange = Math.max(Number(e._alertRange) || 0, 9000);
            // 测试怪的目标必须由防守优先级链重新决定，不继承构造器内的踱步/战术点。
            e._specialTacticalTarget = null;
            e._tacticalTarget = null;
            e._lastKnownTargetPos = null;
            e.target = null;
        }
        ...
        this._toast(`🧪 已生成参与AI的正式测试${label}：${e.name || this._key}`);
    },

    /** 仅怪物/友军选中时显示，并切换对应按钮文案。 */
    _syncTestButton() {
        const visible = this._isCombatEntityKind();
        const busy = this._visualLoadBusy || this._testSpawnBusy;
        if (this._retryVisualBtn) {
            this._retryVisualBtn.hidden = !visible || !this._visualLoadError;
            this._retryVisualBtn.disabled = busy;
        }
        if (this._testRowEl) this._testRowEl.style.display = visible ? 'flex' : 'none';
        if (this._testBtnEl) {
            this._testBtnEl.classList.toggle('active', busy);
            this._testBtnEl.disabled = busy || !this._entity || !!this._visualLoadError;
            this._testBtnEl.textContent = busy
                ? '⏳ 载入资源…'
                : (this._kind === 'friendly' ? '🧪 测试友军' : '🧪 测试怪物');
        }
    },
    ...
    _removePreview() {
        ...
        if (this._previewLabel) { this._previewLabel.destroy(); this._previewLabel = null; }
        this._entity = null;
        // 墙/门/障碍物/陷阱的预览贴图
    ...
    _reset() {
        ...
        if (this._visualLoadBusy) {
            this._toast('⏳ 当前资源仍在载入，请稍候再重置');
            return;
        }
        if (this._kind === 'trap') {
            const zd = DungeonConfig.raw[TRAP_KEY] || (DungeonConfig.raw[TRAP_KEY] = {});
    ...
    _redraw() {
        ...
        // 与正式测试单位区分：冻结预览始终保持半透明。
        const previewSprite = this._getEntitySprite(e);
        if (previewSprite?.active && Math.abs(previewSprite.alpha - PREVIEW_ALPHA) > 0.001) {
            previewSprite.setAlpha(PREVIEW_ALPHA);
        }
        if (this._isCombatEntityKind() && e._editorFrozen) {
            const scene = window.__phaserScene;
            if (!this._previewLabel && scene) {
                this._previewLabel = scene.add.text(0, 0, '冻结预览 · 不参与AI', {
                    fontFamily: 'Microsoft YaHei, sans-serif',
                    fontSize: '12px',
                    color: '#ffd18a',
                    backgroundColor: 'rgba(40, 24, 10, 0.82)',
                    padding: { x: 5, y: 3 },
                }).setOrigin(0.5, 1).setDepth(999999.5);
            }
            this._previewLabel?.setPosition(c.x, topY - 12 / zoom).setScale(1 / zoom);
        }
