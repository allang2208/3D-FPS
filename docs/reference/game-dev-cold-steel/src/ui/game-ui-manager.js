...
function timelineEventHoverCopy(event) {
    ...
    return {
        ...
        timing: event.timeLabel,
        hint: event.detail || (event.status === 'active' ? '事件已经触发' : '当前游标抵达竖线时触发'),
    };
}
...
// 统计只跟随当前现场与游戏时钟；切场/读档人口视图换代后清空历史，不把后台入账算成本地产出。
...
export const GameUIManager = {
    ...
    async load() {
        ...
        if (window.DungeonMapSystem?.active || MailStore.run?.status === 'active') {
            alert('请先结束地牢探险再读档；本版本不支持地牢断点续玩'); return;
        }
        ...
        if (window.SceneManager?.isLoading || WorldStrategySystem._busy || WorldStrategySystem.inBattle) {
            alert('请在场景加载完成、遭遇战结束后读档'); return;
        }
        const wasStrategic = WorldStrategySystem.active;
        let data, restoredMail, restoredMap;
        try {
            ...
            alert(`读档失败，当前地图和信箱未被替换：${error.message || '存档损坏'}`);
            return;
        }
        window.MailboxPanel?.reset();
        if (!this.player) return;
        const positionBeforeLoad = { x: this.player.x, y: this.player.y };
        if (data.worlds?.strategy?.army && (window.SceneManager?.isDungeonRunActive() || window.SceneManager?.isQuestInstance())) {
            alert('请先结束当前地牢或调查，再读取亲征军团存档'); return;
        }
        // Map authority must precede progression, sites, routes and troop restoration.
        ...
        // 旧档只要位面接通即可研究；已完成、正在研究或已有进度都视为早已取得控制权。
        ...
        // 科技恢复必须晚于位面事件恢复，否则合法的位面研究会被未就绪的门槛误删出队列。
        TechnologySystem.restore(savedTechnology, {
            legacyUnlockAll: !savedTechnology, legacyExpeditionEstablished,
        });
        TechnologySystem.notifyWorldRequirementChanged('world-special-building-restore');
        // 世代信息就绪后再清理旧档的同类重复目标，避免借用上一局位面状态。
        reconcileWorldContinuousUpgrades();
        PartySystem.restoreState(data.party);
        TroopLineSystem.restore(data.worlds?.troopLines, { deferSceneEntry: !!data.worlds?.strategy?.army || wasStrategic });
        window.WorldInvasionSystem?.restore?.(data.worlds?.invasion, { deferLive: !!data.worlds?.strategy?.army || wasStrategic });
        ...
        // Even pre-strategy saves must discard another run's armies/sites and event queue.
        try { await WorldStrategySystem.restore(data.worlds?.strategy); }
        catch (error) { console.error('[WorldStrategy] restore failed', error); alert('军团存档已读取，但地图加载失败，请重试读档'); return; }
    ...
    async save() {
        if (this._saveBusy || this._loadBusy) return;
        if (window.DungeonMapSystem?.active || MailStore.run?.status === 'active') {
            alert('请通关、安全撤离或结束地牢后保存；探险暂存不能脱离本次探险单独存档'); return;
        }
        if (!this.player) return;
        if (window.WorldProgressionSystem?.isDevWorldUnlocked?.(window.SceneManager?.currentScene)) {
            alert('开发测试位面不写入正式存档，请先返回主神空间');
            return;
        }
        if (window.SceneManager?.isLoading || WorldStrategySystem._busy || WorldStrategySystem.inBattle) {
            alert('请在遭遇战结算并返回大地图后保存；军团行军中可以保存'); return;
        }
