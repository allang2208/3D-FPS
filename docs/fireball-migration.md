...
- 锁定改准星 15° 锥形最近敌人（参照冰锥），回退旧版 aimRadius 距离判定；
- 传导/伤害/蓝紫闪电链特效原已正常；验证：偏离准星目标锁定成功、主 56/传导 50。

## 声音修复 + 火球特效 100% 对照（`9f45a53` + 工作区）

- 声音：主场景相机挂 AudioListener3D（3D 音效听点；工作区已生效，随并行线提交 main.gd 落库）；
- 火球特效：尾迹橙黄渐小、爆炸三层（橙冲击波描边+填充、26 粒橙黄 ADD、8 粒灰烟尘渐大），
  全部对照旧版 fireball-system.js 参数换算。

## 闪电链连续光带（`ea7ec45`）

- 对照旧版 lightning-bolt.js：0.03m 加密重采样 + 三层圆块连续重叠
  （紫辉光 ADD / 蓝紫 NORMAL / 白芯 ADD），施法端粗→目标端细；
- 消除"点链接"生硬感，验证为连续宽带（紫色像素 34 万+）。

## 闪电链四层补全（`6612ab5`）

- 补全旧版四层：外层辉光 30→5px + 第二层辉光 19→4px + 色块 11→2px + 白芯 5→1px
  （×0.014 换算），施法端 0.42m 粗→目标端细；
- 验证：紫色像素 100 万+，ASCII 明显后粗前细连续光带。

## 火球悬浮加大（`86236a1`）

- 外包络由发射球半径+初速+寿命决定（只调 scale 不变大）；
- 三层火焰整体放大 + 火舌更高更久，火焰团高 256→355px、白热芯+黄焰饱满。

## 火球精准瞄准（`736c5a7`）

- 用户确认"打准星位置、不追踪目标"（`6cf1ca3`）：发射瞬间指向准星射线命中点，
  飞行直线不拐弯；验证：准星线上命中、偏离目标不追踪。

## 火球转身可见性（`f475f06`）

- 悬浮跟随相机 basis 正确（隔离验证转身后仍在相机前方）；
- 握持位调近调高（0.7m）防地形/植被遮挡——转身后火球仍贴左手可见。
