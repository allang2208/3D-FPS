...
**第一人称手模基准（用户于 2026-09-25 确定）**：以后新增、迁移和调整动画统一以当前认可的 V7 裸手、裸臂开发，游戏默认也使用这套。手套、衣袖作为装备外观匹配该手型并复用动画；旧原版手套仅保留为装备恢复版本。具体母版、各原生骨架派生及默认接入约定见 [认可的裸手基准](skills/ue5-fps-arms-animation/references/accepted-bare-hands.md)。此条替代旧文中默认使用原版手套手模的约定。
